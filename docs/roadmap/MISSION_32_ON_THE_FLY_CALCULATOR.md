...
For each schedulable job, or each phase or work package of a longer job, the Studio should recommend a concrete proposed time window with the specific eligible employees or crew and required named equipment when enough scope, duration, availability and resource evidence exists. The proposal must account for working calendars, qualifications, travel and setup where supported, dependencies, approved assignments, and exclusive assets. One employee or machine cannot be committed to overlapping jobs; separate jobs may run concurrently when their qualified people and required exclusive equipment do not conflict. A crew label alone does not prove its members are free. If no feasible window can be established, show why and what availability or resource fact needs review instead of inventing a booking. Mission 32 supplies proposed execution requirements and duration; Mission 22 remains authoritative for scheduling, assignment and dispatch. Released Mission 22 ranks people for an existing appointment interval, so future slot search and cross-job exclusive-asset commitment evidence require additive Mission 22-owned interfaces before the Studio can claim such recommendations. A recommendation never reserves people or equipment or creates an appointment by itself.
...
- Scheduling validation with named people and exclusive equipment: generate and recheck feasible candidate windows for each schedulable job or phase; prevent overlapping commitments of the same person or asset using source-owned cross-job records; permit concurrent jobs using disjoint qualified resources; and return an explicit unavailable reason when a safe window or asset commitment cannot be established. Recheck the proposal through Mission 22 at the moment of human-approved booking.
