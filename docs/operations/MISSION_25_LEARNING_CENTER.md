# Mission 25 Owner Learning Center

Mission 25 Part 7 adds a separate **Learning Center** destination for owners and administrators. It inventories the tenant's native completed-job comparison consent and each recorded external labor source, then opens the existing protected authorities for source consent, imported-outcome consent, calibration consent, staged evidence, worker and job reference review, and service-level calibration proposals.

The paid page reads PostgreSQL through `canonical_learning_center_read` and the previously released guarded learning entry functions. Runtime access remains unable to select the protected source tables or call their internal projection and basis helpers. Members and viewers have no `learning` permission and the route also enforces the owner or administrator role.
