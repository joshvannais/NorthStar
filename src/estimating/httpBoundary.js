...
function estimateDecisionBodyBoundary(req,res,next){
 if(req.method!=='POST'||!/^\/api\/(?:v1\/canonical|demo\/command-center)\/estimates\/[^/]+\/(?:decisions|equipment-plans|equipment-plan-preview|labor-plans|labor-plan-preview|material-plans|material-plan-preview|material-adoptions|material-adoption-preview|cost-adoptions|cost-adoption-preview)\/?$/.test(rawRequestPath(req)))return next();
