...
function team(v,legacy,profile){
 if(!v)return {members:legacy.members.map(m=>({...m,profileId:m.id,skillIds:[],homeLocationId:null})),crews:[],skills:[],services:profile.services.map(s=>({id:s.key,name:s.label})),locations:[],invitations:[],policies:[],schedulingEvidenceAvailable:false,simulated:true};
 return {members:v.members.map(m=>({...m,skillIds:m.serviceIds})),crews:v.crews.map(c=>({...c,key:c.id,members:c.memberIds.map(id=>({profileId:id,role:id===c.leadMemberId?'lead':'member'}))})),skills:v.serviceKeys.map(key=>({id:key,key,name:(profile.services.find(s=>s.key===key)||{}).label||key,serviceId:key})),services:profile.services.map(s=>({id:s.key,name:s.label})),locations:[{id:'headquarters',name:'Simulated Business Location'}],invitations:[],policies:[],schedulingEvidenceAvailable:true,simulated:true,availabilityCoverage:{start:v.coverageStart,end:v.coverageEnd}};
}
