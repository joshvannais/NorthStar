...
- Focused mounted Part 6 PostgreSQL plus contract tests: 2/2 suites and 15/15
  tests green. The mounted suite covers the employee direct
...
- Explicit Mission 22 Parts 1–6 compatibility selection: 13/13 suites and
  172/172 tests green in 60.845 seconds.
- Locally available full Jest corpus, excluding only the separately documented
  unavailable account-migration environment: 153/153 suites and 2,133/2,133
  tests green in 782.88 seconds; zero snapshots.
...
- Browser evidence: eight matrices, 96 screenshots, Chrome
  `151.0.7922.175` and actual Playwright WebKit `26.5`, desktop/mobile and
...
The superseding recovered-writer raw results and screenshot package are under
`outputs/m22-part6-correction-writer`; filenames identify `e72792d` and the
required `max_connections=100` full run.
