...
process.env.AUTH_ACCESS_SECRET = 'm25-external-project-change-order-local-disposable-secret';
...
const output = (process.argv.find(value => value.startsWith('--output=')) || '--output=m25-external-project-change-order-result.json').slice(9);
assert.ok(!fs.existsSync(output));
const sourceKey = 'projects.primary';
const evidenceDigest = crypto.createHash('sha256').update('opaque-project-change-order-provider-evidence').digest('hex');
...
const base = (recordType, externalRecordId) => ({
  ...
  customerReference: 'customer-ext-1', jobReference: 'job-ext-1', estimateReference: 'estimate-ext-1',
  projectReference: 'project-ext-1', changeOrderReference: recordType === 'change_order' ? 'change-ext-1' : null,
  ...
  occurredAt: '2026-09-15T12:00:00.000Z', endedAt: null,
...
(async () => {
  ...
  try {
    ...
    const root = `/api/v1/learning/external-project-change-order-sources/${sourceKey}`;
    ...
    const grant = { action: 'grant', expectedRevision: 0, expectedDigest: 'none',
      reason: 'Use this reviewed project and change-order source.', confirmed: true,
      confirmationVersion: 'm25-external-project-change-order-import-consent-v1' };
    ...
    const adminRoot = '/api/v1/learning/external-project-change-order-sources/projects.admin-control';
    ...
    const maximumRecords = Array.from({ length: 100 }, (_, index) => ({ ...base('project', `project-max-${index}`),
      projectReference: `project-max-${index}`, timeZone: index % 2 ? 'America/Chicago' : 'America/New_York' }));
    ...
    const maximumPage = await request(fixture.app).post(adminRoot + '/batches').set(fixture.actors.admin.session.headers)
      .set('X-CSRF-Token', fixture.actors.admin.csrfToken).set('Idempotency-Key', crypto.randomUUID()).send({
        schemaVersion: 'm25-external-project-change-order-v1', mode: 'historical_backfill',
        ...
        reason: 'Verify the maximum reviewed project page.', confirmed: true,
        confirmationVersion: 'm25-external-project-change-order-import-batch-v1',
      });
    ...
    const records = ['project','change_order'].map(type => base(type, `${type}-1`));
    const batch = (changes = {}) => ({ schemaVersion: 'm25-external-project-change-order-v1', mode: 'historical_backfill',
      expectedConsentRevision: consent.revision, expectedConsentDigest: consent.digest, cursorBefore: null, cursorAfter: null,
      complete: true, records, reason: 'Import one reviewed normalized project and change-order page.', confirmed: true,
      confirmationVersion: 'm25-external-project-change-order-import-batch-v1', ...changes });
    ...
    response = concurrent.find(value => value.status === 201); assert.equal(response.body.data.run.insertedCount, 2);
    assert.equal(concurrent.find(value => value.status === 200).headers['idempotency-replayed'], 'true');
    ledger.cases.push('One bounded historical page stages project and change-order evidence; concurrent exact-key retry creates one immutable run.');
    ...
    assert.equal(source.body.data.recordTotal, 2); assert.equal(source.body.data.currentRecords.every(value => value.reconciliationStatus === 'unmatched'), true);
    assert.equal(source.body.data.currentRecords.find(value => value.recordType === 'project').currentContract.amount, '132500.00');
    assert.equal(source.body.data.currentRecords.find(value => value.recordType === 'change_order').changeOrderValue.amount, '7500.00');
    ...
    assert.equal(administrator.status, 200); assert.equal(administrator.body.data.recordTotal, 2);
    ...
    const directBatch = async body => { const client = await fixture.runtimePool.connect(); try {
      ...
      await client.query('SELECT public.canonical_external_project_change_order_import_batch($1,$2,$3,$4,$5,$6,$7,$8::jsonb)',
    ...
    const invalidZone = base('project','invalid-zone'); invalidZone.timeZone = 'EST';
    ...
    const invalidValue = base('project','invalid-value'); invalidValue.currentContract = { status: 'recorded', amount: null, currency: 'USD' };
    ...
    const crossPurpose = base('project','cross-purpose'); crossPurpose.changeOrderValue = changeValue('1.00');
    ...
    const duplicatePage = base('project','duplicate-page');
    ...
    ledger.cases.push('Database and API boundaries reject missing consent pins, invalid time zones, malformed or cross-purpose contract values, and ambiguous duplicate page identities.');

    response = await write('/batches', batch({ mode: 'continuous_update', complete: false, cursorAfter: 'duplicate-1', records }));
    assert.equal(response.status, 201, JSON.stringify(response.body)); assert.equal(response.body.data.run.duplicateCount, 2);
    const staleCursor = await write('/batches', batch({ mode: 'continuous_update', complete: false, cursorBefore: 'wrong', cursorAfter: 'wrong-2', records: [base('project','stale-cursor')] }));
    assert.equal(staleCursor.status, 409); assert.equal(staleCursor.body.error.code, 'M25_PROJECT_CHANGE_ORDER_IMPORT_CHANGED');
    const corrected = { ...base('project','project-1'), externalVersion: 2, recordState: 'unknown',
      currentContract: { status: 'unavailable', amount: null, currency: null },
    ...
    assert.equal(source.body.data.currentRecords.find(value => value.externalRecordId === 'project-1').currentContract.status, 'unavailable');
    ...
    assert.equal(conflict.status, 409); assert.equal(conflict.body.error.code, 'M25_PROJECT_CHANGE_ORDER_IMPORT_RECORD_CONFLICT');
    ...
    source = await request(fixture.app).get(root).set(owner.session.headers);
    const removed = source.body.data.currentRecords.find(value => value.externalRecordId === 'project-1');
    assert.equal(removed.state, 'tombstone'); assert.equal(removed.projectReference, null); assert.equal(removed.currentContract, null); assert.equal(removed.reconciliationStatus, 'unavailable');
    ...
    const revoke = { action: 'revoke', expectedRevision: consent.revision, expectedDigest: consent.digest,
      reason: 'Stop using this project and change-order source.', confirmed: true,
      confirmationVersion: 'm25-external-project-change-order-import-consent-v1' };
    ...
    response = await write('/batches', batch({ records: [base('project','project-1')], reason: 'Explicitly import current evidence into the new permission period.' }));
    ...
    const privileges = (await fixture.ownerPool.query(`SELECT
      has_table_privilege($1,'canonical_external_project_change_order_import_records','SELECT') table_read,
      has_function_privilege($1,'canonical_external_project_change_order_record_projection(canonical_external_project_change_order_import_records)','EXECUTE') helper,
      has_function_privilege($1,'canonical_external_project_change_order_import_read(uuid,uuid,text,uuid,text)','EXECUTE') entry`, [fixture.roles.runtime])).rows[0];
    assert.deepEqual(privileges, { table_read: false, helper: false, entry: true });
    await assert.rejects(fixture.ownerPool.query('DELETE FROM canonical_external_project_change_order_import_records'));
    const bytes = fs.readFileSync(path.join(__dirname,'../../migrations/114_canonical_external_project_change_order_import_authority.sql'));
    const checksum = crypto.createHash('sha256').update(bytes).digest('hex');
    assert.deepEqual((await fixture.ownerPool.query("SELECT trim(checksum) checksum,count(*) OVER()::int rows FROM _migrations WHERE filename='114_canonical_external_project_change_order_import_authority.sql'")).rows, [{ checksum, rows: 1 }]);
