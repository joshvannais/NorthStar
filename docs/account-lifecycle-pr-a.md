...
The final bounded follow-up found two remaining fail-closed defects: the physical-ledger metadata preflight did not reject comments attached separately to ledger constraints or index relations, and the mounted Jobber journey could exchange credentials before discovering that canonical token persistence was unavailable. This final correction is limited to those two defects, authentic tests, and evidence accuracy. It does not reopen the closed BroadcastChannel, refresh, onboarding, notification-preference, logout, Bearer-retirement, or opaque-state confidentiality work.

The separately reported migration-010 hash discrepancy was not an implementation defect. It came from hashing a CRLF-converted checkout representation instead of the exact committed Git bytes. The authoritative provenance and binary-safe hashes are recorded below.
...
The first 22 published commits remain unchanged. This final correction adds three linear commits for ledger-comment rejection, mounted Jobber fail-closed behavior, and test/documentation evidence. The result is exactly 25 linear commits above base `dfff096241d3be4fd1580a741cfe21ee64a5dfb3` with zero merge commits. The draft PR description records all three exact SHAs, parents, final head, and final diff after publication; a commit cannot truthfully embed its own content-derived SHA.
...
Migration 010 is absent from base `dfff096241d3be4fd1580a741cfe21ee64a5dfb3` and was introduced at `137ad6d473fac69fd5b7ee81aea5e513f3a1e7b4`. Its introduction-time committed bytes hash to `cac651ea70624f013377e21e74b393a5133f5f6551aed20939a12014ea040a1b`. At `9ec1812630a54be3811ec94155824abc868cecdd`, `b794033e22874145fe7de8708b66c28b5e509b75`, and this final correction, the exact 14,419-byte committed LF blob hashes to `0087278b1fb0062ba88a4dd7e4699e2e5c4c98d78e822193e2e7c0bff5c9ca48`. Git plumbing supplies the blob bytes directly to SHA-256; no text-mode reader or checkout newline conversion participates. The previously reported `fe78838214f05ea4a76325fd0881e1b8168103d2cff84d1636ad3b0baeae4fcb` is the hash of a 14,763-byte CRLF-converted representation and is a reviewer newline-normalization artifact. A disposable one-byte alteration fails the protected-hash assertion. Migrations 001-010 remain unchanged by this final correction; migration 011 remains the only new production migration.

## Local correction validation

The final local source state passed the real PostgreSQL 18 focused migration gate (24/24), the mounted/lower-level Jobber OAuth gate (20/20), mounted external-family gate (9/9), and account-containment gate (4/4). The complete API run passed 10 suites and 129 tests. Complete serial Jest passed 54 suites and 1,095 tests. PostgreSQL-required suites used the explicitly identified disposable PostgreSQL 18.4 cluster and did not count missing-identity skips as passing. The changed production code introduces no timer, worker, listener, process, or other handle, so a new `--detectOpenHandles` campaign was not required for this bounded delta.
