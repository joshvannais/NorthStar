...
const importContract = require('./externalMaterialImportContract');
const DIGEST = /^[0-9a-f]{64}$/;

function fail(message) {
  const error = new Error(message);
  error.code = 'M25_MATERIAL_IMPORT_OPERATIONS_INPUT_INVALID';
  error.status = 400;
  throw error;
}
...
function normalizeHold(sourceKey, body) {
  const keys = ['action', 'holdKind', 'expectedRevision', 'expectedDigest', 'confirmed'];
  if (!exact(body, keys) || !['place', 'release'].includes(body.action) ||
      !['legal', 'audit'].includes(body.holdKind) || !pair(body) || body.confirmed !== true) {
    fail('Source hold details are invalid.');
  }
  return Object.freeze({ sourceKey: importContract.normalizeSourceKey(sourceKey), ...body });
}
...
module.exports = { normalizeAdapter, normalizeRetention, normalizeDeletion, normalizeHold, normalizeCleanup };
