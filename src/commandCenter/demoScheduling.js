...
function evidence(workspace,proposal,state,graph,now=new Date()){
 ...
 function evaluate(c){const candidateProposal={...proposal,target:c?{kind:c.kind,id:c.targetId}:proposal.target},base=proposal.scheduledStart===null?{status:'needs_review',hardConflicts:[],warnings:[],needsReview:true,reviewReasons:[{code:'appointment_schedule_unavailable'}]}:evaluateConflictEvidence({proposal:{...proposal,target:c?{kind:c.kind,id:c.targetId}:proposal.target},candidate:c,appointment,businessProfile:profile,skillAuthorityKnown:Boolean(v&&v.serviceKeys.includes(appointment.serviceId)),schedules,scheduleSetTruncated:false,workloadSchedules:schedules,workloadSetTruncated:false});const equipment=require('../scheduling/equipmentReadiness'),item={ids:graph.ids,snapshot:graph.polaris.snapshot},basis=equipment.demoBasis(state,item,candidateProposal,now),addition=equipment.extra(basis,candidateProposal,now);const travel=require('../scheduling/travelReadiness'),travelBasis=travel.demoBasis(state,item,candidateProposal,now),travelAddition=travel.extra(travelBasis,candidateProposal,now);return {...travel.merge(equipment.merge(base,addition),travelAddition),...(basis.notRecorded?{}:{equipmentEvidenceDigest:addition.digest}),...(travelBasis.notRecorded?{}:{travelEvidenceDigest:travelAddition.digest})};}
...
function preview(workspace,state,input,now){
 ...
 const evaluation=evidence(workspace,normalized.proposal,state,graph,now),id=uuid(workspace.session.id+':preview:'+input.idempotencyHash,NS);
...
function approve(workspace,state,input,now){
 ...
 const latest=evidence(workspace,found.response.proposal,state,graph,now);if(sha256(latest.conflicts)!==sha256(found.response.conflicts)||sha256(latest.recommendation)!==sha256(found.response.recommendation)||latest.conflicts.hardConflicts.length)fail(409,'DEMO_SCHEDULE_EVIDENCE_CHANGED','Scheduling information changed. Review the appointment again.');
