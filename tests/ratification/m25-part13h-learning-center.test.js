...
describe('Mission 25 Part 13H acceptance contract', () => {
  test('keeps service discovery bounded and the prior projection helper private', () => {
    ...
    expect(migration).toContain('canonical_learning_center_part12_read'); expect(migration).toContain('outcomeServicesTruncated');
  ...
  test('renders a plain owner review with explicit adoption and rollback decisions', () => {
    ...
    for (const phrase of ['Review for adoption','Adopt planning value','Review rollback','Remove planning value','Earlier suggestions do not become current again']) expect(page).toContain(phrase);
