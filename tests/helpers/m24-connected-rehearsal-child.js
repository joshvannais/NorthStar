...
(async()=>{let fixture,db,server,ownerPool;try{
 let app,metadata={};
 ...
 const canaryRow=(await db.getPool().query("SELECT v.id,v.organization_id,i.external_integration_id agent_id FROM canonical_voice_sessions v JOIN canonical_integration_ownership i ON i.id=v.integration_ownership_id WHERE v.status='active' AND v.provider='retell' AND i.status='active' ORDER BY v.started_at DESC,v.id DESC LIMIT 1")).rows[0];
 const express=require('express'),actual=require(path.join(source,'src/server')).app;
 const accounting=require(path.join(source,'src/polaris/providerAccounting')),countAccounting=accounting.parseCountAccounting({POLARIS_INPUT_COUNT_ACCOUNTING_VERSION:accounting.COUNT_ENDPOINT_VERSION,POLARIS_INPUT_COUNT_MAX_COST_NANO_USD:'0',POLARIS_INPUT_COUNT_TARIFF_EVIDENCE_DIGEST:'a'.repeat(64),POLARIS_INPUT_COUNT_TARIFF_REVIEWED_ON:'2026-09-14'});
 ...
 const canaryBinding=canaryRow&&JSON.stringify({organizationId:canaryRow.organization_id,agentId:canaryRow.agent_id,agentVersion:1,llmId:'fixture-llm',llmVersion:1,basePromptDigest:'b'.repeat(64),consentVersion:'fixture-consent-v1',exclusive:true,syntheticOnly:true,maxGenerationsPerCall:1,maxCallDurationMs:60000});
 ownerPool=new (require(path.join(source,'node_modules/pg')).Pool)({connectionString:process.env.MIGRATION_DATABASE_URL,max:1});await require(path.join(source,'tests/helpers/m24-provider-canary-attestation')).attest(ownerPool,canaryRow.id,canaryBinding);
 app.locals.connectedCallGenerate=require(path.join(source,'src/polaris/callGenerationBinding')).createProductionCallGenerate({POLARIS_CALL_GUIDANCE_ENABLED:'true',POLARIS_GROUNDED_V2_ENABLED:'true',POLARIS_CALL_CANARY_BINDING:canaryBinding},{getPool:()=>db.getPool(),runtime});
 ...
 await new Promise(r=>server.close(r));if(ownerPool)await ownerPool.end();if(fixture)await fixture.cleanup();else await db.close();process.disconnect();
