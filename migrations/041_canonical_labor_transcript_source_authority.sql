-- Mission 23 Part 3 terminal audit correction only.
-- Migrations 039 and 040 remain frozen byte-for-byte. This forward-only
-- migration installs one fail-closed transcript-source classifier and reuses it
-- before labor mutation replay and labor reads.

CREATE OR REPLACE FUNCTION public.canonical_labor_transcript_source_normalized(source_value TEXT)
...
BEGIN
  normalized_value := translate(
    btrim(source_value, trim_characters),
    'ABCDEFGHIJKLMNOPQRSTUVWXYZ',
    'abcdefghijklmnopqrstuvwxyz'
  );
  IF normalized_value !~ '^[a-z]+$'
    OR normalized_value NOT IN ('demo','lead','retell','simulation','voice') THEN
    RETURN NULL;
  END IF;
  RETURN normalized_value;
END $function$;

REVOKE ALL ON FUNCTION public.canonical_labor_transcript_source_normalized(TEXT) FROM PUBLIC;
...
BEGIN
  ...
  SELECT execution.* INTO execution_record FROM public.canonical_field_executions execution
   ...
   WHERE execution.organization_id=organization_id_value AND execution.id=execution_id_value
     AND public.canonical_labor_transcript_source_normalized(transcript.source) IN ('lead','retell','voice') FOR UPDATE OF execution;
