...
const MIGRATION = '053_current_worker_execution_projection.sql';
const quote = value => '"' + String(value).replace(/"/g, '""') + '"';

real('Mission 23 Part 9A additive current-worker projection migration', () => {
  let database;
  let ownerPool;
  let runtimePool;
  let roles;
  ...
  beforeAll(async () => {
    database = await createSuiteDatabase('m23p9a-projection-migration');
    const suffix = `${process.pid}_${crypto.randomBytes(3).toString('hex')}`;
    roles = { owner: `m23p9a_owner_${suffix}`, runtime: `m23p9a_runtime_${suffix}` };
    ...
    preceding = fs.mkdtempSync(path.join(os.tmpdir(), 'northstar-part9a-pre053-'));
    const migrations = path.resolve(__dirname, '../../migrations');
    for (const name of fs.readdirSync(migrations)
      .filter(name => name.endsWith('.sql') && Number(name.slice(0, 3)) < 53)) {
    ...
    await ownerPool.query(
      "INSERT INTO public.organizations(id,name,email) VALUES('f9000000-0000-4000-8000-000000000001','Part 9A upgrade sentinel','part9a-upgrade@example.test')"
    );
  ...
  afterAll(async () => {
    ...
    if (preceding && path.dirname(preceding) === os.tmpdir() &&
        path.basename(preceding).startsWith('northstar-part9a-pre053-')) {
      fs.rmSync(preceding, { recursive: true });
    }
  }, 180000);

  test('rolls back interruption, preserves 001-052 and existing data, applies 053 once, and grants only the entry read', async () => {
        ...
        return {
          query: async (...args) => {
            ...
            if (!intercepted && typeof args[0] === 'string' &&
                args[0].includes('-- Mission 23 Part 9A:')) {
              intercepted = true;
              throw new Error('Deterministic interruption after 053 DDL before ledger commit');
            }
            return result;
          },
    ...
    expect((await ownerPool.query(
      "SELECT to_regprocedure('public.canonical_field_execution_read_by_appointment(uuid,uuid,text,uuid,uuid)') AS authority"
    )).rows[0].authority).toBeNull();
    expect((await ownerPool.query(
      'SELECT filename,checksum,applied_at FROM public._migrations ORDER BY filename'
    )).rows).toEqual(before);
    expect((await ownerPool.query(
      "SELECT name,email FROM public.organizations WHERE id='f9000000-0000-4000-8000-000000000001'"
    )).rows).toEqual([{ name: 'Part 9A upgrade sentinel', email: 'part9a-upgrade@example.test' }]);
    ...
    const metadata = (await ownerPool.query(
      `SELECT procedure.prosecdef,procedure.provolatile,procedure.proparallel,procedure.proconfig,
        ...
        WHERE procedure.oid='public.canonical_field_execution_read_by_appointment(uuid,uuid,text,uuid,uuid)'::regprocedure`
    )).rows[0];
    ...
    expect((await ownerPool.query(
      `SELECT has_function_privilege($1,
                'public.canonical_field_execution_read_by_appointment(uuid,uuid,text,uuid,uuid)','EXECUTE') AS entry,
              has_function_privilege('public',
                'public.canonical_field_execution_read_by_appointment(uuid,uuid,text,uuid,uuid)','EXECUTE') AS public_entry,
