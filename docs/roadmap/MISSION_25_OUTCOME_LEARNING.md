...
Slice D is implemented as a candidate at migration 127. An owner or administrator separately permits cross-job proposal preparation and explicitly selects five to 100 distinct current Part 13C summaries for one exact normalized service. Every summary, graph, evaluation, source digest and permission period is pinned. Comparable measures retain their exact source class, unit or currency; jobs have equal weight and duplicate values from one job fail closed. Each measure reports sample size, cohort coverage, median, lower and upper quartiles and interquartile range. A multiplier is offered only inside the inclusive 0.25 to 4.00 review range, while a recorded gross-margin percentage remains bounded to zero through 100. Insufficient, stale, conflicting, incompatible or unsupported evidence remains unavailable. Scope remains unavailable because current scope records do not establish one reusable numeric planning value. Revocation or any pinned lineage change masks earlier advice, and a later permission period revives nothing. The versioned registry, impact preview, adoption, lifecycle propagation and rendered experience remain Slices E-H.

Slice E is implemented as a candidate at migration 128. One exact current Part 13D proposal becomes a deterministic owner-readable impact preview. Supported multiplier advice is expressed only as a relative planning review; a connected gross-margin percentage remains reference-only, and absolute company values remain unavailable. An owner or administrator may explicitly save the preview as an immutable per-service registry revision that pins the current proposal permission period, proposal canonical and source digests, exact preview digest, prior revision, calculation version, actor authority and request identity. Any pinned lineage or permission change makes the revision stale and masks its detail. Revocation hides the period, and a later grant starts empty without reviving earlier registry versions. Saving a preview changes no planning or operating authority. Explicit adoption, lifecycle propagation and the rendered experience remain Slices F-H, while native financial truth remains Mission 27.

Slice F is implemented as a candidate at migration 129. The current company owner may explicitly adopt one exact supported multiplier from the current saved Part 13E registry version into an immutable per-service planning-value history. Labor, travel, vehicle and equipment, and material planning remain separate; scope, connected-financial reference values, unavailable evidence and absolute amounts cannot be adopted. Each adoption pins the registry, preview, selected measure, prior planning revision, actor authority and request identity. Exact current retries are idempotent and concurrent changes serialize. An explicit rollback can restore one exact earlier value or leave it unset, including when learning permission is unavailable. A later source or permission change marks the adopted value as needing review but does not automatically remove or replace it. Lifecycle propagation and the rendered experience remain Slices G-H, and native financial truth remains Mission 27.

Slice G is implemented as a candidate at migration 130. The owner planning-value read now follows the exact current adoption through its saved proposal, preview, selection and permission lineage. It separately reports later owner planning revisions, newer saved proposals, ended permission, a new permission period and connected evidence that was corrected, removed or became unavailable. A later permission period cannot revive earlier learning. The adopted multiplier remains in effect while review is required; propagation never creates a planning revision or mutates an estimate, price, schedule, job, asset, material, financial record, provider record, business profile or policy. The paid and isolated-demo experience remains Slice H, and native financial truth remains Mission 27.
...
The paid-tenant acceptance journey uses one mounted production application and a fresh PostgreSQL database to prove the complete path from provider-neutral source permission and import, through exact reviewed reconciliation, single-job observation and robust five-job calibration, to cross-source graph evaluation, complete job summaries, a pinned cross-job proposal, owner-readable preview and explicit owner adoption and rollback. Migration `132_canonical_paid_learning_calibration_lineage.sql` makes imported-labor advice depend on the exact current calibration and pins its proposal, permission, sample and calculation identities in the selected-job manifest.

The journey proves exact replay, tenant isolation, current source and purpose permission, complete source manifests, current reviewed links and adopted-plan lineage. Missing calibration blocks proposal generation. Correction, removal, revocation or a replacement permission period retires the pinned advice and cannot revive an earlier result. Learning remains advisory until the company owner explicitly adopts one supported value, and adoption or rollback does not rewrite customers, estimates, cost plans, source evidence, financial records or other operating truth. The isolated-demo journey, mission-wide recovery, performance, accessibility, responsive review, independent audit, merge, deployment and founder verdict remain Slices B-G.

## Part 14 Slice B candidate — complete resettable fictional demo journey

The account-free Learning Center now uses its existing PostgreSQL-backed, cookie-bound fictional workspace for a complete owner walkthrough. Each visitor explicitly allows the two fictional record uses, selects reviewed completed jobs, prepares a suggestion, reviews its exact sample and uncertainty, saves the review, adopts one future-planning multiplier, removes it and can reset the whole fictional journey. The journey uses bounded compare-and-swap revisions, idempotency records, action limits and same-origin intent checks. A stale browser or a second visitor cannot alter the first visitor's state.

The demo operation writes only the isolated demo session and demo mutation ledger. It cannot call or write paid learning, business profile, estimate, price, schedule, job, asset, source-evidence, financial or provider authority. Reset creates a fresh deterministic fictional workspace and removes the prior demo journey from current use. Missing, incompatible and financial dimensions stay visibly unavailable; fictional evidence is never presented as a company result. Recovery proof, performance bounds, mission-wide visual and accessibility acceptance, independent audit, release and founder verdict remain Slices C-G.

## Part 14 Slice C candidate — migration and lifecycle recovery proof

A mounted PostgreSQL 17 journey now preserves one populated provider-neutral labor source through repeated production database-pool startup. Migration 134 makes PostgreSQL 17 and 18 migration-ledger catalog representations restart-safe and retires labor-source permission, import and connection retries after their exact permission period changes. Migration 135 returns the same bounded public cleanup-run fields for first execution and exact replay. The first restart leaves all 133 migration filenames, checksums and applied times unchanged, replays the exact current permission, import and connection requests, and resumes a paused connection through its current revision. A current correction replaces the earlier current source version without rewriting history. Explicit revocation hides the source and retires old requests. A later permission period may expose retained staged source evidence under the accepted import authority, while old requests, reviewed matches and downstream learning remain retired until separately renewed.

Retention tombstones both eligible staged records under its exact current policy. Deletion then processes 101 current records in a bounded 100-record page, persists its checkpoint through another database-pool restart, and reproduces the first page's exact documented response after an uncertain retry. It resumes from the exact cursor, finishes once, restarts again and reproduces the completed page's public response. Cleanup replays use camel-case public fields and withhold raw database, tenant, actor, session, request and authority fields. Existing accepted all-source lifecycle suites retain broader source-class coverage. Bounds, performance, concurrency, failure recovery and operational observability remain Slice D. Mission-wide rendered accessibility and responsive review remain Slice E; no rendered path changes in Slice C.

## Part 14 Slice D candidate — bounds, performance, concurrency, failure recovery and operational observability proof

The mounted PostgreSQL 17 proof exercises the existing provider-neutral labor import authority at its exact one through 100 records per request boundary. A full 100-record page completes with headroom under the existing five-second statement limit, while 101 records fail before persistence. Repeated barrier-controlled identical requests create once, reject the still-overlapping request with the serializable-change response and return the winner's exact replay on a later retry. Repeated concurrent different requests sharing a request key keep one winner and return the specific request-key conflict for the loser. A held source lock reaches the existing two-second lock limit, returns a plain temporary-unavailable response with no partial record or request claim, and the exact request succeeds after pressure clears.

Bounded request telemetry reports only a correlation identifier, method class, status and duration. It contains no company, source, cursor, imported record, request-key or request-body content. Full-row digests cover the complete public-table catalog and prove that every table outside the three accepted labor-import history tables and `audit_logs` remains byte-equivalent. Request auditing is the only additional expected write. Its exact bounded tenant, owner, action, route template, status and duration are asserted; raw client and network text is omitted at the durable boundary, entity classes and correlation identifiers fail closed, and stored details contain no request keys, imported records, request body or other source evidence. Migrations 089, 134 and 135 remain byte-identical. No rendered path changes in Slice D; mission-wide accessibility and responsive review remain Slice E.

## Part 14 Slice E candidate — mission-wide accessibility and responsive review

The complete paid and isolated fictional-demo Learning Center journeys are reviewed in Chrome and Playwright WebKit at the canonical 360 by 800 dark, 390 by 844 light, 768 by 1024 dark, 1024 by 768 light and 1440 by 900 dark layouts. The owner journey covers first use, explicit five-job selection, evidence and uncertainty, save, adoption, rollback, stale and unavailable states, administrator review and plain recovery. The fictional journey completes the same review decisions inside its isolated resettable workspace.
...
## Part 12 Slice G candidate — reviewed scope, change-order and project-delivery outcomes

Migration `119_canonical_external_project_outcomes.sql` adds separate current-period permission and immutable observations for one exact canonical estimate using one current reviewed external project reference and the complete current change-order set for that project. Every observation pins the current source and purpose permission periods, reviewed project link, every current change-order reference and reviewed-link state, current NorthStar estimate basis, complete project source manifest, calculation version and request identity.

Scope, change-order and project-delivery outcomes remain independent. Exactly one current project record preserves original and current contract claims; their difference is available only in one exact currency. Change-order state counts require one current record and a same-estimate reviewed link for every distinct current change order. A signed total additionally requires every current value in one currency. Project delivery preserves only the recorded state and interval, and reports duration only when an end time exists. No exchange rate, approval, completion, missing amount, missing change order or causal relationship between contract movement and change orders is inferred.

Corrections, tombstones, source-permission changes, purpose-permission changes, reviewed-link changes and NorthStar target changes stale and mask prior advice. Revocation hides and blocks derived history, and a later grant revives no earlier evidence, link or observation. The result is advisory and changes no project, change order, customer, job, estimate, schedule, price, financial record, imported source record or company policy. External financial outcomes, calibration, lifecycle and Learning Center work remain mandatory Slices H-K.

## Part 12 Slice H candidate — reviewed external financial outcomes

Migration `120_canonical_external_financial_outcomes.sql` adds separate current-period permission and immutable observations for external recorded revenue, collection, realized cost and margin for one exact canonical estimate. Every observation pins the current financial source and purpose-permission periods, the complete relevant reviewed reference and source-manifest set, the current NorthStar estimate basis, calculation version and request identity.

Revenue uses only explicit posted accounting entries with a revenue basis. Collection uses only explicit amount-collected evidence. Realized cost uses only explicit posted direct-cost, overhead-cost and other-cost accounting entries. Each result requires exactly one current record per contributing reference, current same-estimate reviewed links, recorded amounts and one exact currency. Margin requires compatible revenue and cost in the same currency and recorded revenue greater than zero. Invoice and payment states do not become revenue or collection. Missing amounts or records do not become zero, and no exchange rate, reversal sign or completeness is inferred.

Corrections, tombstones, source-permission changes, purpose-permission changes, reviewed-link changes and NorthStar target changes stale and mask prior advice. Revocation hides and blocks derived history, and a later grant revives no earlier evidence, link or observation. The result is advisory and changes no customer, job, estimate, schedule, price, financial record, source record or company policy. Native NorthStar invoice, payment, collection, accounting, revenue, cost and margin authority remains unavailable until Mission 27. Calibration, lifecycle and Learning Center work remain mandatory Slices I-K.

## Part 12 Slice I candidate — source-specific multi-record calibration

Migration `121_canonical_external_business_calibration.sql` adds separate owner or administrator permission and immutable same-service calibration for the accepted customer, project and external financial observations. Customer calibration remains scoped to one exact CRM and communication source pair. Project and financial calibration each remain scoped to one exact source. Every calibration period pins the current outcome-learning permission and cannot survive a source or outcome-permission change.

Five to 100 current observations form one deterministic sample. Each observation contributes one equal-weight job sample and preserves its exact observation identity, revision, digest, source digest and external relationship identity. Customer states are summarized as exact distributions. Project contract change, signed change-order value and delivery duration are summarized separately from delivery state. External recorded revenue, collection, realized cost and margin are summarized independently. Numeric summaries retain median and lower and upper quartiles. Missing values, mixed currencies, stale evidence and incompatible dimensions remain unavailable; the authority does not infer a conversion, causal relationship or missing value.

Calibration is descriptive advice for owner review. It does not adopt a new assumption or change a customer, lead, appointment, estimate, project, change order, job, invoice, payment, price, schedule or company policy. Revocation hides ordinary and replay reads. A later grant starts a new calibration period and cannot revive an earlier proposal. Lifecycle operations and the complete Learning Center remain mandatory Slices J-K.

## Part 12 Slice J candidate — source lifecycle and bounded cleanup

Migration `122_canonical_external_business_source_operations.sql` adds immutable provider-neutral adapter, checkpoint, retention, deletion, legal/audit hold and bounded cleanup authority across the accepted CRM/field-service, project/change-order, communication and external financial source classes. Every source permission, import, hold, deletion and cleanup mutation for one tenant/source uses one transaction lock. A committed hold blocks cleanup, while cleanup that owns the lock finishes before a new hold can commit.

Cleanup processes at most 100 current records and appends minimized tombstones with an exact resumable checkpoint. A deletion request immediately revokes source permission and blocks imports and permission regrant. Recovery requires explicit deletion cancellation and a new source permission period; the new period starts empty and cannot revive deleted detail, reviewed links, outcomes or calibration. The authority stores no provider credential, calls no provider and changes no customer, job, estimate, project, schedule, price, invoice, payment, collection, accounting record or policy. The rendered Learning Center and complete Part 12 acceptance remain mandatory Slice K; native financial authority remains Mission 27.

Idempotent replay is current-authority specific. Exact retries return the saved result only while it remains the current permission, adapter, retention, deletion, hold or cleanup revision. Adapter, retention and hold history pins the exact source-permission revision and state. Revocation, deletion, cancellation, a new permission period or any later permission, lifecycle or cleanup revision retires the earlier response, which is then rejected without exposing its successful payload.

## Part 12 Slice K candidate — business-system Learning Center and Part 12 acceptance

Migration `123_canonical_learning_center_business_systems.sql` extends the bounded tenant-private Learning Center inventory to the accepted CRM/field-service, project/change-order, communication and external-financial sources. The owner and administrator page presents source permission, import progress, reviewed reference links, separate outcome and calibration permissions, learned and unavailable measures, retention, legal/audit holds, deletion and bounded cleanup in plain business language. Customer outcome learning requires one exact CRM and communication pair; the page fails closed when its partner source is missing or ambiguous.

Paid and isolated-demo experiences preserve the same responsive, keyboard-accessible and recovery behavior. The demo contains isolated read-only records for all four source classes. The surface renders safe company labels and plain recovery messages while withholding opaque target values, source references, digests, request identities and backend codes. It remains advisory and cannot apply a suggestion or change an operating or financial record. Physical devices, physical Safari, manual assistive-technology review, real providers, private production and the founder's visual verdict remain separate unavailable evidence.
