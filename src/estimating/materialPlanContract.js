...
const V3='estimate-material-plan-v3';
const source=require('./materialSourceContract');
...
function calculate(inputs,currency,version=VERSION){
 ...
 if(![V2,V3].includes(version)||!object(inputs,version===V3?['lines','sourceAssessment']:['lines'])||!Array.isArray(inputs.lines)||inputs.lines.length<1||inputs.lines.length>20)fail('Add between 1 and 20 materials to this plan.');
 const ids=new Set();let total=0n;
 const lines=inputs.lines.map((line,index)=>{
  if(!object(line,version===V3?['lineId',...KEYS,'evidence']:['lineId',...KEYS])||typeof line.lineId!=='string'||!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(line.lineId)||ids.has(line.lineId.toLowerCase()))fail('Review the separate material entries before continuing.');
  ids.add(line.lineId.toLowerCase());if(version===V3)source.validate(line,currency);
 ...
 return stableValue({contract:version,...(version===V3?{sourceAssessment:inputs.sourceAssessment}:{}),lines,lineCount:lines.length,total:decimal(total,2),currency,rounding:'The plan total adds each material cost after rounding to cents.'});
}
