...
async function grantAndVerify(client, runtimeRole) {
  ...
  const tables = (await client.query("SELECT oid,relname FROM pg_class WHERE relnamespace='public'::regnamespace AND relkind='r' AND relname=ANY($1)", [[
    'canonical_external_travel_import_consents', 'canonical_external_travel_import_runs', 'canonical_external_travel_import_records']])).rows;
  ...
  if (tables.length !== 3 || functions.filter(fn => ENTRIES.has(fn.proname)).length !== ENTRIES.size) throw new Error('Travel import authority unavailable');
}
