...
(async () => { let f;
  try {
    ...
    change = () => f.ownerPool.query("UPDATE auth_sessions SET access_expires_at=clock_timestamp()-interval '1 second' WHERE id=$1", [f.actors.owner.authSessionId]);
    ...
    assert.equal(revoked.body.data, undefined); result.cases.push('actual durable session expiry during provider wait suppresses generated output');
    result.pass = true;
