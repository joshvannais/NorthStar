'use strict';
const ENTRIES = new Set(['canonical_external_travel_import_consent_read', 'canonical_external_travel_import_consent_mutate', 'canonical_external_travel_import_batch', 'canonical_external_travel_import_read', 'canonical_external_travel_reference_matches_read', 'canonical_external_travel_reference_match_mutate', 'canonical_imported_travel_learning_consent_read', 'canonical_imported_travel_learning_consent_mutate', 'canonical_imported_travel_outcome_observe', 'canonical_imported_travel_outcome_read', 'canonical_imported_travel_calibration_consent_read', 'canonical_imported_travel_calibration_consent_mutate', 'canonical_imported_travel_calibration_propose', 'canonical_imported_travel_calibration_read', 'canonical_external_travel_adapter_mutate', 'canonical_external_travel_retention_mutate', 'canonical_external_travel_deletion_mutate', 'canonical_external_travel_operations_read', 'canonical_external_travel_cleanup_execute']);
async function grantAndVerify(client, runtimeRole) {
  ...
  const tables = (await client.query("SELECT oid,relname FROM pg_class WHERE relnamespace='public'::regnamespace AND relkind='r' AND relname=ANY($1)", [[
    'canonical_external_travel_import_consents', 'canonical_external_travel_import_runs', 'canonical_external_travel_import_records', 'canonical_external_travel_reference_matches', 'canonical_external_travel_import_learning_consents', 'canonical_external_travel_outcome_observations', 'canonical_external_travel_calibration_consents', 'canonical_external_travel_calibration_proposals', 'canonical_external_travel_adapter_revisions', 'canonical_external_travel_retention_revisions', 'canonical_external_travel_deletion_revisions', 'canonical_external_travel_cleanup_runs']])).rows;
  ...
  if (tables.length !== 12 || functions.filter(fn => ENTRIES.has(fn.proname)).length !== ENTRIES.size) throw new Error('Travel import authority unavailable');
}
