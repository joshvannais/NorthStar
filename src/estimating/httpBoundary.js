...
function estimateDecisionBodyBoundary(req,res,next){
 if(req.method!=='POST'||!/^\/api\/(?:v1\/canonical|demo\/command-center)\/estimates\/[^/]+\/(?:decisions|labor-plans|labor-plan-preview|material-plans|material-plan-preview|material-adoptions|material-adoption-preview)\/?$/.test(rawRequestPath(req)))return next();
