...
window.CustomerDetail = (function() {
  ...
  function open(customerId, options) {
    ...
    _decisionDraft = null; _laborPlanDraft = null; _equipmentPlanDraft = null; _materialPlanDraft = null; _adoptionDraft = null; _selectedEstimateRevision = null; _estimateReview = null;
  ...
  function renderEquipmentPlan(review,parent){
    ...
    function showKnowledge(value,target){if(typeof value==='string')para(value,target);else if(Array.isArray(value))value.forEach(function(v){showKnowledge(v,target);});else if(value&&typeof value==='object')Object.keys(value).forEach(function(k){showKnowledge(value[k],target);});}
    ...
    function rerender(focusId){var open=root.open;root.remove();renderEquipmentPlan(review,parent);var mounted=document.getElementById('cdEquipmentPlan');if(mounted)mounted.open=open||!!_equipmentPlanDraft;var focus=focusId&&document.getElementById(focusId);if(focus)focus.focus();}
    ...
    if(draft.action==='save')draft.inputs.lines.forEach(function(line,i){var box=el('fieldset',null,form);box.style.cssText='min-width:0;margin:1rem 0;padding:0.75rem;border:1px solid var(--border-color,#9ca3af);border-radius:0.5rem;';el('legend','Equipment '+(i+1),box);
      ...
      var identity=el('details',null,box);el('summary','Identity And Configuration',identity);identityKeys.forEach(function(k,n){var control=field(identity,identityLabels[n]+' (Blank If Unknown)','cdEquipment-'+k+'-'+i,line.identity[k],function(v){line.identity[k]=v||null;line.requirements.forEach(function(q){q.specificationIndex=null;});});control.readOnly=!!line.assetId;});
      ...
      var requirements=el('details',null,box);el('summary','Job Requirements',requirements);para('Choose the requirement source and an exact specification. Units must match; unrecorded information stays unresolved.',requirements);
  ...
  function renderRevisionSelector(review,parent) {
    ...
    select.onchange=function(){_selectedEstimateRevision=Number(select.value);_laborPlanDraft=null;_equipmentPlanDraft=null;_decisionDraft=null;_materialPlanDraft=null;_adoptionDraft=null;refreshEstimateReview('revision-selected');};label.appendChild(select);parent.appendChild(label);
  ...
  function renderMaterialAdoption(review,parent) {
    ...
    var available={material:review.materialPlans&&review.materialPlans.current,labor:review.laborPlans&&review.laborPlans.current};
    ...
    var basis=JSON.stringify([review.pins,review.costComponents,review.decisions.writeBasis,available.material&&available.material.digest,available.labor&&available.labor.digest]);
    ...
    if(!_adoptionDraft){['material','labor'].forEach(function(kind){var saved=available[kind],title=kind==='material'?'Material':'Labor';if(!saved||saved.action!=='save')return;if(review.costComponents&&review.costComponents[kind]&&review.costComponents[kind].id===saved.id)return;if(JSON.stringify(saved.sourcePins)!==JSON.stringify(review.pins)){para('Review and save the '+kind+' plan for this current estimate before applying it.');return;}button('Use '+title+' Plan In Estimate',function(){_adoptionDraft={component:kind,basis:basis,reason:'',confirmed:false,result:null,request:null};redraw();$('cdAdoptionReason').focus();},kind==='material'?'cdAdoptionStart':'cdLaborAdoptionStart');});return;}
    ...
    var label=document.createElement('label');label.textContent='Reason For Using This '+(component==='material'?'Material':'Labor')+' Plan';label.style.display='block';
    ...
    function send(preview){
      ...
      var body={sourcePins:review.pins,expectedPlanId:plan.id,expectedPlanRevision:plan.revision,expectedPlanDigest:plan.digest,expectedDecisionRevision:review.decisions.writeBasis.revision,expectedDecisionDigest:review.decisions.writeBasis.digest,reason:draft.reason.trim(),confirmed:preview?false:draft.confirmed,confirmationVersion:'estimate-cost-adoption-v1',changedComponent:component,expectedComponents:review.costComponents,assessment:draft.assessment||{}};
      ...
      window.NorthStarAccountSession.fetch('/api/v1/canonical/estimates/'+encodeURIComponent(review.pins.estimateId)+(preview?'/cost-adoption-preview':'/cost-adoptions'),{method:'POST',headers:headers,body:JSON.stringify(attempt.body)}).then(function(response){return response.json().catch(function(){return {};}).then(function(data){if(!response.ok)throw {status:response.status,category:data.error&&data.error.category};return data;});}).then(function(data){
        ...
        else{_adoptionDraft=null;_decisionDraft=null;_materialPlanDraft=null;_laborPlanDraft=null;_equipmentPlanDraft=null;_selectedEstimateRevision=null;refreshEstimateReview('adoption-saved');}
    ...
    formButton('Preview Estimate Costs',function(){send(true);});formButton('Save New Estimate',function(){send(false);});formButton('Cancel Estimate Change',function(){_adoptionDraft=null;redraw();var start=$(component==='material'?'cdAdoptionStart':'cdLaborAdoptionStart');if(start)start.focus();});form.onsubmit=function(e){e.preventDefault();};
  }
