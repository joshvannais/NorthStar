...
BEGIN
 ...
 IF replay_value IS NOT NULL THEN IF rtrim(replay_value->>'request_digest')<>request_hash THEN RAISE EXCEPTION 'Source operation key conflict' USING ERRCODE='23505'; END IF; RETURN jsonb_build_object(kind,public.canonical_external_business_operation_projection(kind,replay_value),'replayed',TRUE); END IF;
 ...
 digest_value:=public.canonical_completion_digest(jsonb_build_object('organizationId',org,'sourceClass',class_value,'sourceKey',source_value,'kind',kind,'revision',next_revision,'previousId',previous,'actorUserId',actor,'body',body,'requestDigest',request_hash));
 IF kind='adapter' THEN
  ...
  INSERT INTO public.canonical_external_business_adapter_revisions(organization_id,source_class,source_key,revision,previous_id,action,adapter_kind,cadence,actor_user_id,membership_id,auth_session_id,request_key_hash,request_digest,canonical_digest)
  VALUES(org,class_value,source_value,next_revision,previous,body->>'action',body->>'adapterKind',body->>'cadence',actor,(authority->>'membershipId')::uuid,session_value,key_hash,request_hash,digest_value) RETURNING to_jsonb(canonical_external_business_adapter_revisions.*) INTO inserted;
 ELSIF kind='retention' THEN
  ...
  INSERT INTO public.canonical_external_business_retention_revisions(organization_id,source_class,source_key,revision,previous_id,action,retention_days,actor_user_id,membership_id,auth_session_id,request_key_hash,request_digest,canonical_digest)
  VALUES(org,class_value,source_value,next_revision,previous,body->>'action',CASE WHEN body->>'action'='set' THEN (body->>'retentionDays')::integer END,actor,(authority->>'membershipId')::uuid,session_value,key_hash,request_hash,digest_value) RETURNING to_jsonb(canonical_external_business_retention_revisions.*) INTO inserted;
 ELSE
...
RETURNS JSONB LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public,pg_temp AS $$
DECLARE authority JSONB; current_row public.canonical_external_business_hold_revisions%ROWTYPE; replay_row public.canonical_external_business_hold_revisions%ROWTYPE; inserted public.canonical_external_business_hold_revisions%ROWTYPE; key_hash TEXT; request_hash TEXT; digest_value TEXT; next_revision BIGINT;
BEGIN
 IF current_setting('transaction_isolation')<>'serializable' THEN RAISE EXCEPTION 'Serializable required' USING ERRCODE='25001'; END IF;
 ...
 IF FOUND THEN IF rtrim(replay_row.request_digest)<>request_hash THEN RAISE EXCEPTION 'Source hold key conflict' USING ERRCODE='23505'; END IF; RETURN jsonb_build_object('hold',public.canonical_external_business_operation_projection('hold',to_jsonb(replay_row)),'replayed',TRUE); END IF;
 ...
 next_revision:=COALESCE(current_row.revision,0)+1; digest_value:=public.canonical_completion_digest(jsonb_build_object('organizationId',org,'sourceClass',class_value,'sourceKey',source_value,'revision',next_revision,'previousId',current_row.id,'action',body->>'action','holdKind',body->>'holdKind','actorUserId',actor,'requestDigest',request_hash));
 INSERT INTO public.canonical_external_business_hold_revisions(organization_id,source_class,source_key,revision,previous_id,action,hold_kind,actor_user_id,membership_id,auth_session_id,request_key_hash,request_digest,canonical_digest)
 VALUES(org,class_value,source_value,next_revision,current_row.id,body->>'action',body->>'holdKind',actor,(authority->>'membershipId')::uuid,session_value,key_hash,request_hash,digest_value) RETURNING * INTO inserted;
...
BEGIN
 ...
 IF FOUND THEN IF rtrim(replay_run.request_digest)<>request_hash THEN RAISE EXCEPTION 'Cleanup key conflict' USING ERRCODE='23505'; END IF; RETURN jsonb_build_object('run',public.canonical_external_business_cleanup_projection(replay_run),'replayed',TRUE); END IF;
