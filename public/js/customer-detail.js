        ...
        if(_groundedReview){
          ...
          if(!target||target.estimateId!==selected.ids.estimate||target.selectedRevision!==review.selectedRevision||JSON.stringify(stable(target.handoffBasis))!==JSON.stringify(stable(currentHandoffBasis))||JSON.stringify(stable(target.sourcePins))!==JSON.stringify(stable(review.pins))){paragraph('The estimate changed since this conversation. Review its current details before making changes.');}
            ...
            if(proposal&&proposal.change){
              ...
              if(proposal.editor!=='labor'||!review.laborPlans||!review.laborPlans.canMutate||!saved||saved.digest!==proposal.planDigest||!line||line.lineId!==change.lineId||line.basis!=='worker_hours'||change.field!=='workerHours'||change.source!=='explicit_user_assumption'||line.workerHours!==change.previous||! /^(0|[1-9][0-9]{0,8})(\.[0-9]{1,6})?$/.test(change.value)){paragraph('The proposed task changed. Review the current labor plan before entering a new assumption.');return;}
