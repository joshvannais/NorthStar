...
CSV files use the exact `m25-external-labor-time-v1` fields and contain one to 100 records per page. The owner marks a page as final or supplies its next checkpoint; the next file resumes from the saved checkpoint. Parsing rejects unknown headers, malformed quoting, duplicate record identities, invalid timestamps and files over 256 KB before the guarded import entry runs. Historical and continuous cursors remain separate and resumable.

The provider-neutral lifecycle records only adapter type, cadence and connect, pause, resume or disconnect state. Its contract has no field for a provider account identifier, credential, access token or password. A provider-specific connector still requires its own authorized implementation and release evidence; registering a lifecycle does not claim that a vendor connection exists.

Retention and deletion use revision and digest pins, idempotency keys, serializable transactions and a bounded cleanup limit of one to 100 current records. Every resumable checkpoint is bound to the exact policy or deletion-request revision and digest that created it; changing that authority starts a new cursor chain at the beginning. Cleanup appends detail-free tombstone revisions and records an immutable run checkpoint. Corrections and tombstones make dependent reviewed matches, observations and calibration proposals stale. A deletion request also revokes current source consent immediately, so new imports and derived reads stop before cleanup completes.
