...
BEGIN
 ...
 WITH imported AS (
  ...
  FROM public.canonical_external_labor_cleanup_runs
...
BEGIN
 ...
 SELECT * INTO previous_run FROM public.canonical_external_labor_cleanup_runs WHERE organization_id=org AND source_key=source_value AND operation=operation_value
 ...
 SELECT COALESCE(max(sequence),0)+1 INTO sequence_value FROM public.canonical_external_labor_cleanup_runs WHERE organization_id=org AND source_key=source_value AND operation=operation_value;
