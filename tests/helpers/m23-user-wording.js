  ...
  browser.newContext = async function(options) {
    ...
    await context.addInitScript(() => {
      ...
      const check = () => {
        ...
        const forbidden = /\b(?:PostgreSQL|SHA-?256|digest|idempotency|canonical|tenant|runtime|payload|schema|endpoint|SQL|API|HTTP|server-owned|server snapshot|execution revision|assignment revision|source pins?|immutable history|fieldEvidence)\b|\/api\/|\b[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\b|\b[0-9a-f]{64}\b/i;
