(function (global) {
  ...
  function transport(input, options) {
    ...
    var preparedAdoption=/^\/api\/v1\/canonical\/estimates\/([a-f0-9-]+)\/(proposal-adoptions|proposal-adoption-preview)$/.exec(url.pathname);
    if(preparedAdoption&&['GET','POST'].indexOf(method)>=0){return loadWorkspace().then(function(){var h=new Headers(options&&options.headers||{});if(method==='POST'){h.set('X-NorthStar-Demo-Intent','proposal-adoption');if(!h.has('X-NorthStar-Demo-Revision'))h.set('X-NorthStar-Demo-Revision',String(workspace.integrity.revision));}return nativeFetch('/api/demo/command-center/estimates/'+preparedAdoption[1]+'/'+preparedAdoption[2],Object.assign({},options||{},{headers:h,credentials:'same-origin'})).then(function(response){if(method==='POST'&&preparedAdoption[2]==='proposal-adoptions'&&response.ok){return loadWorkspace(true).then(function(){return response;});}return response;});});}
  ...
  function installToolbar(value) {
    ...
    function resolve(previous){resolved=resolver&&resolver(space,choices,Math.random,previous);persist();simulate.disabled=!resolved||value.session.simulationCount>=12;var problem='Choose compatible lead details or use Random.';
      var definition=space.dimensions.find(function(d){return d.id==='service';}).options.find(function(o){return o.id===choices.service;});
      if(choices.service!=='random'&&choices.intent!=='random'&&!global.NorthStarCommandCenterContract.demoJobType(choices.service,choices.intent,null,definition))problem='That service does not support this caller intent in the demo. Choose another intent.';
      else if(choices.scheduling==='weather_window'&&choices.service!=='random'&&!(definition&&definition.weatherSensitive)&&['fence','roofing','concrete'].indexOf(choices.service)<0)problem='Weather-dependent timing is available for outdoor services. Choose another timing option.';
    ...
    panel.append(control('strong','Customize Lead'),grid,help);if(space.industryCoverage)panel.append(control('p',space.industryCoverage.note,'northstar-demo-scenario-help'));builder.append(panel);builder.addEventListener('keydown',function(e){if(e.key==='Escape'){builder.open=false;summary.focus();e.preventDefault();}});
