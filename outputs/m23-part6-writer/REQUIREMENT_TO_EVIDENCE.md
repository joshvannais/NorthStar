...
| Secure tenant file authority | Database-reserved opaque object and immutable generation identities, pre-storage idempotency validation, JPEG/PNG/WebP extension/MIME/magic agreement, declared SHA-256, 10 MiB per file, 25 files and 100 MiB per execution, streaming, compression rejection and a 40-megapixel decompression-safety ceiling | Unit streaming, digest/magic/polyglot/decompression/size gates; mounted reservation conflict/replay/concurrency, count and byte constraints |
| Quarantine, scanning, encryption and cleanup | Capability gate requires durable encrypted immutable-create quarantine, malware scanning, metadata removal, retention/orphan cleanup, generation-scoped deletion and short-lived retrieval; DB registration follows provider release | Unit call-order, generation-bound scan receipt, accepted-replay short circuit, generation cleanup and scanner-unavailable tests |
| File accessibility state | Every file requires bounded inert `described`, `unavailable`, or `needs_review` metadata; append-only correction preserves storage identity and retrieval discloses the current state | Header/hostile-state unit cases; mounted digest/idempotency, correction, audit, revision and retrieval cases |
...
| Authorized retrieval and access logging | Current authority and retention rechecked; immutable database-owned access event binds object generation/version, content digest and current accessibility; HTTPS attachment-only octet-stream URL bounded to five minutes | Mounted authorized retrieval/access-log divergence checks; unit provider-response validation |
