(function () {
  ...
  async function json(path,options){var r=await (demo?window.NorthStarDemoRuntime.fetch:window.fetch)(path,Object.assign({credentials:'same-origin',cache:'no-store'},options||{})),body=await r.json();if(!r.ok)throw Object.assign(new Error((demo&&r.status===429&&body.error&&['session','saved_work'].includes(body.error.limitKind)?'This demo has reached its '+(body.error.limitKind==='saved_work'?'saved-work':'action')+' limit. Your saved history remains available. To continue with a new demo, deliberately choose Reset Demo; this clears this demo’s saved changes.':null)||({400:'Check the required fields and selected records before continuing.',401:'Sign in again before continuing.',403:'Your current access does not allow this update.',404:'This work is no longer available. Refresh the job list.',409:'The work or supporting information changed, or a required step is incomplete. Refresh and review the current work before trying again.',410:'This review has expired. Refresh and review the current work.',429:'Too many requests. Wait briefly, then try again.',503:'Work updates are temporarily unavailable. Refresh to check the saved work.'})[r.status]||'Work details are unavailable. Refresh before continuing.'),{status:r.status});return body;}
  function close(){formHost.replaceChildren();if(returnFocus&&returnFocus.isConnected)returnFocus.focus();}
  async function reloadSelection(){if(pending)return;var generation=++serial,previous=select.value||new URLSearchParams(location.search).get('appointmentId');close();model=null;content.replaceChildren();status.textContent='Loading Job List…';
    try{var response=await json('/api/v1/field-executions/owner-work');if(generation!==serial)return;var value=response.data;if(!value||value.authority!==(demo?'isolated_demo_postgresql':'postgresql')||!Array.isArray(value.records))throw new Error('Invalid job list');select.replaceChildren();value.records.forEach(function(r){var o=node('option',r.title);o.value=r.appointmentId;select.append(o);});if(previous&&value.records.some(function(r){return r.appointmentId===previous;}))select.value=previous;await load();}
    catch(e){if(generation===serial)status.textContent=e.status?e.message:'The job list could not be loaded. Refresh and try again.';}}
  async function load(){if(pending)return;var generation=++serial;close();model=null;content.replaceChildren();if(!select.value){status.textContent='No Eligible Jobs Are Available. Schedule Existing Work Before Opening Its Work Details.';return;}status.textContent='Loading Current Work…';
  ...
  root.querySelector('[data-work-refresh]').addEventListener('click',reloadSelection);select.addEventListener('change',load);
