...
function mutationInput(input) {
  if (!input || typeof input !== 'object' || !['simulate_lead', 'reset', 'estimate_review','pricing_plan','travel_plan','equipment_ready','equipment_cost','equipment_plan','labor_plan','material_plan','estimate_adopt','schedule_preview','schedule_approve','work_action'].includes(input.operation)) {
...
class DemoCommandCenterRepository {
  ...
  async mutate(token, rawInput, rawAdmission) {
    ...
    try {
      ...
      if(sourceOperation || operations || ['labor_plan','equipment_plan','equipment_cost'].includes(input.operation)||input.operation==='estimate_adopt'&&['estimate-cost-adoption-v2','estimate-cost-adoption-v3'].includes(input.adoption.confirmationVersion))now=date((await client.query('SELECT clock_timestamp() now')).rows[0].now);else if(scheduling)now=date(this.clock());
      ...
      if (replay.rows.length) {
        ...
        if(input.operation==='equipment_ready'&&(current.state.equipmentReadinessPlans?.[input.estimateId]||[]).some(e=>e.requestKey===input.idempotencyHash)){const moment=date((await client.query('SELECT clock_timestamp() now')).rows[0].now);if(date(lockedRow.expires_at)<=moment)fail(410,'DEMO_SESSION_EXPIRED','This demo session expired. Refresh to start again.');await client.query('COMMIT');open=false;return {record:current,replayed:true};}
        if(input.operation==='equipment_cost'&&(current.state.equipmentCostPlans?.[input.estimateId]||[]).some(e=>e.requestKey===input.idempotencyHash)){const moment=date((await client.query('SELECT clock_timestamp() now')).rows[0].now);if(date(lockedRow.expires_at)<=moment)fail(410,'DEMO_SESSION_EXPIRED','This demo session expired. Refresh to start again.');await client.query('COMMIT');open=false;return {record:current,replayed:true};}
        if(input.operation==='travel_plan'&&(current.state.travelPlans?.[input.estimateId]||[]).some(e=>e.requestKey===input.idempotencyHash)){const moment=date((await client.query('SELECT clock_timestamp() now')).rows[0].now);if(date(lockedRow.expires_at)<=moment)fail(410,'DEMO_SESSION_EXPIRED','This demo session expired. Refresh to start again.');await client.query('COMMIT');open=false;return {record:current,replayed:true};}
        if(input.operation==='pricing_plan'&&(current.state.pricingPlans?.[input.estimateId]||[]).some(e=>e.requestKey===input.idempotencyHash)){const moment=date((await client.query('SELECT clock_timestamp() now')).rows[0].now);if(date(lockedRow.expires_at)<=moment)fail(410,'DEMO_SESSION_EXPIRED','This demo session expired. Refresh to start again.');await client.query('COMMIT');open=false;return {record:current,replayed:true};}
