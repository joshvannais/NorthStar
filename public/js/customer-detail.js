...
window.CustomerDetail = (function() {
  ...
  function materialLines(plan) {
    ...
    return ['estimate-material-plan-v2','estimate-material-plan-v3'].indexOf(plan.calculationVersion)>=0 ? plan.inputs.lines : [plan.inputs];
  }
  function materialResult(result, target) {
    ...
    if (result.lines) {
      ...
      text('Plan Total',decisionMoney(result.total,result.currency));if(result.sourceAssessment)materialSourceAssessment(result.sourceAssessment,target);return;
    }
  ...
  function renderMaterialPlan(review,parent) {
    ...
    if(!plans||plans.contract!=='estimate-material-plan-v3'||JSON.stringify(plans.sourcePins)!==JSON.stringify(review.pins)||plans.simulated!==review.simulated){para('Material planning is unavailable. Refresh this estimate.');return;}
    ...
    function start(action){var lines=current&&current.inputs?materialLines(current).map(function(line){return Object.assign({},JSON.parse(JSON.stringify(line)),{lineId:line.lineId||crypto.randomUUID()});}):[Object.assign(emptyLine(),{material:review.materialReview.material||''})];_materialPlanDraft={action:action,estimateId:review.pins.estimateId,basis:materialPlanBasis(review),inputs:{lines:lines,sourceAssessment:null},reason:'',confirmed:false,result:null,request:null};render();focusField();}
