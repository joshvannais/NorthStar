...
- Frozen first-correction migration source commit: `d6fc5fa5aaa66906e40413e912b0881a7e50f2c4`
...
| Current actor/performer/crew authority | HTTP derives tenant/account/role/session/CSRF. Before the transaction snapshot, the repository acquires a shared ordered supporting-authority session lock; both SQL entries verify that the lock is already held and deny direct callers that bypass the order. Statement-level exclusive triggers cover transcript, appointment, subscription, onboarding, account, membership, session, workforce, crew, assignment, and execution writers. Migration 044 makes every one of those trigger paths advance a singleton MVCC fence. Material entries reject same-backend exclusive-lock reentrancy, row-lock the fence, and therefore fail with serialization denial when a repeatable-read/serializable snapshot predates a committed authority write. PostgreSQL then reloads every authority under the attested current snapshot. Members act only as themselves; owner/admin on-behalf facts remain attributable. | Authentic two-connection PostgreSQL stale-snapshot mutation/replay/read and all 11 writer-trigger probes, the exact runtime `REPEATABLE READ READ ONLY` disclosure reproduction, direct-entry/no-lock/reentrancy denial, zero-effects, contention, timeout, and clean retry pass |
...
- Tests: 5,361 passed / 5,361 total
- Snapshots: 0
- Failures: 0
