...
function rawSources(state,item,inputs=null,now=new Date(),historicalPlan=null){
 const plan=historicalPlan||state.equipmentPlans?.[item.ids.estimate]?.[0],basis=plan?.action==='save'?{planId:plan.id,revision:plan.revision,digest:plan.digest}:null;
