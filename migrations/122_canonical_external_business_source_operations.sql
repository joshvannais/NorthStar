...
BEGIN
 ...
 IF kind='adapter' THEN EXECUTE format('SELECT to_jsonb(x) FROM (SELECT * FROM public.%I_import_consents WHERE organization_id=$1 AND source_key=$2 ORDER BY revision DESC LIMIT 1 FOR UPDATE)x',prefix_value) INTO consent_row USING org,source_value; END IF;
 ...
 digest_value:=public.canonical_completion_digest(jsonb_build_object('organizationId',org,'sourceClass',class_value,'sourceKey',source_value,'kind',kind,'revision',next_revision,'previousId',previous,'sourceConsentId',CASE WHEN kind='adapter' THEN consent_row->>'id' END,'sourceConsentRevision',CASE WHEN kind='adapter' THEN (consent_row->>'revision')::bigint END,'sourceConsentDigest',CASE WHEN kind='adapter' THEN rtrim(consent_row->>'canonical_digest') END,'actorUserId',actor,'body',body,'requestDigest',request_hash));
