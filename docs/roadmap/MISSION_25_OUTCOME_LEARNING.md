...
Idempotent replay is current-authority specific. Exact retries return the saved result only while it remains the current permission, adapter, retention, deletion, hold or cleanup revision. Revocation, deletion, cancellation, a new permission period or a later lifecycle/cleanup revision retires the earlier response, which is then rejected without exposing its successful payload.
