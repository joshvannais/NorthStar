...
| External travel evidence | `canonical_external_travel_import_consents`, `canonical_external_travel_import_runs`, `canonical_external_travel_import_records`, `canonical_external_travel_reference_matches` and guarded external-travel routes | Route duration, mileage, fuel quantity and fuel cost estimate-versus-actual outcomes | Part 9 Slice A stages normalized, consented evidence with explicit units, currency, time zone and evidence class. Slice B explicitly reconciles opaque job and vehicle references to same-tenant estimates and current reviewed vehicle assets without guessing. Missing dimensions remain unavailable. Observations, calibration, source operations and owner UI remain required before Part 9 release. |
...
## Part 9 Slice B candidate — reviewed travel reconciliation
