...
The mounted PostgreSQL 17 proof exercises the existing provider-neutral labor import authority at its exact one through 100 records per request boundary. A full 100-record page completes with headroom under the existing five-second statement limit, while 101 records fail before persistence. Concurrent identical requests create once, reject the overlapping serializable attempt and return the winner's exact replay on retry; concurrent different requests sharing a request key keep one winner and reject the conflict. A held source lock reaches the existing two-second lock limit, returns a plain temporary-unavailable response with no partial record or request claim, and the exact request succeeds after pressure clears.

Bounded request telemetry reports only a correlation identifier, method class, status and duration. It contains no company, source, cursor, imported record, request-key or request-body content. Full-row digests prove that the exercise changes no customer or estimate plan, schedule, job execution, asset, native evidence, company, financial, provider or isolated-demo state. Accepted external learning history remains the only intended state change. Migrations 089, 134 and 135 remain byte-identical. No rendered path changes in Slice D; mission-wide accessibility and responsive review remain Slice E.
