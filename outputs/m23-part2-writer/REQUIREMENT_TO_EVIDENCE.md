...
| PostgreSQL/time compatibility | Disposable PostgreSQL 18.4 reports UTF8, `TimeZone=UTC`, `lc_collate=C`, and data checksums on. The dated production receipts report PostgreSQL 18.6, UTC, UTF8, and compatible pre-application collation/ctype plus the exact post-application row and unchanged later-start identity. | Pass for pre-application, first application, and later-start zero-op compatibility |
...
| Recovery truth | No dated backup receipt or isolated restore rehearsal exists. The same receipt records the founder-authorized conservative disposition: keep/restore healthy app with 038 source retained, leave additive schema inert, correct schema only by a new reviewed forward-fix, no destructive rollback, and root coordinator responsible. | Authorized alternative disposition recorded; backup/restore remains unavailable |
| No premature release claim | Root status distinguishes independently accepted, merged, deployed, first-application, later-start zero-op, and health facts. It does not turn unavailable backup/restore evidence into a pass and does not call a later writer candidate released. | Pass |
...
| Later-start migration zero-op | PR #162 merged normally at `2026-09-04T09:47:56Z` as `e8c30f96d9c0bc0c4287c1f181a400e3cedd4748`; GitHub deployment `6261881255` and Railway deployment `8ea1badb-3a7f-49bd-a0f8-0fa0a94865df` ran that exact commit. The later container start at `2026-09-04T09:48:41.774970382Z` logged no migration application. Read-only receipt `2026-09-04T09:49:39.687Z` retained 36 rows, exactly one 038, the exact checksum and original `applied_at`, with database/canonical health healthy. | Achieved; Part 3 progression gate passed |
| Receipt-follow-up scope | `PRODUCTION_APPLICATION_RECEIPT.md`, this ledger, the unavailable ledger, roadmap status, migration-identity cross-reference, and one ratification test only. PR #162 independently passed, merged, and deployed without adding runtime, migration, route, provider, credential, configuration, or production mutation. | Pass and released at `e8c30f96d9c0bc0c4287c1f181a400e3cedd4748` |
