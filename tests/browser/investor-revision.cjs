...
(async()=>{
  ...
  try {
    for(const setting of cases){
      ...
      check(await page.evaluate(()=>window.currentNorthStarInvestorResult.summary.openingLegalEarmark===15000 && window.currentNorthStarInvestorResult.summary.startingOperatingCash===10000),'new legal earmark15k and operating budget10k');
      ...
      receipts.push({case:setting.name,assertions,monthsOpened:120,errors,externalRequests:requests,screenshots:[setting.name+'-initial.png',setting.name+'-months.png',setting.name+'-month120-detail.png']});
