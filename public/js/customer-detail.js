...
window.CustomerDetail = (function() {
  ...
  function renderEquipmentCosts(review,parent){
    ...
    function showBasis(plan,target){
      var detail=el('details',null,target);el('summary','Recorded Cost Basis',detail);
    ...
    if(current&&current.action==='save'){show(current.result,root);showBasis(current,root);if(!current.sourceBasisCurrent)p('These costs were saved for an earlier estimate. Review them again before applying a new change.');}
    else p(current?'The equipment cost plan was withdrawn. Saved history remains available.':'No equipment costs have been saved.');
    if(review.adoptedEquipmentCostPlan)p('Included In This Estimate: '+decisionMoney(review.adoptedEquipmentCostPlan.result.total,review.currency)+'. Editing the plan does not change these included costs.');
    ...
    if(!equipment||equipment.action!=='save'){p('Save an Equipment Plan before entering its costs.');return;}
    if(equipment.currentSourcesChanged){p('Review the changed equipment sources before saving costs.');return;}
    var basis=JSON.stringify([review.pins,plans.decisionBasis,current&&current.digest,equipment.digest,review.equipmentOutsideBasis]);
    ...
    function start(action){var old=current&&current.action==='save'&&current.inputs;var lines=equipment.inputs.lines.map(function(l){var prior=old&&old.lines.find(function(c){return c.lineId===l.lineId;});return prior?Object.assign(JSON.parse(JSON.stringify(prior)),{access:l.accessBasis}):emptyLine(l);});_equipmentCostDraft={estimateId:review.pins.estimateId,basis:basis,action:action,inputs:{serviceKey:equipment.inputs.serviceKey,equipmentBasis:{planId:equipment.id,revision:equipment.revision,digest:equipment.digest,sourcePins:equipment.sourcePins},lines:lines,assessment:null},reason:'',explanation:'',confirmed:false,result:null,assessment:null,request:null};redraw(action==='save'?'cdCostMethod-0':'cdCostReason');}
    if(!_equipmentCostDraft){button(current&&current.action==='save'?'Revise Equipment Costs':'Plan Equipment Costs',function(){start('save');}).id='cdEquipmentCostStart';if(current&&current.action==='save')button('Withdraw Equipment Costs',function(){start('withdraw');});return;}
    var draft=_equipmentCostDraft;if(draft.estimateId!==review.pins.estimateId){_equipmentCostDraft=null;redraw();return;}
  ...
  function renderRevisionSelector(review,parent) {
    ...
    select.onchange=function(){_selectedEstimateRevision=Number(select.value);_laborPlanDraft=null;_equipmentPlanDraft=null;_equipmentCostDraft=null;_decisionDraft=null;_materialPlanDraft=null;_adoptionDraft=null;refreshEstimateReview('revision-selected');};label.appendChild(select);parent.appendChild(label);
  ...
  function renderMaterialAdoption(review,parent) {
    ...
    function send(preview){
      ...
      window.NorthStarAccountSession.fetch('/api/v1/canonical/estimates/'+encodeURIComponent(review.pins.estimateId)+(preview?'/cost-adoption-preview':'/cost-adoptions'),{method:'POST',headers:headers,body:JSON.stringify(attempt.body)}).then(function(response){return response.json().catch(function(){return {};}).then(function(data){if(!response.ok)throw {status:response.status,category:data.error&&data.error.category};return data;});}).then(function(data){
        ...
        else{_adoptionDraft=null;_decisionDraft=null;_materialPlanDraft=null;_laborPlanDraft=null;_equipmentPlanDraft=null;_equipmentCostDraft=null;_selectedEstimateRevision=null;refreshEstimateReview('adoption-saved');}
