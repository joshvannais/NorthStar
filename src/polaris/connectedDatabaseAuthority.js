'use strict';
const ENTRIES = new Set(['demo_polaris_provider_reserve', 'demo_polaris_provider_reconcile',
...
async function grantAndVerify(client, runtimeRole) {
  ...
  const tables = (await client.query("SELECT oid,relname FROM pg_class WHERE relnamespace='public'::regnamespace AND relkind='r' AND (relname LIKE 'demo_polaris_provider_%' OR relname LIKE 'canonical_tax_research_%')")).rows;
  ...
  const functions = (await client.query("SELECT oid,proname,oid::regprocedure::text identity FROM pg_proc WHERE pronamespace='public'::regnamespace AND (proname LIKE 'demo_polaris_provider_%' OR proname LIKE 'canonical_tax_research_%' OR proname='connected_reasoning_retire')")).rows;
