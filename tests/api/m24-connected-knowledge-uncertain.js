...
const transport=createRetellProjectionTransport({resolveBinding:async()=>binding,client:{llm:{retrieve:async()=>{gets++;return structuredClone(remote);},update:async(id,body,options)=>{assert.equal(options.maxRetries,0);assert.deepEqual(Object.keys(body).sort(),['general_prompt','version']);updates++;remote={...remote,general_prompt:body.general_prompt};if(updates===1)throw new Error('Synthetic lost update response after remote commit');return structuredClone(remote);}}}});
const worker=new SyncWorker({repository:sync,transports:{'intercepted.voice-provider':transport},batchSize:3});const uncertain=await worker.drainOnce();result.uncertain=uncertain;assert.equal(uncertain.succeeded,0);assert.equal(updates,1);const pending=(await f.ownerPool.query('SELECT status,last_known_good_event_id FROM canonical_knowledge_sync_states WHERE target_id=$1',[configured.target.id])).rows[0];assert.equal(pending.last_known_good_event_id,null);await new Promise(r=>setTimeout(r,15100));const first=await worker.drainOnce();result.first=first;assert.equal(first.succeeded,1);assert.equal(updates,1);assert.ok(remote.general_prompt.includes('Company connected'));assert.ok(!remote.general_prompt.includes('private-tax-connected'));assert.ok(!remote.general_prompt.includes('internalCost'));assert.ok(!remote.general_prompt.includes('canonicalPricing'));assert.ok(!remote.general_prompt.includes('private-connected@example.test'));
result.cases.push('actual uncertain committed update retained as pending, retry performs exact GET only without resubmission');
