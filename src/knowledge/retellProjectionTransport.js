...
function createRetellProjectionTransport({client,resolveBinding}){
 ...
 return {async applyProjection(request,{signal:upstreamSignal,authorityClient}={}){
  ...
  try {
  check();const binding=await resolveBinding(request,{client:authorityClient,signal});check();
  ...
  if(before.general_prompt!==expected){
   ...
   const current=await resolveBinding(request,{client:authorityClient,signal});check();checkBinding(current);if(canonicalStringify(current)!==canonicalStringify(binding))fail();
  ...
  const current=await resolveBinding(request,{client:authorityClient,signal});check();checkBinding(current);
