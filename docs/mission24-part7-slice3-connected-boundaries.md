...
Call guidance requires raw provider authentication plus current integration ownership, exact agent/call/session association, active subscription, current pinned business profile and permitted published caller knowledge. It remains provisional on the existing voice session, with no parallel estimate/job graph. A two-hour session bound, maximum thirty-two immutable guidance events and twenty-five-second request deadline apply; exact current replay can remain available at the event limit. Ordinary terminal ingestion creates the single canonical graph. Missing knowledge remains explicit. The production call binding is implemented but default-off. Its dedicated session ledger shares the existing037 paid monthly accounting through global then tenant then session locks. Admission allows one active request per session, two per organization, thirty-two admissions per session, ten per organization/day and one hundred globally/day; each reserves twenty million nano-USD. Unknown or expired outcomes retain that ceiling exactly once. The ceiling is a local assumption, not verified sufficient live model pricing. No transaction stays open during model transport.

## Existing Knowledge Outbox Exception

The existing `executeClaimWithAuthority` target/publication lock path is retained across knowledge transport/finalization, explicitly excepted from the model/tax no-network-in-transaction rule. Its borrowed internal client is passed to the current binding resolver without nested pool acquisition. Each SQL statement is capped by the remaining whole-operation ten-second deadline; actual statement cancellation releases transaction locks. The resolver cannot commit, release or query concurrently. The new transport uses one ten-second abort deadline for all initial GET/update/readback steps, inherits the worker abort signal, disables SDK automatic retries, and checks cancellation after waits. Exact lock cleanup and abort-aware behavior are fixture-tested; an already-sent provider update can still complete after a timeout. A Promise timeout is not proof of remote cancellation.
...
## Consolidated PR218 Correction And Binding Scope

Existing subscription policy is checked before generation, after generation and on replay, including limited roles and historical selected records. Production v2 additionally requires explicit POLARIS_GROUNDED_V2_ENABLED; v1 configuration remains unchanged. Expanded Recorded Figures distinguish Current Estimate from Earlier Estimate From Saved History without exposing numeric internal version labels.

Tax acquisition requires TAX_SOURCE_ACQUISITION_ENABLED and an exact reviewed public context/source pack. The shipped registry is empty. Supported packs record access review and expiry, source dates, coverage and exclusions; unavailable dates remain null. A whole multi-document eight-second transport deadline and bounded bytes apply within the claim lease. Shutdown aborts acquisition and leaves unfinished claims for durable lease recovery; it cannot finish a late result. These candidates do not have tax validation/publication privileges.

Knowledge activation additionally requires KNOWLEDGE_PROVIDER_SYNC_ENABLED and exact server-owned tenant/target/agent/LLM-version/base-prompt bindings. The native adapter uses documented GET get-retell-llm and PATCH update-retell-llm; only general_prompt is patched, redirects/retries are disabled and responses bounded. Binding declarations do not prove external writer exclusion or published agent use. Official protocol references: https://docs.retellai.com/api-references/get-retell-llm and https://docs.retellai.com/api-references/update-retell-llm .

Call rows retain live reservations. Up to200 expired reservations per retirement tick settle at the full ceiling; up to200 settled rows older than90days may be removed. Monthly accounting remains the existing037 thirteen-month policy. Both paid reserve and reconciliation routines are in037;038 is field execution and is not their accounting source. All71 applied SQL files remain immutable. New call table/functions and grants reside only in unreleased074.

Local fixtures prove accounting, cancellation, source checks and provider-shaped transport. They do not close either named operational checkpoint. Actual model pricing/output ceilings, reviewed source packs, exclusive external knowledge writer control, legal/provider consent and finite real canary authorization remain required before live activation.
