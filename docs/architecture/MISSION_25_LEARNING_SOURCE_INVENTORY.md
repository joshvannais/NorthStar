...
| External historical and continuous sources | `canonical_external_labor_import_consents`, `canonical_external_labor_import_runs`, `canonical_external_labor_import_records`, `canonical_external_labor_import_reference_matches` and guarded external-labor routes | Older and ongoing CRM, fleet, accounting, payroll, fuel, routing, procurement and communications outcomes | Provider-neutral normalized labor/time envelope and reviewed worker and job reference reconciliation released: explicit source consent, bounded historical/continuous batches, cursors, schema/version pins, dedupe, immutable corrections, tombstones, revocation and stale-aware same-tenant matches. Adapter/provider connection, imported outcome observations and other source classes remain unimplemented. |
