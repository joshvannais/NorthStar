...
(async()=>{let f;const ledger={checks:[]};try{
 ...
 await f.ownerPool.query("UPDATE auth_sessions SET status='revoked',revoked_at=NOW(),revoke_reason='synthetic review test' WHERE id=$1",[f.actors.owner.authSessionId]);result=await read();assert.equal(result.status,401);ledger.checks.push('revoked session denied');
 ledger.pass=true;
