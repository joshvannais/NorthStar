...
test.each(['inferred','unresolved','unknown','unsupported',null])('status %s stays qualified in evidence and To Confirm',status=>{const g=graph('hvac',[fact('laborHours',8,status)]),before=JSON.stringify(g),r=api().describeGraph(g);expect(r.evidence[0]).toMatch(/needs confirmation/i);expect(r.missing.join(' ')).toMatch(/Labor Time needs confirmation/);expect(JSON.stringify(g)).toBe(before);});
test('filtered technical risk preserves actual concern and emergency presence',()=>{const r=api().describeRisks(['Emergency reported. customerRiskFlag: exposed wiring needs review.']);expect(r.join(' ')).toContain('exposed wiring needs review');expect(r.join(' ')).toContain('Emergency reported');expect(r.join(' ')).not.toMatch(/customerRiskFlag|No specific risks/);});
test('nonempty unusable concern is not an empty risk assessment',()=>{expect(api().describeRisks(['{"internal":true}'])[0]).toMatch(/A concern is recorded/);expect(api().describeRisks([{unknown:true}])[0]).toMatch(/A concern is recorded/);expect(api().describeRisks([])[0]).toBe('No specific risks identified from available details.');});
test('a non-array concern fails conservatively instead of claiming no risks',()=>{expect(api().describeRisks({unknown:true})[0]).toMatch(/A concern is recorded/);});
test.each(['Demo record detail collected for customerRiskFlag.','{"internal":true}',null,''])('explicit emergency survives unpresentable evidence %s',text=>{const entry={emergency:true,text},before=JSON.stringify(entry);expect(api().describeRisks([entry])).toEqual(['Emergency reported. Review the job details before proceeding.']);expect(JSON.stringify(entry)).toBe(before);});
test.each([false,null,'true',undefined])('non-true emergency %s does not invent an emergency',emergency=>{expect(api().describeRisks([{emergency,text:'{"internal":true}'}])[0]).toBe('A concern is recorded. Review the job details before proceeding.');});
