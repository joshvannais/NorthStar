...
app.get('/api/demo/command-center/estimates/:estimateId/customer-estimate-preview', (req, res) => {
  ...
  const document=req.params.estimateId === draftId ? {...estimate,state:'draft'} : req.params.estimateId === boundaryId ? boundaryEstimate : estimate;
...
app.get(['/', '/boundary', '/not-ready'], (req, res) => {
  const id = req.path === '/boundary' ? boundaryId : req.path === '/not-ready' ? draftId : ordinaryId;
...
(async () => {
  ...
  try {
    ...
    }
    {
      ...
      const entry=page.getByRole('button',{name:'Estimate, draft'});
      assert.equal(await entry.isVisible(),true);
      assert.equal(await entry.isEnabled(),true);
      assert.equal(await page.getByRole('button', { name:'Finish Estimate' }).count(), 0);
      await entry.click();
      await page.getByRole('dialog',{name:'Draft Customer Estimate'}).waitFor();
      assert.equal(requests.length, before + 1);
      await page.keyboard.press('Escape');
      await page.getByRole('button',{name:'Review Before Issuing'}).click();
    ...
    for (const recovery of [
      ...
      const dialog = page.getByRole('dialog', { name:'Issued Customer Estimate' });
    ...
    assert.deepEqual(requests, [ordinaryId, ordinaryId, boundaryId, draftId].map(id => '/api/demo/command-center/estimates/' + id + '/customer-estimate-preview'));
