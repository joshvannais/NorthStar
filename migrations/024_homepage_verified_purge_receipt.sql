...
ALTER TABLE homepage_demo_purge_operations
  ADD COLUMN projection_started_at TIMESTAMPTZ;
...
ALTER TABLE homepage_demo_purge_operations
  ADD CONSTRAINT homepage_demo_purge_operations_projection_start_check
    CHECK (
      projection_started_at IS NULL OR
      (state = 'consumed' AND projection_permitted = TRUE AND
       verified_at IS NOT NULL AND projection_started_at >= verified_at)
    );

COMMENT ON COLUMN homepage_demo_purge_operations.projection_permitted IS
  'Monotonic deletion-time permission for one receipt-gated, browser-memory-only Polaris projection; cancellation may revoke it until projection start.';

COMMENT ON COLUMN homepage_demo_purge_operations.projection_started_at IS
  'Durable row-locked linearization marker set immediately before Polaris calculation; stores no call, transcript, receipt, or result content.';
