...
function calcPrice(svc,s){
  switch(svc.name){
    case"Tree removal":{let p=150+(60*s.height*s.trees);if(s.stump)p+=150*s.trees;if(s.nearHouse)p+=800;if(s.emergency)p+=500;return p;}
    case"Roof inspection/replacement":{if(s.material==="patch")return 750;let psf=s.material==="metal"?7.5:s.material==="architectural"?5.5:4.5;let p=s.sqft*psf;if(s.layers>1)p+=s.sqft*2;if(s.pitch==="steep")p*=1.3;return Math.round(p);}
    case"Emergency plumbing":{let lh=s.replace?4:s.emergency?1.5:1;let parts=s.replace?s.pipeType==="galvanized"?1200:600:s.emergency?250:80;let p=150+(lh*125)+parts;if(s.accessibility==="hard")p+=200;if(s.emergency)p=Math.round(p*1.25);return Math.round(p);}
    case"Electrical panel upgrade":{if(s.amperage===0)return s.emergency?250:180;let p=s.amperage===200?1800:1200;if(s.outdoor)p+=400;if(s.permit)p+=250;return p;}
    case"Landscape design":{let p=s.sqft*8;if(s.patio)p+=3000;if(s.plantings)p+=1500;if(s.hardscape)p+=2000;if(s.irrigation)p+=1200;return Math.round(p);}
    case"HVAC repair":{if(!s.repair){return Math.round(s.tonnage*(s.system==="heat-pump"?2800:2200)+1500);}if(s.system==="duct")return 1000;if(s.system==="furnace")return 700;return 380;}
    case"Gutter cleaning":{let p=s.linearFt*(s.stories>1?1.5:1);if(s.debris==="heavy")p*=1.5;return Math.round(p);}
    case"Pest control":{const sv={low:150,moderate:300,severe:600};let p=sv[s.severity]||300;if(s.treatment==="termite")p+=400;if(s.treatment==="rodent")p+=200;p+=s.sqft*0.05;return Math.round(p);}
    case"Concrete driveway":{let p=s.sqft*8;if(s.reinforced)p+=s.sqft*2.5;if(s.demolition)p+=1200;if(s.finish==="stamp")p*=1.35;return Math.round(p);}
    case"Fence installation":{const m={wood:25,vinyl:35,"chain-link":15,"split-rail":12};let p=s.linearFt*(m[s.material]||25)+s.linearFt*12;p+=s.gates*150;if(s.terrain==="slight-slope")p*=1.15;if(s.terrain==="moderate-slope")p*=1.3;return Math.round(p);}
    case"Window replacement":{const wp={double:500,triple:850};let pw=wp[s.pane]||500;if(s.story>1)pw*=1.2;if(s.count===1)pw=1800;return s.count*pw;}
    case"Carpet cleaning":{let p=s.sqft*0.35;if(s.stains)p+=100;if(s.stairs)p+=75;return Math.round(p);}
    case"Pressure washing":{const b={driveway:0.25,house:0.20,both:0.22,deck:0.30};let p=s.sqft*(b[s.surface]||0.22);if(s.level==="heavy")p*=1.4;return Math.max(Math.round(p),100);}
    case"Interior Painting":{let rate=s.ceilingHeight>8?3.5:(s.wallSqft>0?3:2.5);let p=s.wallSqft*rate;if(s.trim)p+=s.wallSqft*0.5;if(s.grade==="premium")p*=1.3;if(s.colorChange)p+=200;return Math.round(p);}
    case"Drywall Repair/Installation":{if(s.patchType==="small")return 120;if(s.patchType==="medium"&&s.ceiling)return 350;if(s.waterDamage)return Math.round(s.sqft*2.5+400);let p=s.sqft*2;if(s.textureMatch)p+=200;if(s.ceiling)p*=1.2;return Math.round(p);}
    case"Flooring Installation":{const m={hardwood:12,laminate:5,tile:8,vinyl:4};let p=s.sqft*(m[s.material]||5);if(s.underlayment)p+=s.sqft*0.5;if(s.furnitureMoving)p+=300;if(s.stairs)p+=600;return Math.round(p);}
    case"Garage Door Service":{const d=s.doorSize==="double"?1.5:1;if(s.springIssue)return Math.round(250*d);if(s.offTrack)return Math.round(300*d);let p=d*1200;if(s.newOpener)p+=600;if(s.insulated)p+=400;return Math.round(p);}
    case"Solar Panel Installation":{if(s.roofType==="tile")return 25000;let p=s.panels*2500;if(s.battery)p+=10000;if(s.mount==="ground")p+=3000;if(s.permit)p+=1000;return Math.round(p);}
    case"Deck Building/Repair":{const m={treated:30,cedar:40,composite:45};let p=s.sqft*(m[s.material]||30);if(s.railings)p+=s.sqft*15;if(s.stairs>0)p+=s.stairs*200;if(s.height==="second")p*=1.3;if(s.stain)p+=500;return Math.round(p);}
    case"Pool Service/Repair":{const t=s.poolType==="in-ground"?1.2:1;if(s.service==="weekly")return Math.round(150*t);if(s.equipment==="pump")return Math.round(1000*t);if(s.equipment==="heater")return Math.round(1500*t);if(s.opening)return Math.round(450*t);if(s.closing)return Math.round(300*t);return 100;}
    case"Appliance Repair":{const a={refrigerator:400,washer:280,oven:350,dishwasher:250,dryer:280};let p=a[s.appliance]||300;if(s.underWarranty)p=100;return Math.round(p);}
    case"Siding Installation/Repair":{const m={vinyl:6,"fiber-cement":10,wood:8};let p=s.sqft*(m[s.material]||6);if(!s.newInstall&&s.sqft>0)p=Math.round(p*0.6);if(s.trim)p+=500;if(s.stories>1)p*=1.15;return Math.round(p);}
    case"Chimney Service":{if(s.service==="cleaning")return 250;if(s.service==="crown")return Math.round(550+(s.stories>1?200:0));if(s.service==="liner")return Math.round(2200+(s.stories>1?500:0));if(s.service==="damper")return 350;if(s.service==="rebuild")return 5500;return 250;}
    case"Foundation Repair":{if(s.bowingWall)return Math.round(4500);if(s.waterInBasement&&s.location==="exterior")return Math.round(4000);if(s.location==="exterior"&&s.crackWidth!=="0")return Math.round(2500);if(s.crackWidth==="1/8in")return 700;return 1500;}
    case"Mold Remediation":{if(s.location==="hvac")return 800;let p=s.sqft*4;if(s.location==="whole-house")p*=1.5;if(s.waterSource)p+=300;if(s.mustySmell)p+=200;return Math.round(p);}
    case"Well Pump Service":{if(s.wellIssue==="no-water")return 250;if(s.wellIssue==="pump-failure")return Math.round(2000+(s.depth>200?500:0));if(s.wellIssue==="low-pressure")return 600;if(s.wellIssue==="no-power")return 350;if(s.wellIssue==="bacteria")return 400;return 250;}
    case"Septic System Service":{if(s.backup&&s.fieldIssue)return 5500;if(s.tankSize>=1500&&s.lastPumping>=10)return 5000;if(s.fieldIssue)return 5000;if(s.backup)return 700;return Math.round(s.tankSize*0.4);}
    case"Generator Installation":{if(s.kw===0&&!s.transferSwitch)return 700;if(s.kw===0&&s.transferSwitch)return 1400;let p=s.kw>=22?9000:5000;if(s.wholeHouse)p+=2000;if(s.permit)p+=1000;return Math.round(p);}
    case"Bathroom Remodeling":{if(s.type==="vanity")return 2800;if(s.type==="tub-shower")return 6000;if(s.type==="master")return 17000;if(s.type==="half-bath")return 5000;if(s.accessible)return 8000;return 5000;}
    case"Insulation Installation":{if(s.type==="encapsulation")return 2800;const r={blown:1.5,injection:1.8,"spray-foam":3,batt:0.75};let p=s.sqft*(r[s.type]||1.5);if(s.airSealing)p+=800;if(s.vaporBarrier)p+=500;return Math.round(p);}
    default:return 500;
  }
}
function calcBreakdown(svc,s){var items=[];
  switch(svc.name){
    case"Tree removal":{items.push({l:"Base rate",a:150});items.push({l:s.trees+" trees x "+s.height+"ft x $60",a:60*s.height*s.trees});if(s.stump)items.push({l:"Stump grinding",a:150*s.trees});if(s.nearHouse)items.push({l:"Near-house premium",a:800});if(s.emergency)items.push({l:"Emergency surcharge",a:500});return items;}
    case"Roof inspection/replacement":{if(s.material==="patch"){items.push({l:"Patch repair flat rate",a:750});return items;}var psf=s.material==="metal"?7.5:s.material==="architectural"?5.5:4.5;items.push({l:s.sqft+"sqft x $"+psf.toFixed(1)+" material",a:Math.round(s.sqft*psf)});if(s.layers>1)items.push({l:"Extra tear-off ("+s.layers+" layers x $2/sqft)",a:Math.round(s.sqft*(s.layers-1)*2)});if(s.pitch==="steep"){var p=Math.round((s.sqft*psf)*0.3);items.push({l:"Steep pitch surcharge (+30%)",a:p});}return items;}
    case"Emergency plumbing":{var lh=s.replace?4:s.emergency?1.5:1;var parts=s.replace?s.pipeType==="galvanized"?1200:600:s.emergency?250:80;items.push({l:"Trip fee",a:150});items.push({l:"Labor "+lh+"hr x $125",a:Math.round(lh*125)});items.push({l:"Parts/materials",a:parts});if(s.accessibility==="hard")items.push({l:"Hard access fee",a:200});if(s.emergency)items.push({l:"Emergency rush (+25%)",a:Math.round((150+lh*125+parts+(s.accessibility==="hard"?200:0))*0.25)});return items;}
    case"Electrical panel upgrade":{if(s.amperage===0){items.push({l:s.emergency?"Emergency breaker fix":"Fixture installation",a:s.emergency?250:180});return items;}items.push({l:s.amperage+"A panel upgrade",a:s.amperage===200?1800:1200});if(s.outdoor)items.push({l:"Outdoor-rated panel",a:400});if(s.permit)items.push({l:"Permit fee",a:250});return items;}
    case"Landscape design":{items.push({l:s.sqft+"sqft x $8 base design",a:s.sqft*8});if(s.patio)items.push({l:"Patio design",a:3000});if(s.plantings)items.push({l:"Planting plan",a:1500});if(s.hardscape)items.push({l:"Hardscape elements",a:2000});if(s.irrigation)items.push({l:"Irrigation system design",a:1200});return items;}
    case"HVAC repair":{if(!s.repair){items.push({l:s.tonnage+" ton "+s.system+" unit",a:Math.round(s.tonnage*(s.system==="heat-pump"?2800:2200))});items.push({l:"Installation labor & materials",a:1500});return items;}if(s.system==="duct"){items.push({l:"Ductwork cleaning & seal",a:1000});return items;}if(s.system==="furnace"){items.push({l:"Furnace blower motor repair",a:700});return items;}items.push({l:s.tonnage+" ton AC capacitor repair",a:380});return items;}
    case"Gutter cleaning":{items.push({l:s.linearFt+"ft x $"+(s.stories>1?"$1.50":"$1.00")+" base",a:Math.round(s.linearFt*(s.stories>1?1.5:1))});if(s.debris==="heavy")items.push({l:"Heavy debris surcharge (+50%)",a:Math.round(s.linearFt*(s.stories>1?1.5:1)*0.5)});return items;}
    case"Pest control":{const sv={low:150,moderate:300,severe:600};var base=sv[s.severity]||300;items.push({l:s.severity+" severity treatment",a:base});if(s.treatment==="termite")items.push({l:"Termite treatment add-on",a:400});if(s.treatment==="rodent")items.push({l:"Rodent exclusion add-on",a:200});items.push({l:s.sqft+"sqft x $0.05 inspection",a:Math.round(s.sqft*0.05)});return items;}
    case"Concrete driveway":{items.push({l:s.sqft+"sqft x $8 concrete",a:s.sqft*8});if(s.reinforced)items.push({l:"Rebar reinforcement x $2.50/sqft",a:Math.round(s.sqft*2.5)});if(s.demolition)items.push({l:"Demolition & removal",a:1200});if(s.finish==="stamp")items.push({l:"Stamped finish (+35%)",a:Math.round(s.sqft*8*0.35)});return items;}
    case"Fence installation":{var m={wood:25,vinyl:35,"chain-link":15,"split-rail":12};var mat=m[s.material]||25;items.push({l:s.linearFt+"ft x $"+mat+" material",a:s.linearFt*mat});items.push({l:s.linearFt+"ft x $12 labor",a:s.linearFt*12});if(s.gates>0)items.push({l:s.gates+" gate(s) x $150",a:s.gates*150});if(s.terrain==="slight-slope")items.push({l:"Slope surcharge (+15%)",a:Math.round((s.linearFt*mat+s.linearFt*12)*0.15)});if(s.terrain==="moderate-slope")items.push({l:"Moderate slope surcharge (+30%)",a:Math.round((s.linearFt*mat+s.linearFt*12)*0.3)});return items;}
    case"Window replacement":{const wp={double:500,triple:850};var pw=wp[s.pane]||500;if(s.story>1)pw=Math.round(pw*1.2);if(s.count===1)pw=1800;items.push({l:s.count+" x "+s.pane+"-pane window"+(s.count>1?"s":"")+" @ $"+pw+" each",a:s.count*pw});if(s.story>1&&s.count>1)items.push({l:"Second story (+20%)",a:Math.round(s.count*500*0.2)});return items;}
    case"Carpet cleaning":{items.push({l:s.sqft+"sqft x $0.35",a:Math.round(s.sqft*0.35)});if(s.stains)items.push({l:"Stain treatment",a:100});if(s.stairs)items.push({l:"Stair cleaning",a:75});return items;}
    case"Pressure washing":{const b={driveway:0.25,house:0.20,both:0.22,deck:0.30};var rate=b[s.surface]||0.22;items.push({l:s.sqft+"sqft "+s.surface+" x $"+rate.toFixed(2),a:Math.round(s.sqft*rate)});if(s.level==="heavy")items.push({l:"Heavy cleaning surcharge (+40%)",a:Math.round(s.sqft*rate*0.4)});return items;}
    case"Interior Painting":{var rate=s.ceilingHeight>8?3.5:(s.wallSqft>0?3:2.5);items.push({l:s.rooms+" rooms "+(s.wallSqft>0?s.wallSqft+"sqft":"")+" x $"+rate, a:Math.round((s.wallSqft||600)*rate)});if(s.trim)items.push({l:"Trim painting "+s.wallSqft+"sqft x $0.50",a:Math.round((s.wallSqft||600)*0.5)});if(s.grade==="premium")items.push({l:"Premium paint upgrade (+30%)",a:Math.round(((s.wallSqft||600)*rate)*0.3)});if(s.colorChange)items.push({l:"Color change fee",a:200});return items;}
    case"Drywall Repair/Installation":{if(s.patchType==="small"){items.push({l:"Small patch repair",a:120});return items;}if(s.patchType==="medium"&&s.ceiling){items.push({l:"Medium ceiling patch",a:350});return items;}if(s.waterDamage){items.push({l:"Water damage repair",a:400});items.push({l:s.sqft+"sqft x $2.50 drywall",a:Math.round(s.sqft*2.5)});return items;}items.push({l:s.sqft+"sqft x $2 drywall",a:s.sqft*2});if(s.textureMatch)items.push({l:"Texture matching",a:200});if(s.ceiling)items.push({l:"Ceiling premium (+20%)",a:Math.round(s.sqft*2*0.2)});return items;}
    case"Flooring Installation":{const mf={hardwood:12,laminate:5,tile:8,vinyl:4};var mrate=mf[s.material]||5;items.push({l:s.sqft+"sqft "+s.material+" x $"+mrate, a:s.sqft*mrate});if(s.underlayment)items.push({l:"Underlayment x $0.50/sqft",a:Math.round(s.sqft*0.5)});if(s.furnitureMoving)items.push({l:"Furniture moving fee",a:300});if(s.stairs)items.push({l:"Stair installation",a:600});return items;}
    case"Garage Door Service":{var d=s.doorSize==="double"?1.5:1;if(s.springIssue){items.push({l:"Spring repair ("+s.doorSize+")",a:Math.round(250*d)});return items;}if(s.offTrack){items.push({l:"Track realignment ("+s.doorSize+")",a:Math.round(300*d)});return items;}items.push({l:s.doorSize+" door x $"+Math.round(1200*d),a:Math.round(1200*d)});if(s.newOpener)items.push({l:"New opener",a:600});if(s.insulated)items.push({l:"Insulated door premium",a:400});return items;}
    case"Solar Panel Installation":{if(s.roofType==="tile"){items.push({l:"Tile roof solar install",a:25000});return items;}items.push({l:s.panels+" panels x $2,500",a:s.panels*2500});if(s.battery)items.push({l:"Battery storage",a:10000});if(s.mount==="ground")items.push({l:"Ground mount system",a:3000});if(s.permit)items.push({l:"Permit & interconnection",a:1000});return items;}
    case"Deck Building/Repair":{var md={treated:30,cedar:40,composite:45};var mrate=md[s.material]||30;items.push({l:s.sqft+"sqft "+s.material+" x $"+mrate,a:s.sqft*mrate});if(s.railings)items.push({l:"Railing system x $15/sqft",a:s.sqft*15});if(s.stairs>0)items.push({l:s.stairs+" stair(s) x $200",a:s.stairs*200});if(s.height==="second")items.push({l:"Second story premium (+30%)",a:Math.round(s.sqft*mrate*0.3)});if(s.stain)items.push({l:"Staining & sealing",a:500});return items;}
    case"Pool Service/Repair":{var t=s.poolType==="in-ground"?1.2:1;if(s.service==="weekly"){items.push({l:"Weekly service ("+s.poolType+")",a:Math.round(150*t)});return items;}if(s.equipment==="pump"){items.push({l:"Pump replacement ("+s.poolType+")",a:Math.round(1000*t)});return items;}if(s.equipment==="heater"){items.push({l:"Heater repair ("+s.poolType+")",a:Math.round(1500*t)});return items;}if(s.opening){items.push({l:"Pool opening ("+s.poolType+")",a:Math.round(450*t)});return items;}if(s.closing){items.push({l:"Pool closing ("+s.poolType+")",a:Math.round(300*t)});return items;}items.push({l:"Pool service ("+s.poolType+")",a:100});return items;}
    case"Appliance Repair":{const ap={refrigerator:400,washer:280,oven:350,dishwasher:250,dryer:280};var base2=ap[s.appliance]||300;items.push({l:s.appliance.charAt(0).toUpperCase()+s.appliance.slice(1)+" repair",a:base2});if(s.underWarranty)items.push({l:"Warranty discount",a:-Math.round(base2-100)});return items;}
    case"Siding Installation/Repair":{var ms={vinyl:6,"fiber-cement":10,wood:8};var mrate2=ms[s.material]||6;items.push({l:s.sqft+"sqft "+s.material+(s.newInstall?" new":" repair")+" x $"+mrate2,a:Math.round(s.sqft*mrate2)});if(!s.newInstall&&s.sqft>0){var rep=Math.round(s.sqft*mrate2*0.6)-Math.round(s.sqft*mrate2);items.push({l:"Repair vs new discount (-40%)",a:Math.round(rep)});}if(s.trim)items.push({l:"Trim installation",a:500});if(s.stories>1)items.push({l:"Multi-story premium (+15%)",a:Math.round(s.sqft*mrate2*0.15)});return items;}
    case"Chimney Service":{if(s.service==="cleaning"){items.push({l:"Standard cleaning",a:250});return items;}if(s.service==="crown"){var c=550;if(s.stories>1){items.push({l:"Crown repair 2-story",a:750});return items;}items.push({l:"Crown repair",a:550});return items;}if(s.service==="liner"){items.push({l:"Liner replacement",a:2200});if(s.stories>1)items.push({l:"2-story surcharge",a:500});return items;}if(s.service==="damper"){items.push({l:"Damper replacement",a:350});return items;}if(s.service==="rebuild"){items.push({l:"Full chimney rebuild",a:5500});return items;}items.push({l:"Chimney service",a:250});return items;}
    case"Foundation Repair":{if(s.bowingWall){items.push({l:"Wall anchor system",a:4500});return items;}if(s.waterInBasement&&s.location==="exterior"){items.push({l:"Exterior waterproofing",a:4000});return items;}if(s.location==="exterior"&&s.crackWidth!=="0"){items.push({l:"Exterior crack repair",a:2500});return items;}if(s.crackWidth==="1/8in"){items.push({l:"Minor crack injection",a:700});return items;}items.push({l:"Foundation drainage system",a:1500});return items;}
    case"Mold Remediation":{if(s.location==="hvac"){items.push({l:"HVAC mold treatment",a:800});return items;}items.push({l:s.location+" remediation "+s.sqft+"sqft x $4",a:s.sqft*4});if(s.location==="whole-house")items.push({l:"Whole-house containment (+50%)",a:Math.round(s.sqft*4*0.5)});if(s.waterSource)items.push({l:"Water source remediation",a:300});if(s.mustySmell)items.push({l:"Odor treatment",a:200});return items;}
    case"Well Pump Service":{if(s.wellIssue==="no-water"){items.push({l:"Diagnostic visit",a:250});return items;}if(s.wellIssue==="pump-failure"){items.push({l:"Submersible pump replacement",a:2000});if(s.depth>200)items.push({l:"Deep well surcharge (>200ft)",a:500});return items;}if(s.wellIssue==="low-pressure"){items.push({l:"Pressure tank replacement",a:600});return items;}if(s.wellIssue==="no-power"){items.push({l:"Control box repair",a:350});return items;}if(s.wellIssue==="bacteria"){items.push({l:"Well chlorination treatment",a:400});return items;}items.push({l:"Well service",a:250});return items;}
    case"Septic System Service":{if(s.backup&&s.fieldIssue){items.push({l:"Drain field replacement",a:4500});items.push({l:"Emergency pumping",a:700});items.push({l:"System inspection",a:300});return items;}if(s.tankSize>=1500&&s.lastPumping>=10){items.push({l:"Tank replacement ("+s.tankSize+"gal)",a:4500});items.push({l:"Disposal & removal",a:500});return items;}if(s.fieldIssue){items.push({l:"Drain field repair",a:4500});items.push({l:"System inspection",a:500});return items;}if(s.backup){items.push({l:"Emergency pumping",a:450});items.push({l:"Inspection",a:250});return items;}items.push({l:s.tankSize+"gal tank pumping",a:Math.round(s.tankSize*0.4)});return items;}
    case"Generator Installation":{if(s.kw===0&&!s.transferSwitch){items.push({l:"Portable generator inlet",a:700});return items;}if(s.kw===0&&s.transferSwitch){items.push({l:"Transfer switch installation",a:1400});return items;}var gen=s.kw>=22?"22kW":"10kW";items.push({l:gen+" generator",a:s.kw>=22?9000:5000});if(s.wholeHouse)items.push({l:"Whole-house wiring",a:2000});if(s.permit)items.push({l:"Permit & inspection",a:1000});return items;}
    case"Bathroom Remodeling":{if(s.type==="vanity"){items.push({l:"Vanity & lighting update",a:2800});return items;}if(s.type==="tub-shower"){items.push({l:"Tub to shower conversion",a:4500});if(s.tile)items.push({l:"Tile work",a:1500});return items;}if(s.type==="master"){items.push({l:"Complete master bath gut",a:12000});if(s.tile)items.push({l:"Custom tile",a:3500});if(s.fixtures)items.push({l:"Premium fixtures",a:1500});return items;}if(s.type==="half-bath"){items.push({l:"Half bath powder room",a:3500});if(s.fixtures)items.push({l:"Fixtures & vanity",a:1500});return items;}if(s.accessible){items.push({l:"Accessible shower conversion",a:6000});items.push({l:"Grab bars & safety",a:2000});return items;}items.push({l:"Bathroom remodel",a:5000});return items;}
    case"Insulation Installation":{if(s.type==="encapsulation"){items.push({l:"Crawlspace encapsulation",a:2000});items.push({l:"Vapor barrier",a:800});return items;}const ri={blown:1.5,injection:1.8,"spray-foam":3,batt:0.75};var ratei=ri[s.type]||1.5;items.push({l:s.location+" "+s.sqft+"sqft "+s.type+" x $"+ratei.toFixed(1),a:Math.round(s.sqft*ratei)});if(s.airSealing)items.push({l:"Air sealing",a:800});if(s.vaporBarrier)items.push({l:"Vapor barrier",a:500});return items;}
    default:{items.push({l:"Total estimate",a:calcPrice(svc,s)});return items;}
  }
}
