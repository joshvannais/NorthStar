...
- Exact grant, import, connection, retention and hold retries succeed only during the same current source-permission revision and lifecycle revision. Manual revocation or deletion retires those results, and deletion cancellation plus a new permission period does not make them visible again. Direct runtime entry calls and direct-table deletion guards enforce the same rule for all four source classes.
- The mounted journey verifies the 100-plus-1 cleanup boundary, exact cursor continuation, concurrent current same-key retention and hold replay, retired lifecycle and cleanup replay rejection, retention cleanup, both legal-hold/cleanup lock orderings, tenant isolation, owner/administrator authority, member denial, immutable history and no operational mutation.
