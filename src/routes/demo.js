...
router.get('/command-center/estimates/:estimateId/customer-estimate-links',async function(req,res){
  res.set('Cache-Control','no-store');res.vary('Cookie');const repository=require('../estimating/customerEstimateDeliveryRepository');try{const token=commandCenterToken(req,res),record=await commandCenterRepository.read(token),item=demoCanonicalItems(demoWorkspace(record)).find(value=>value.ids.estimate===req.params.estimateId);if(!item)return res.status(404).json({success:false,error:{message:'That demo estimate is unavailable.'}});const data=await repository.demoRead(commandCenterRepository.pool(),{sourceTokenHash:token.tokenHash,tenantId:token.tenantId,estimateId:req.params.estimateId});return res.json({success:true,data});}catch(error){const e=repository.failure(error);return res.status(e.status).json({success:false,error:{category:e.code,message:e.message}});}
});
router.post('/command-center/estimates/:estimateId/customer-estimate-links',async function(req,res){
  res.set('Cache-Control','no-store');res.vary('Cookie');const repository=require('../estimating/customerEstimateDeliveryRepository');if(!mutationBoundary(req,res,'customer-estimate-delivery'))return;if(!req.customerEstimateDeliveryBodyValidated)return res.status(400).json({success:false,error:{message:'Check the demo customer estimate link details.'}});try{const token=commandCenterToken(req,res),record=await commandCenterRepository.read(token),history=record.state.customerEstimateVersions?.[req.params.estimateId]||[],version=history.find(value=>value.id===req.body.versionId);if(!version)return res.status(409).json({success:false,error:{message:'Issue this demo estimate before creating a customer link.'}});const data=await repository.demoCreate(commandCenterRepository.pool(),{sourceTokenHash:token.tokenHash,tenantId:token.tenantId,estimateId:req.params.estimateId,idempotencyKey:req.get('Idempotency-Key')},req.body,version.document);return res.status(data.replayed?200:201).json({success:true,data});}catch(error){const e=repository.failure(error);return res.status(e.status).json({success:false,error:{category:e.code,message:e.message}});}
});
router.post('/command-center/estimates/:estimateId/customer-estimate-links/:linkId/revoke',async function(req,res){
  res.set('Cache-Control','no-store');res.vary('Cookie');const repository=require('../estimating/customerEstimateDeliveryRepository');if(!mutationBoundary(req,res,'customer-estimate-delivery'))return;if(!req.customerEstimateDeliveryBodyValidated||!req.body||Object.keys(req.body).length!==0)return res.status(400).json({success:false,error:{message:'Check the demo customer estimate link details.'}});try{const token=commandCenterToken(req,res),data=await repository.demoRevoke(commandCenterRepository.pool(),{sourceTokenHash:token.tokenHash,tenantId:token.tenantId,estimateId:req.params.estimateId,idempotencyKey:req.get('Idempotency-Key')},req.params.linkId);return res.json({success:true,data});}catch(error){const e=repository.failure(error);return res.status(e.status).json({success:false,error:{category:e.code,message:e.message}});}
});
