...
The independent micro-review of published head `9ec1812630a54be3811ec94155824abc868cecdd` found three issues that were unresolved at that head: fresh and historical migration-ledger paths did not have the same physical column order, mounted Jobber OAuth state exposed readable durable identifiers, and BroadcastChannel deduplication retained unvalidated or unbounded coordination history. Those findings were corrected additively through published 22-commit head `b794033e22874145fe7de8708b66c28b5e509b75`.

The next bounded follow-up found two remaining fail-closed defects: the physical-ledger metadata preflight did not reject comments attached separately to ledger constraints or index relations, and the mounted Jobber journey could exchange credentials before discovering that canonical token persistence was unavailable. Those corrections were published through 25-commit head `43752350fc9acbce57a80cc87c212cd9d9bbf53c`. The latest independent micro-review closed both defects, supported physical-ledger behavior, lower-level opaque OAuth state, and the bounded Jobber UI. It found no remaining P0 or P1.

The sole remaining blocker was test-only: the protected migration-010 provenance assertion read Windows checkout bytes instead of the committed Git blob. This final surgical correction changes only that test and this evidence document. Production application code, migrations, browser code, dependencies, configuration, and data remain unchanged from `43752350fc9acbce57a80cc87c212cd9d9bbf53c`. It does not reopen the closed constraint/index-comment, physical-ledger, Jobber, BroadcastChannel, refresh, onboarding, notification-preference, logout, Bearer-retirement, or opaque-state work.

The separately reported migration-010 hash discrepancy was not an implementation defect. It came from hashing a CRLF-converted checkout representation instead of the exact committed Git bytes. The authoritative provenance, binary-safe method, and two-checkout results are recorded below.
...
All 25 commits through `43752350fc9acbce57a80cc87c212cd9d9bbf53c` remain unchanged. This final correction adds exactly one test/documentation-only commit whose parent is `43752350fc9acbce57a80cc87c212cd9d9bbf53c`. The result is exactly 26 linear commits above base `dfff096241d3be4fd1580a741cfe21ee64a5dfb3` with zero merge commits. The live draft PR description records the additive commit's exact content-derived SHA, parent, final head, and final diff after publication; a commit cannot truthfully embed its own SHA in its own contents.
...
The independent Windows micro-review at `43752350fc9acbce57a80cc87c212cd9d9bbf53c` truthfully produced 23/24 focused migration tests because the old provenance assertion hashed the CRLF checkout representation. It did not establish a 24/24 result at that head.

After this test-only correction, the real PostgreSQL 18 focused migration gate passed 1 suite and 24/24 tests from the implementation checkout. The same gate independently passed 1 suite and 24/24 tests from a fresh genuine-history Windows checkout whose working-tree migration 010 was CRLF-expanded to 14,763 bytes. Both runs asserted the same raw Git-blob length of 14,419 bytes and SHA-256 `0087278b1fb0062ba88a4dd7e4699e2e5c4c98d78e822193e2e7c0bff5c9ca48`. The suite continued to execute the real production migration runner and all existing physical PostgreSQL comment, convergence, rollback, concurrency, and sequence cases; no required test was skipped.

The previously published mounted/lower-level Jobber OAuth gate (20/20), mounted external-family gate (9/9), account-containment gate (4/4), complete API run (10 suites, 129 tests), and complete serial Jest run (54 suites, 1,095 tests) remain historical validation for unchanged production code. They were not rerun or relabeled as evidence for this test/documentation-only delta. No browser code, timer, worker, listener, process, or other runtime handle changed, so no new complete API, serial Jest, browser, provider, or `--detectOpenHandles` campaign was required.
