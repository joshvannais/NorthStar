...
window.CustomerDetail = (function() {
  ...
  function reviewPinsMatch(review, selected) {
    ...
    return review && review.contract === 'NorthStarEstimateReview/v1' && p && ids && profile &&
      ...
      (p.revision ? review.originalRecordedAt === selected.snapshotCreatedAt && p.revision.number === review.selectedRevision && ['estimate-material-adoption-v1','estimate-material-adoption-v2'].indexOf(p.revision.calculationVersion)>=0 : review.recordedAt === selected.snapshotCreatedAt);
  }
  ...
  function materialLines(plan) {
    ...
    return plan.calculationVersion === 'estimate-material-plan-v2' ? plan.inputs.lines : [plan.inputs];
  }
  function materialResult(result, target) {
    ...
    if (result.lines) {
      ...
      text('Plan Total',decisionMoney(result.total,result.currency));return;
    }
  ...
  function materialSource(line,target) {
    var p=document.createElement('p');p.style.overflowWrap='anywhere';p.textContent='Price Source: '+(line.sourceType==='my_estimate'?'My Cost Estimate. ':'Price Information Entered By A Person. ')+line.sourceNote+' Price Date: '+(line.priceDate||'Not Provided')+'.';target.appendChild(p);
  }
  function renderMaterialPlan(review,parent) {
    ...
    if(!plans||plans.contract!=='estimate-material-plan-v2'||JSON.stringify(plans.sourcePins)!==JSON.stringify(review.pins)||plans.simulated!==review.simulated){para('Material planning is unavailable. Refresh this estimate.');return;}
    ...
    if(current&&current.action==='save'){
      ...
      materialResult(current.result,root);materialLines(current).forEach(function(line){materialSource(line,root);});
    ...
    function start(action){var lines=current&&current.inputs?materialLines(current).map(function(line){return Object.assign({},JSON.parse(JSON.stringify(line)),{lineId:line.lineId||crypto.randomUUID()});}):[Object.assign(emptyLine(),{material:review.materialReview.material||''})];_materialPlanDraft={action:action,estimateId:review.pins.estimateId,basis:materialPlanBasis(review),inputs:{lines:lines},reason:'',confirmed:false,result:null,request:null};render();focusField();}
    ...
    if(draft.action==='save'){
      draft.inputs.lines.forEach(function(line,index){var box=document.createElement('fieldset');box.style.cssText='min-width:0;margin:0.75rem 0;padding:0.75rem;border:1px solid currentColor;border-radius:0.5rem;';box.dataset.materialLine=line.lineId;var legend=document.createElement('legend');legend.textContent='Material '+(index+1);box.appendChild(legend);form.appendChild(box);var suffix=index===0?'':'-'+line.lineId;
        ...
        unit.onchange=function(){line.unit=unit.value;line.quantity='';line.unitPrice='';quantity.value='';price.value='';invalidate();status.textContent='The unit changed. Enter the quantity and price for this unit.';};
        var source=entry('Price Source','sourceType','cdPlanSourceType',{type:'select'});[['my_estimate','My Cost Estimate'],['entered_price','Price Information I Entered']].forEach(function(x){var o=document.createElement('option');o.value=x[0];o.textContent=x[1];source.appendChild(o);});source.value=line.sourceType;
        entry('Source Note','sourceNote','cdPlanSourceNote',{max:1000,required:true});entry('Price Date (Optional)','priceDate','cdPlanPriceDate',{type:'date'});
      ...
      var add=button('Add Material',function(){invalidate();var line=emptyLine();draft.inputs.lines.push(line);render();focusField('cdPlanMaterial-'+line.lineId);},form);add.disabled=draft.inputs.lines.length>=20;
    }
  ...
  function renderMaterialReview(review, parent) {
    ...
    if (review.adoptedMaterialPlan) {
      ...
      materialResult(review.financialCosts.material,details);materialLines(adopted).forEach(function(line){materialSource(line,details);});
