...
async function preview(client,input,review,item,body){
 ...
 return proposal.build({review,item,sources:{...context(raw,input,now,item),resources},now},body);
}
function demo(record,review,item,body,now=new Date(),workspace=null){
 ...
 return proposal.build({review,item,sources,now,expiresAt:record.expiresAt,simulated:true},body);
}
...
module.exports={context,preview,demo,failure};
