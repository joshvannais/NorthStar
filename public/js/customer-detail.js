...
window.CustomerDetail = (function() {
  ...
  function materialResult(result, target) {
    ...
    if (result.lines) {
      ...
      text('Plan Total',decisionMoney(result.total,result.currency));if(result.sourceAssessment)materialSourceAssessment(result.sourceAssessment,target);if(result.availabilityAssessment)materialAvailabilityAssessment(result.availabilityAssessment,target);return;
    }
  ...
  function materialAvailabilityAssessment(a,target){
  ...
  function renderMaterialPlan(review,parent) {
    ...
    if(current&&current.action==='save'){
      ...
      if(current.currentAvailabilityAssessment)materialAvailabilityAssessment(current.currentAvailabilityAssessment,root);
    ...
    if(draft.alternative){para('Compare With '+draft.alternative.old.material+' — Saved Plan Total: '+decisionMoney(draft.alternative.oldTotal,review.currency));para('Enter the alternative’s own quantity, price and evidence. Nothing changes until you review and save it.');}
    ...
    function invalidate(){draft.inputs.sourceAssessment=null;draft.inputs.availabilityAssessment=null;draft.confirmed=false;draft.result=null;draft.request=null;if(confirm)confirm.checked=false;if(result)result.replaceChildren();}
