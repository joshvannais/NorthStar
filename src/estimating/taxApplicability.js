...
function facts(v){
 ...
 for(const k of ['serviceOperation','propertyUse','workContext','customerExemption'])if(!text(v.evidenceRef[k],1000,true)||v[k]!==null&&v[k]!=='unknown'&&!v.evidenceRef[k].trim())fail();
 return v;
}
...
function matches(f,a){facts(f);applicability(a);return f.serviceOperation!==null&&f.propertyUse!=='unknown'&&f.workContext!=='unknown'&&f.customerExemption!=='unknown'&&a.serviceOperations.includes(f.serviceOperation)&&a.propertyUses.includes(f.propertyUse)&&a.workContexts.includes(f.workContext)&&a.customerExemptions.includes(f.customerExemption);}
