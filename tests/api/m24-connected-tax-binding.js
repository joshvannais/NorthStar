...
(async()=>{let f;try{
 ...
 let acquired;
 let registry={version:'tax-source-registry-v1',entries:[{context:jobs[0].public_context,review:{reviewedBy:'Fixture reviewer',reviewedOn:new Date().toISOString().slice(0,10),expiresOn:new Date(Date.now()+86400000).toISOString().slice(0,10),accessBasis:'Synthetic local fixture only'},documents:[{url:'https://official.example/fixture',allowedOrigins:['https://official.example'],effectiveOn:null,endsOn:null,coverage:'Fixture only',exclusions:'Not real tax coverage'}]}]};
 research.acquire=require('../../src/estimating/taxSourceBinding').createProductionTaxAcquisition({TAX_SOURCE_ACQUISITION_ENABLED:'true'},{getRegistry:()=>registry,transportFactory:()=>({acquire:async context=>{acquired=context;return [{url:'https://official.example/fixture',content:'Synthetic source requires independent review.',effectiveOn:null,endsOn:null,coverage:'Fixture only',exclusions:'No validated tax treatment'}];}})});
 ...
 assert.match(candidates[0].content.candidates[0].sourceRegistryDigest,/^[a-f0-9]{64}$/);result.cases.push('Production acquisition binding matched exact reviewed public registry and retained pack hash; no rate publication');result.pass=true;
