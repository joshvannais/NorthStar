...
process.env.AUTH_ACCESS_SECRET = 'm25-external-crm-field-service-performance-local-secret';
...
const output = (process.argv.find(value => value.startsWith('--output=')) || '--output=m25-external-crm-field-service-performance-result.json').slice(9);
...
const evidenceDigest = crypto.createHash('sha256').update('crm-field-service-performance-evidence').digest('hex');

function records(prefix, count, invalidZone = false) {
  return Array.from({ length: count }, (_, index) => ({
    externalRecordId: `${prefix}-${index + 1}`, externalVersion: 1, state: 'active', recordType: 'job',
    customerReference: `customer-${index + 1}`, leadReference: `lead-${index + 1}`, jobReference: `job-${index + 1}`,
    appointmentReference: null, estimateReference: null, recordState: index % 2 ? 'completed' : 'unknown',
...
(async () => {
  ...
  try {
    ...
    async function source(name) {
      const root = `/api/v1/learning/external-crm-field-service-sources/${name}`;
      const response = await write(root, '/consent', { action: 'grant', expectedRevision: 0, expectedDigest: 'none',
        reason: 'Measure a bounded normalized CRM and field-service page.', confirmed: true,
    ...
    const page = (consent, values, changes = {}) => ({ schemaVersion: 'm25-external-crm-field-service-v1',
    ...
    const cold = await source('field-service.performance-cold');
    ...
    for (const count of [1, 50, 100]) {
      const scale = await source(`field-service.performance-${count}`);
    ...
    const repeat = await source('field-service.performance-repeat');
    ...
    const invalid = await source('field-service.performance-invalid');
    ...
    const stored = (await fixture.ownerPool.query(`SELECT
      (SELECT count(*)::int FROM canonical_external_crm_field_service_import_runs WHERE source_key='field-service.performance-invalid') runs,
      (SELECT count(*)::int FROM canonical_external_crm_field_service_import_records WHERE source_key='field-service.performance-invalid') records`)).rows[0];
