...
process.env.AUTH_ACCESS_SECRET = 'm25-travel-import-operations-local-disposable-secret';
...
const output = (process.argv.find(value => value.startsWith('--output=')) || '--output=m25-travel-import-operations-result.json').slice(9);
...
const record = (id, version = 1, date = '2026-01-10') => ({ externalRecordId: id, externalVersion: version,
  state: 'active', jobReference: `job-${id}`, vehicleReference: `vehicle-${id}`,
  routeStartedAt: `${date}T13:00:00.000Z`, routeEndedAt: `${date}T14:00:00.000Z`, timeZone: 'America/New_York',
  distance: { value: '24', unit: 'mi', basis: 'gps' },
  fuel: { quantity: '3', unit: 'us_gal', costAmount: '11.70', currency: 'USD', basis: 'fuel_card' },
  evidenceClass: 'provider_recorded', sourceUpdatedAt: `${date}T14:05:00.000Z` });
const batch = (consent, records, cursorBefore = null, cursorAfter = null, mode = 'historical_backfill') => ({
  schemaVersion: 'm25-external-travel-actual-v1', mode, expectedConsentRevision: consent.revision,
  expectedConsentDigest: consent.digest, cursorBefore, cursorAfter, complete: mode === 'historical_backfill', records,
  reason: 'Stage reviewed travel records.', confirmed: true,
...
(async () => {
  ...
  try {
    ...
    const source = key => {
      const root = `/api/v1/learning/external-travel-sources/${key}`;
    ...
    response = await primary.write('/batches', batch(consent, [record('old-route')]));
    ...
    ledger.cases.push('Provider-neutral travel adapter lifecycle is revision pinned, idempotent and credential free.');
    ...
    response = await deletionSource.write('/batches', batch(deletionConsent, [record('delete-route')], null, 'stream-1', 'continuous_update'));
    assert.equal(response.status, 201, JSON.stringify(response.body));
    ...
    const blocked = await deletionSource.write('/batches', batch(deletionConsent, [record('later-route')], 'stream-1', 'stream-2', 'continuous_update'));
    ...
    ledger.cases.push('Deletion immediately revokes source use, blocks imports and completes by bounded tombstoning.');
    ...
    const records = Array.from({ length: 101 }, (_, index) => record(`route-${String(index + 1).padStart(3, '0')}`, 1, '2026-09-15'));
    response = await paged.write('/batches', batch(pagedConsent, records.slice(0, 100), null, 'page-100', 'continuous_update'));
    assert.equal(response.status, 201, JSON.stringify(response.body));
    ...
    const resetRun = (await fixture.ownerPool.query(`SELECT authority_revision,previous_run_id
      FROM canonical_external_travel_cleanup_runs WHERE organization_id=$1 AND source_key=$2 AND operation='retention'
    ...
    ledger.cases.push('Changing retention authority starts a new cleanup chain at null so newly eligible routes cannot be skipped.');
    ...
    assert.deepEqual((await fixture.ownerPool.query("SELECT trim(checksum) checksum FROM _migrations WHERE filename='094_canonical_external_travel_import_operations.sql'")).rows, [{ checksum }]);
