...
router.get('/tax-preparation',requireTenantAccess,async function(req,res){
 ...
 try{client=await db.getPool().connect();await client.query('BEGIN ISOLATION LEVEL REPEATABLE READ');await client.query("SET LOCAL statement_timeout='10000ms'");await client.query("SET LOCAL lock_timeout='2000ms'");const data=await require('../estimating/taxPreparationRepository').read(client,require('../scheduling/operatorDirectory').actorInput(req),{acquisitionStatus:req.app.locals.taxAcquisitionStatus});await client.query('ROLLBACK');return res.json({success:true,data:{...data,mutationsPaused:!require('../estimating/commercialWritePolicy').mutationsEnabled||!require('../estimating/commercialWritePolicy').preparationEnabled,simulated:false}});}
