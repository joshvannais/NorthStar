...
function fail() { const error = new Error('Imported labor calibration details are invalid.');
  error.code = 'M25_IMPORTED_CALIBRATION_INPUT_INVALID'; error.status = 400; throw error; }
function exact(value, keys) { return value && typeof value === 'object' && !Array.isArray(value) &&
  Object.keys(value).sort().join('|') === [...keys].sort().join('|'); }
function text(value) { return typeof value === 'string' && value === value.trim() && value === value.normalize('NFC') &&
...
function normalizeConsent(sourceKey, body) {
  ...
  if (!exact(body, CONSENT_KEYS) || !['grant', 'revoke'].includes(body.action) || !Number.isInteger(body.expectedRevision) ||
      ...
      ((body.expectedRevision === 0) !== (body.expectedDigest === 'none')) || !text(body.reason) || body.confirmed !== true ||
      body.confirmationVersion !== 'm25-imported-labor-calibration-consent-v1') fail();
  return Object.freeze({ sourceKey: source, ...body });
}
function normalizeProposal(sourceKey, serviceKey, body) {
  const source = normalizeSourceKey(sourceKey); const service = normalizeServiceKey(serviceKey);
  if (!exact(body, PROPOSAL_KEYS) || !Number.isInteger(body.expectedConsentRevision) || body.expectedConsentRevision < 1 ||
      body.expectedConsentRevision > 10000 || !DIGEST.test(String(body.expectedConsentDigest || '')) || !text(body.reason) ||
      body.confirmed !== true || body.confirmationVersion !== 'm25-imported-labor-calibration-proposal-v1') fail();
