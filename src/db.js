...
async function grantAndVerifyRuntimeAuthority(client, authority) {
    ...
    BEGIN
      ...
      IF pg_catalog.to_regclass('public.canonical_field_executions') IS NOT NULL THEN
        ...
        IF pg_catalog.to_regprocedure(
          'public.canonical_field_execution_read_by_appointment(uuid,uuid,text,uuid,uuid)'
        ) IS NOT NULL THEN
          EXECUTE pg_catalog.format(
            'REVOKE ALL ON FUNCTION public.canonical_field_execution_read_by_appointment(uuid,uuid,text,uuid,uuid) FROM %I',
            runtime_role
          );
          EXECUTE pg_catalog.format(
            'GRANT EXECUTE ON FUNCTION public.canonical_field_execution_read_by_appointment(uuid,uuid,text,uuid,uuid) TO %I',
            runtime_role
          );
        END IF;
        IF pg_catalog.to_regprocedure(
      ...
      END IF;
      IF pg_catalog.to_regclass('public.canonical_equipment_learning_consents') IS NOT NULL THEN
        EXECUTE pg_catalog.format(
          'REVOKE ALL PRIVILEGES ON TABLE public.canonical_equipment_learning_consents, public.canonical_native_equipment_utilization_observations FROM %I',
          runtime_role
        );
        EXECUTE pg_catalog.format('REVOKE ALL ON FUNCTION public.canonical_equipment_learning_consent_projection(public.canonical_equipment_learning_consents) FROM %I', runtime_role);
        EXECUTE pg_catalog.format('REVOKE ALL ON FUNCTION public.canonical_native_equipment_utilization_basis(uuid,uuid) FROM %I', runtime_role);
        EXECUTE pg_catalog.format('REVOKE ALL ON FUNCTION public.canonical_native_equipment_utilization_projection(public.canonical_native_equipment_utilization_observations) FROM %I', runtime_role);
        EXECUTE pg_catalog.format('GRANT EXECUTE ON FUNCTION public.canonical_equipment_learning_consent_read(uuid,uuid,text,uuid) TO %I', runtime_role);
        EXECUTE pg_catalog.format('GRANT EXECUTE ON FUNCTION public.canonical_equipment_learning_consent_mutate(uuid,uuid,text,uuid,text,text,jsonb) TO %I', runtime_role);
      ...
      IF pg_catalog.to_regclass('public.canonical_forecast_price_event_snapshots') IS NOT NULL THEN
        ...
        EXECUTE pg_catalog.format('REVOKE ALL ON FUNCTION public.canonical_forecast_price_event_snapshot_immutable() FROM %I', runtime_role);
        EXECUTE pg_catalog.format('REVOKE ALL ON FUNCTION public.canonical_forecast_price_event_snapshot_guard() FROM %I', runtime_role);
        EXECUTE pg_catalog.format('REVOKE ALL ON FUNCTION public.canonical_forecast_price_event_snapshot_projection(public.canonical_forecast_price_event_snapshots) FROM %I', runtime_role);
      ...
      IF pg_catalog.to_regclass('public.canonical_forecast_settings_revisions') IS NOT NULL THEN
        ...
        EXECUTE pg_catalog.format('REVOKE ALL ON FUNCTION public.canonical_forecast_settings_immutable() FROM %I', runtime_role);
        EXECUTE pg_catalog.format('GRANT EXECUTE ON FUNCTION public.canonical_forecast_settings_capture(uuid,uuid,text,uuid,text,text,text,integer,text,text,jsonb) TO %I', runtime_role);
        EXECUTE pg_catalog.format('GRANT EXECUTE ON FUNCTION public.canonical_forecast_settings_read(uuid,uuid,text,uuid) TO %I', runtime_role);
      END IF;
      IF pg_catalog.to_regclass('public.canonical_forecast_profile_month_attestations') IS NOT NULL THEN
        EXECUTE pg_catalog.format('REVOKE ALL PRIVILEGES ON TABLE public.canonical_forecast_profile_month_attestations FROM %I', runtime_role);
        EXECUTE pg_catalog.format('REVOKE ALL ON FUNCTION public.canonical_forecast_profile_month_attestation_immutable() FROM %I', runtime_role);
      ...
      IF pg_catalog.to_regclass('public.canonical_forecast_retell_call_snapshots') IS NOT NULL THEN
        ...
        EXECUTE pg_catalog.format('REVOKE ALL ON FUNCTION public.canonical_forecast_retell_call_snapshot_immutable() FROM %I', runtime_role);
        EXECUTE pg_catalog.format('REVOKE ALL ON FUNCTION public.canonical_forecast_retell_call_snapshot_guard() FROM %I', runtime_role);
      ...
      END IF;
      IF pg_catalog.to_regclass('public.canonical_forecast_retell_source_consents') IS NOT NULL THEN
        EXECUTE pg_catalog.format('REVOKE ALL PRIVILEGES ON TABLE public.canonical_forecast_retell_source_consents FROM %I', runtime_role);
        EXECUTE pg_catalog.format('REVOKE ALL ON FUNCTION public.canonical_forecast_retell_source_consent_projection(public.canonical_forecast_retell_source_consents) FROM %I', runtime_role);
        EXECUTE pg_catalog.format('REVOKE ALL ON FUNCTION public.canonical_forecast_retell_source_permission_current(uuid,uuid,text) FROM %I', runtime_role);
        EXECUTE pg_catalog.format('REVOKE ALL ON FUNCTION public.canonical_forecast_retell_snapshot_consent_guard() FROM %I', runtime_role);
        EXECUTE pg_catalog.format('GRANT EXECUTE ON FUNCTION public.canonical_forecast_retell_source_consent_read(uuid,uuid,text,uuid) TO %I', runtime_role);
        EXECUTE pg_catalog.format('GRANT EXECUTE ON FUNCTION public.canonical_forecast_retell_source_consent_mutate(uuid,uuid,text,uuid,text,text,jsonb) TO %I', runtime_role);
      END IF;
      ...
      IF pg_catalog.to_regclass('public.canonical_external_asset_import_consents') IS NOT NULL THEN
        ...
        IF pg_catalog.to_regclass('public.canonical_external_asset_reference_matches') IS NOT NULL THEN
          ...
          EXECUTE pg_catalog.format('REVOKE ALL ON FUNCTION public.canonical_external_asset_reference_target_basis(uuid,text,uuid) FROM %I', runtime_role);
        ...
        EXECUTE pg_catalog.format('REVOKE ALL ON FUNCTION public.canonical_external_asset_cost_valid(jsonb) FROM %I', runtime_role);
        EXECUTE pg_catalog.format('REVOKE ALL ON FUNCTION public.canonical_external_asset_maintenance_valid(jsonb) FROM %I', runtime_role);
        EXECUTE pg_catalog.format('REVOKE ALL ON FUNCTION public.canonical_external_asset_downtime_valid(jsonb) FROM %I', runtime_role);
        EXECUTE pg_catalog.format('REVOKE ALL ON FUNCTION public.canonical_external_asset_consent_projection(public.canonical_external_asset_import_consents) FROM %I', runtime_role);
        EXECUTE pg_catalog.format('REVOKE ALL ON FUNCTION public.canonical_external_asset_run_projection(public.canonical_external_asset_import_runs) FROM %I', runtime_role);
        EXECUTE pg_catalog.format('REVOKE ALL ON FUNCTION public.canonical_external_asset_record_projection(public.canonical_external_asset_import_records) FROM %I', runtime_role);
        EXECUTE pg_catalog.format('GRANT EXECUTE ON FUNCTION public.canonical_external_asset_import_consent_read(uuid,uuid,text,uuid,text) TO %I', runtime_role);
        EXECUTE pg_catalog.format('GRANT EXECUTE ON FUNCTION public.canonical_external_asset_import_consent_mutate(uuid,uuid,text,uuid,text,text,text,jsonb) TO %I', runtime_role);
        EXECUTE pg_catalog.format('GRANT EXECUTE ON FUNCTION public.canonical_external_asset_import_batch(uuid,uuid,text,uuid,text,text,text,jsonb) TO %I', runtime_role);
        EXECUTE pg_catalog.format('GRANT EXECUTE ON FUNCTION public.canonical_external_asset_import_read(uuid,uuid,text,uuid,text) TO %I', runtime_role);
      END IF;
  ...
  await require('./estimating/equipmentPlanDatabaseAuthority').grantAndVerify(client, authority.runtimeRole);
  await require('./estimating/equipmentCostDatabaseAuthority').grantAndVerify(client, authority.runtimeRole);
  await require('./estimating/equipmentReadinessDatabaseAuthority').grantAndVerify(client, authority.runtimeRole);
  await require('./estimating/travelPlanDatabaseAuthority').grantAndVerify(client, authority.runtimeRole);
  await require('./estimating/pricingPlanDatabaseAuthority').grantAndVerify(client, authority.runtimeRole);
  ...
  await require('./learning/importDatabaseAuthority').grantAndVerify(client, authority.runtimeRole);
  await require('./learning/travelImportDatabaseAuthority').grantAndVerify(client, authority.runtimeRole);
    ...
    `SELECT
       ...
       (SELECT bool_and(
         ...
         WHERE namespace.nspname = 'public'
           ...
           AND relation.relname <> 'canonical_estimate_proposal_adoptions'
       ...
       (to_regclass('public.canonical_equipment_learning_consents') IS NULL OR (
         NOT has_table_privilege($1,'public.canonical_equipment_learning_consents','SELECT,INSERT,UPDATE,DELETE')
         AND NOT has_table_privilege($1,'public.canonical_native_equipment_utilization_observations','SELECT,INSERT,UPDATE,DELETE')
       )) AS native_equipment_learning_tables_withheld,
       (to_regclass('public.canonical_equipment_learning_consents') IS NULL OR (
         has_function_privilege($1,'public.canonical_equipment_learning_consent_read(uuid,uuid,text,uuid)','EXECUTE')
         AND has_function_privilege($1,'public.canonical_equipment_learning_consent_mutate(uuid,uuid,text,uuid,text,text,jsonb)','EXECUTE')
         AND has_function_privilege($1,'public.canonical_native_equipment_utilization_observe(uuid,uuid,text,uuid,text,text,uuid,bigint,text,text,boolean,text)','EXECUTE')
         AND has_function_privilege($1,'public.canonical_native_equipment_utilization_read(uuid,uuid,text,uuid,uuid)','EXECUTE')
       )) AS native_equipment_learning_entry_execute,
       (to_regclass('public.canonical_equipment_learning_consents') IS NULL OR (
         NOT has_function_privilege($1,'public.canonical_equipment_learning_consent_projection(public.canonical_equipment_learning_consents)','EXECUTE')
       ...
       (to_regclass('public.canonical_forecast_price_event_snapshots') IS NULL OR (
         has_function_privilege($1,'public.canonical_forecast_price_event_snapshot_capture(uuid,uuid,text,uuid,text,text)','EXECUTE')
         AND has_function_privilege($1,'public.canonical_forecast_price_event_snapshot_read(uuid,uuid,text,uuid,uuid)','EXECUTE')
       )) AS price_event_snapshot_entries_allowed,
       (to_regclass('public.canonical_forecast_price_event_snapshots') IS NULL OR (
         NOT has_function_privilege($1,'public.canonical_forecast_price_decision_events(uuid,timestamptz)','EXECUTE')
         AND NOT has_function_privilege($1,'public.canonical_forecast_price_event_snapshot_immutable()','EXECUTE')
       ...
       (to_regclass('public.canonical_forecast_retell_source_consents') IS NULL OR
         NOT has_table_privilege($1,'public.canonical_forecast_retell_source_consents','SELECT,INSERT,UPDATE,DELETE')) AS retell_consent_table_withheld,
       (to_regclass('public.canonical_forecast_retell_source_consents') IS NULL OR (
         has_function_privilege($1,'public.canonical_forecast_retell_source_consent_read(uuid,uuid,text,uuid)','EXECUTE')
         AND has_function_privilege($1,'public.canonical_forecast_retell_source_consent_mutate(uuid,uuid,text,uuid,text,text,jsonb)','EXECUTE')
       )) AS retell_consent_entries_allowed,
       (to_regclass('public.canonical_forecast_retell_source_consents') IS NULL OR
         (NOT has_function_privilege($1,'public.canonical_forecast_retell_source_consent_projection(public.canonical_forecast_retell_source_consents)','EXECUTE')
          AND NOT has_function_privilege($1,'public.canonical_forecast_retell_source_permission_current(uuid,uuid,text)','EXECUTE')
          AND NOT has_function_privilege($1,'public.canonical_forecast_retell_snapshot_consent_guard()','EXECUTE'))) AS retell_consent_helper_withheld,
  ...
  if (!runtimePrivileges.schema_usage || runtimePrivileges.schema_create ||
      ...
      !runtimePrivileges.retell_consent_table_withheld ||
      !runtimePrivileges.retell_consent_entries_allowed ||
      !runtimePrivileges.retell_consent_helper_withheld ||
