...
(async()=>{let f;try{
 ...
 for(let i=1;i<32;i++){const b=JSON.parse(raw);b.args.question='Clarify fixture access item '+i;const r=await send(JSON.stringify(b));assert.equal(r.status,200,JSON.stringify(r.body));}assert.equal(generated,32);
const b=JSON.parse(raw);b.args.question='Another new fixture question';const rejected=await send(JSON.stringify(b));assert.equal(rejected.status,429);assert.equal(generated,32);assert.equal((await send()).status,200);assert.equal(generated,32);assert.equal((await f.ownerPool.query("SELECT count(*)::int n FROM canonical_voice_session_events WHERE event_type='grounded_tool'")).rows[0].n,32);result.cases.push('actual call history cap rejects new generation at thirty-two while exact current replay remains available');
 result.pass=true;
