...
The PDF keeps scope and financial details selectable, preserves Unicode contractor/customer identity through locally rendered document imagery and metadata, and paginates bounded long content. Its MIT-licensed pdfmake runtime and embedded Roboto font data load from local NorthStar assets only when a user requests a PDF. The PNG is rendered at high resolution and grows to fit the complete bounded projection rather than clipping it. Static exports contain no acceptance or question controls and no private action token.
