...
function fail() {
  const error = new Error('Material and purchasing calibration details are invalid.');
  error.code = 'M25_IMPORTED_MATERIAL_CALIBRATION_INPUT_INVALID';
  error.status = 400;
  throw error;
}
...
function normalizeConsent(sourceKey, body) {
  ...
  if (!exact(body, CONSENT_KEYS) || !['grant', 'revoke'].includes(body.action) ||
      ...
      body.confirmed !== true || body.confirmationVersion !== 'm25-imported-material-calibration-consent-v1') fail();
...
function normalizeProposal(sourceKey, serviceKey, body) {
  ...
  if (!exact(body, PROPOSAL_KEYS) || !Number.isInteger(body.expectedConsentRevision) ||
      ...
      body.confirmationVersion !== 'm25-imported-material-calibration-proposal-v1') fail();
