...
process.env.AUTH_ACCESS_SECRET = 'm25-part12k-learning-center-browser-secret';
...
(async () => {
  try {
    ...
    for (const layout of layouts) {
      ...
      await page.getByRole('button', { name: /Accounting Demo, External financial records source/i }).click();
      await page.locator('#learningDetailTitle').filter({ hasText: 'Accounting Demo · External financial records' }).waitFor();
      const body = await page.locator('body').innerText();
      for (const phrase of ['External Financial Records', 'Financial Outcome Comparisons', 'What Polaris Has Learned From External Financial Records', 'Recorded Revenue', 'Recorded Collections', 'Recorded Realized Cost', 'Recorded Margin', 'Legal And Audit Hold']) assert.match(body, new RegExp(phrase, 'i'));
    ...
    ledger.cases.push('Five isolated demo layouts show business-system permissions, evidence, independent outcome measures, lifecycle controls and planning suggestions without overflow or internal identifiers.');
    ...
    for (const [index, layout] of layouts.entries()) {
      ...
      const sourceKey = `accounting.browser-${index + 1}`; await page.locator('#learningSourceKind').selectOption('financial'); await page.locator('#learningSourceKey').fill(sourceKey);
      const grant = page.waitForResponse(response => response.url().endsWith(`/external-financial-sources/${sourceKey}/consent`) && response.request().method()==='POST'); await page.locator('#learningSourceAdd').click(); assert.equal((await grant).status(), 201);
      await page.locator('#learningDetailTitle').filter({ hasText: new RegExp(`Accounting Browser ${index + 1}.*External Financial Records`, 'i') }).waitFor(); assert.equal(await page.locator('#learningConsentCards .learning-consent-card').count(), 3);
      const place = page.waitForResponse(response => response.url().endsWith(`/external-business-sources/financial/${sourceKey}/hold`) && response.request().method()==='POST'); await page.getByRole('button',{name:'Place hold'}).click(); assert.equal((await place).status(),201);
