...
describe('Mission 24 Part 8 customer estimate preview', () => {
  ...
  test('always provides a downloadable draft while keeping issuance approval separate', () => {
    ...
    pending.item.snapshot={...pending.item.snapshot,taxRatePercent:0,tax:0,totalIncludingTax:1568.66,taxDisposition:{status:'calculated',reason:null}};
    ...
    expect(draft).toMatchObject({state:'draft',subtotal:'1568.66',tax:'0.00',total:'1568.66',taxes:[{label:'Estimated Tax',treatment:'zero_rate',amount:'0.00'}],capabilities:{downloadPdf:true,downloadImage:true,accept:false,askQuestion:false}});
    ...
    expect(documentRenderer.presentationRows(draft)).toContainEqual({label:'Estimated Tax · Zero rate',amount:'$0.00',kind:'tax'});
    expect(documentRenderer.presentationRows(draft)).not.toContainEqual({label:'Tax',amount:'Needs Review',kind:'tax'});
