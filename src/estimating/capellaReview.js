...
function buildCapellaReview(review, snapshot) {
  ...
  const direct = childRevision
    ? [...MATERIAL_ADOPTION_VERSIONS,require('./costAdoptionContract').VERSION,require('./equipmentCostComposition').VERSION,require('./travelCostComposition').VERSION].includes(childRevision.calculationVersion) ? priceCents(review.financialCosts?.knownDirectCosts) : null
