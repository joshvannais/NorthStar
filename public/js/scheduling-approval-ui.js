(function (global) {
  ...
  function textForEvidence(entry) {
    ...
    Object.assign(explanations,{
      travel_plan_changed:'The current travel plan or the travel included in this estimate needs review.',
      travel_location_changed:'A recorded starting point or job location changed. Review the travel plan.',
      travel_source_needs_review:'Some travel dates, measurements or source applicability are unconfirmed.',
      travel_location_coordinates_unknown:'Coordinates are missing, so distance from the operating area cannot be checked.',
      travel_outside_declared_radius:'The straight-line distance exceeds the company’s recorded service radius. Confirm the actual route and coverage.',
      travel_duration_unknown:'Travel duration has not been recorded.',
      travel_exceeds_declared_time:'The declared travel time exceeds the company’s recorded travel limit.',
      travel_buffer_review:'The declared travel time exceeds the recorded scheduling buffer. Review the surrounding jobs.',
      travel_target_origin_unknown:'The selected worker or crew’s starting location is unconfirmed.',
      travel_driving_route_unverified:'Driving routes and travel times have not been independently verified.',
      travel_access_needs_review:'Confirm that the reported access window and its dates apply to this job.',
      travel_access_closed:'Current recorded access information says this job is closed during the proposed time.',
      travel_loads_need_review:'Hauling quantities, capacity or disposal and return legs still need review.',
      travel_stages_need_review:'Work-stage timing or shared resources have unresolved gaps or conflicts.'
    });
    Object.assign(explanations,{equipment_readiness_changed:'The equipment plan or readiness review changed. Review its current information.',equipment_recorded_downtime:'Equipment has an open downtime record.',equipment_recorded_checkout:'Equipment is checked out to other work or an operator outside this team.',equipment_checkout_execution_unknown:'The practice equipment hold names this job, but a linked work record is not recorded.',equipment_recorded_fault:'An equipment fault needs review; a maintenance entry does not clear it.',equipment_operational_history_unknown:'A complete equipment history is not recorded.',equipment_equipment_source_changed:'The equipment configuration or reviewed source needs confirmation.',equipment_requirement_not_met:'Equipment does not match a recorded job requirement.',equipment_requirements_unknown:'Equipment requirements or specifications need confirmation.',equipment_source_date_unknown_or_expired:'Equipment availability has no current source end date.',equipment_reported_out_of_service:'The source reports equipment is out of service.',equipment_reported_condition_problem:'The source reports an equipment condition problem.',equipment_condition_unknown:'Equipment condition has not been reported.',equipment_required_window_unknown:'The required equipment work window is missing.',equipment_quantity_unknown:'The required or reported equipment quantity is missing.',equipment_reported_quantity_shortfall:'The reported equipment quantity is insufficient.',equipment_identified_asset_overlap:'The same identified equipment is required for overlapping work in this plan.',equipment_identified_asset_windows_unknown:'Confirm separate work windows before using the same identified equipment for multiple needs.',equipment_identified_asset_quantity:'A single identified asset cannot supply multiple items.',equipment_reported_window_unknown:'The reported equipment availability window is missing.',equipment_reported_window_shortfall:'Reported equipment availability does not cover these times.',equipment_source_expires_before_work_ends:'Equipment source validity ends before the work finishes.',equipment_location_unknown:'The required or reported equipment location is missing.',equipment_location_needs_confirmation:'Confirm that the reported location covers this work.',equipment_reported_restrictions:'Review the source’s recorded restrictions.',equipment_lead_time_after_work_start:'The reported lead time runs past the start of this work. Confirm delivery timing.',equipment_lead_time_unknown:'Equipment lead time has not been stated.',equipment_maintenance_due:'An equipment maintenance date or threshold needs review.',equipment_maintenance_meter_unknown:'Equipment maintenance cannot be compared with a current same-unit reading.',equipment_included_equipment_differs:'The estimate includes costs for a different equipment review.'});
  ...
  function responseFailure(response, body, context) {
    ...
    if(response.status===503&&['M22_SCHEDULING_PAUSED','DEMO_SCHEDULE_PAUSED'].indexOf(code)>=0)message='New scheduling changes are paused. Refresh to check the saved appointment and dispatch status.';
      ...
      return response.json().catch(function () { return null; }).then(function (body) {
        if (!response.ok || !body || body.success !== true) {
          ...
          if (simulated && context !== 'team-search') {
            if (response.status >= 500 && ['M22_SCHEDULING_PAUSED','DEMO_SCHEDULE_PAUSED'].indexOf(failure.code)<0) failure.message = 'The result could not be confirmed. Refresh to check the appointment and its dispatch status before starting a different change; retrying this attempt keeps the same request.';
