...
async function grantAndVerify(client, runtimeRole) {
  ...
  if (tables.length !== 8 || functions.filter(fn => ENTRIES.has(fn.proname)).length !== ENTRIES.size) throw new Error('Travel import authority unavailable');
}
