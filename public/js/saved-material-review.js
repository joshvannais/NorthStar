(function(root, factory) {
  ...
  function project(review) {
    ...
    var readiness=review.equipmentReadiness&&review.equipmentReadiness.current;
    if(readiness){facts.push(['Current Equipment Readiness',readiness.action==='withdraw'?'Review Withdrawn':readiness.currentSourcesChanged?'Evidence Changed':readiness.result&&({blocked:'Recorded Conflict',needs_review:'Needs Review',no_recorded_conflict:'No Recorded Conflict For This Plan',not_required:'Not Required For This Plan'}[readiness.result.status])||'Not Recorded']);notes.push('Current equipment readiness informs scheduling review; it does not change the equipment costs included in this estimate or reserve equipment.');if(readiness.result&&readiness.result.lines.some(function(l){return l.review.indexOf('included_equipment_differs')>=0;}))notes.push('The included equipment costs refer to a different equipment plan. Review and apply fresh costs separately before approving a new price.');}
    var travel=review.adoptedTravelPlan,latestTravel=review.travelPlans&&review.travelPlans.current,travelRow=review.rows.find(function(r){return r.label==='Recorded travel cost';});
    facts.push(['Included Travel Cost',amount(travel?review.financialCosts&&review.financialCosts.knownTravelInternalCost:travelRow&&travelRow.sourceState==='recorded'?travelRow.amount:null,review.currency)]);
    notes.push(travel?'Travel uses the included saved plan and its reviewed cost allocations. Recorded distances and times do not establish verified driving directions or resource availability.':'Travel costs retain the original estimate basis.');
    if(travel&&review.financialCosts&&review.financialCosts.travelAlreadyIncluded&&review.financialCosts.travelAlreadyIncluded!=='0.00')facts.push(['Travel Already Included In Other Costs',amount(review.financialCosts.travelAlreadyIncluded,review.currency)]);
    if(latestTravel&&latestTravel.action==='save'&&(!travel||latestTravel.id!==travel.id)){facts.push([latestTravel.result&&latestTravel.result.complete?'Latest Saved Travel Plan':'Latest Travel Known Cost Subtotal',amount(latestTravel.result&&(latestTravel.result.complete?latestTravel.result.total:latestTravel.result.knownCostSubtotal),review.currency)]);notes.push('The latest travel plan is separate from the costs included in this estimate.');}
    if(latestTravel&&latestTravel.action==='withdraw')notes.push('The latest travel plan was withdrawn. Earlier estimates retain their included travel costs and sources.');
    ...
    var policy=review.pricingPolicyCheck;
    if(policy){notes.push(policy.message);if(policy.result){facts.push(['Pricing Policy Threshold',amount(policy.result.threshold,review.currency)],['Recorded Costs With Declared Allowance',amount(policy.result.policyCost,review.currency)]);if(policy.result.proposed)facts.push(['Proposal Difference From Policy Threshold',amount(policy.result.proposed.thresholdDifference,review.currency)]);if(policy.result.reviewed)facts.push(['Reviewed Price Difference From Policy Threshold',amount(policy.result.reviewed.thresholdDifference,review.currency)]);}}
    var commercial=review.commercialTerms;
    if(commercial){var terms=commercial.current,approved=commercial.approvalState==='commercial_approved'&&commercial.binding&&terms&&terms.current;
      notes.push(approved?'The current scope, price and full commercial terms were approved together.':commercial.approvalState==='scope_price_only'?'The scope and price review does not approve full commercial terms or tax treatment.':'No current full commercial approval is recorded.');
      if(terms&&terms.action==='save'&&terms.result){facts.push([approved?'Approved Net Price Before Tax':'Saved Commercial Net Price Before Tax',amount(terms.result.netBeforeTax,review.currency)],['Tax Collected Separately',amount(terms.result.tax,review.currency)],['Total Payable',amount(terms.result.total,review.currency)]);notes.push('Tax collected does not increase the amount remaining after costs. Payment stages divide the total; they are not additional charges.');if(!terms.current)notes.push('These commercial terms belong to an earlier review and need renewed approval.');if(terms.result.taxAuthority==='owner_recorded')notes.push('The company recorded externally reviewed tax treatment. NorthStar has not validated this treatment.');}
    }
