...
function buildLegacyArtifact({manifest,candidates,registry}){
...
function buildV2Artifact({manifest,candidates,registry}){
 if(!manifest||manifest.version!=='tax-coverage-review-v2'||!['synthetic_fixture','reviewed_release'].includes(manifest.mode)||!Array.isArray(manifest.entries)||!manifest.entries.length||manifest.entries.length>100||!Array.isArray(candidates)||candidates.length>100||!Array.isArray(registry)||registry.length>1000)fail();
 const review=manifest.review;if(!review||review.verdict!=='approved'||typeof review.authorId!=='string'||!review.authorId||typeof review.reviewerId!=='string'||!review.reviewerId||review.authorId===review.reviewerId||!day(review.reviewedOn)||!hex(review.evidenceDigest))fail();
 const keys=new Set(),operations=[];
 for(const entry of manifest.entries){
  if(!entry||typeof entry.ruleKey!=='string'||!entry.ruleKey||entry.ruleKey.length>200||keys.has(entry.ruleKey)||!Number.isSafeInteger(entry.expectedRevision)||entry.expectedRevision<0||entry.expectedRevision>10000||!(entry.expectedDigest==='none'&&entry.expectedRevision===0||hex(entry.expectedDigest)&&entry.expectedRevision>0)||!hex(entry.candidateDigest)||!hex(entry.sourceDigest)||!hex(entry.reviewedRuleDigest))fail();keys.add(entry.ruleKey);
  const matches=candidates.filter(c=>c.digest===entry.candidateDigest);if(matches.length!==1)fail();const c=matches[0];if(typeof c.contentText!=='string'||Buffer.byteLength(c.contentText)>65536||hash(c.contentText)!==c.digest)fail();let candidate;try{candidate=JSON.parse(c.contentText);}catch(_){fail();}
  if(candidate.version!=='tax-source-candidate-v1'||candidate.state!=='candidate'||!Array.isArray(candidate.candidates))fail();const sources=candidate.candidates.filter(s=>s.contentDigest===entry.sourceDigest);if(sources.length!==1)fail();const source=sources[0];if(typeof source.excerpt!=='string'||hash(source.excerpt)!==source.contentDigest||!day(source.fetchedOn)||source.effectiveOn!==null&&!day(source.effectiveOn)||source.endsOn!==null&&!day(source.endsOn)||source.effectiveOn&&source.endsOn&&source.endsOn<source.effectiveOn||source.reviewRequired!==true)fail();
  const rule=entry.rule;try{require('./taxApplicability').rule(rule);}catch(_){fail();}if(rule.reviewedOn!==review.reviewedOn||source.effectiveOn&&rule.legalEffectiveOn<source.effectiveOn||source.endsOn&&(rule.legalEndsOn===null||rule.legalEndsOn>source.endsOn))fail();
  const context=publicContext(source.context);for(const key of Object.keys(context))if(rule[key]!==context[key])fail();
  const contentText=stableStringify(rule);if(hash(contentText)!==entry.reviewedRuleDigest)fail();
  const old=registry.filter(r=>r.ruleKey===entry.ruleKey).sort((a,b)=>b.revision-a.revision)[0];if((old?.revision||0)!==entry.expectedRevision||(old?.digest||'none')!==entry.expectedDigest)fail();
  operations.push({ruleKey:entry.ruleKey,expectedRevision:entry.expectedRevision,expectedDigest:entry.expectedDigest,revision:entry.expectedRevision+1,content:rule,digest:entry.reviewedRuleDigest,candidateDigest:entry.candidateDigest,sourceDigest:entry.sourceDigest});
 }
 const content={version:'tax-coverage-publication-artifact-v2',mode:manifest.mode,syntheticFixture:manifest.mode==='synthetic_fixture',releaseAuthorizationRequired:true,reviewEvidenceDigest:review.evidenceDigest,reviewerId:review.reviewerId,operations};
 return {...content,artifactDigest:hash(stableStringify(content))};
}
function buildCoverageArtifact(input){if(input?.manifest?.version==='tax-coverage-review-v2')return buildV2Artifact(input);if(input?.manifest?.mode==='reviewed_release')fail();return buildLegacyArtifact(input);}
function verifyCoverageArtifact(artifact,input){const rebuilt=input?.manifest?.version==='tax-coverage-review-v1'?buildLegacyArtifact(input):buildCoverageArtifact(input);if(stableStringify(artifact)!==stableStringify(rebuilt))fail();return {matches:true,writes:0,releaseAuthorizationRequired:true,syntheticFixture:rebuilt.syntheticFixture};}
