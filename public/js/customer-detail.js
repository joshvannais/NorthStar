...
window.CustomerDetail = (function() {
  ...
  function renderEquipmentCosts(review,parent){
    ...
    if(!_equipmentCostDraft){var controls=el('div');controls.style.cssText='display:flex;flex-wrap:wrap;gap:.75rem;margin:.75rem 0;';button(current&&current.action==='save'?'Revise Equipment Costs':'Plan Equipment Costs',function(){start('save');},controls).id='cdEquipmentCostStart';if(current&&current.action==='save')button('Withdraw Equipment Costs',function(){start('withdraw');},controls);return;}
