...
The authorized owner or administrator can explicitly dismiss or request a handoff. A click on a forecast card or record link is navigation, not consent or approval. Before presenting any action, NorthStar must recheck the current session, forecast permission, source purpose, run validity and scope. Creating, dismissing or consuming a Mission 26 proposal or review record is itself a forecast-history mutation and requires current mutation authority, CSRF, idempotency and exact current revisions where applicable. The receiving workflow then independently rechecks its own tenant, role, record scope, eligibility, source facts and exact revision; it uses its own CSRF and idempotency rules for any receiving mutation. A current forecast does not prove that a future schedule slot, supplier price, employee qualification, equipment assignment or customer permission exists. Rejected, expired and already-consumed proposals must not replay. A handoff cannot convert a deterministic scenario into a calibrated probability or replace a human's commercial judgment.
...
Final acceptance requires an authenticated saved run and valid Part 11C currentness; a bounded, auditable proposal and review record with Mission 26 mutation controls; real receiving-workflow adapters owned by each participating mission; denial and stale-revision tests; replay, concurrency, expiry and recovery tests; no receiving or operational mutation on navigation or proposal creation; customer-safe and private-data checks; paid/demo isolation; rendered plain-language and mobile review; and independent exact-head security and authority review. No CI, provider, private-production, physical-device or founder visual evidence is implied by this gate.
