...
suite('Mounted isolated demo scheduling',()=>{
 ...
 test('preview, explicit confirmation, exact replay and reload preserve original graphs',async()=>{
  ...
  const after=await read(),row=after.schedulingOverview.records.find(r=>r.appointmentId===p.id);expect(row.allowedActions).toEqual(['reschedule','assign']);expect(row.authority.scheduledEnd).toBe(saved.body.data.scheduleAuthority.scheduledEnd);
 ...
 test('shared discovery and assign/reassign/unassign persist exact current targets without time or graph changes',async()=>{
  const initial=await read(),id=initial.schedulingOverview.records.find(r=>r.authority.scheduleState==='scheduled').appointmentId;
  const raw=(await f.ownerPool.query('SELECT state FROM demo_command_center_sessions')).rows[0].state.graphs;
  const directory=await agent.get('/api/demo/command-center/operator-targets?query=Crew').set('Cookie',cookie);expect(directory.status).toBe(200);expect(directory.body.data.targets.length).toBe(2);
  let ordinal=0;for(const action of ['assign','reassign','unassign']){const w=await read(),record=w.schedulingOverview.records.find(r=>r.appointmentId===id),target=action==='unassign'?{kind:'unassigned',id:null}:{kind:'crew',id:directory.body.data.targets[ordinal].id};const body={expectedRevision:record.authority.revision,expectedDigest:record.authority.digest,expectedTimeZone:w.configuration.businessProfile.timeZone,action,target,scheduledStart:time.formatInstant(record.authority.scheduledStart,w.configuration.businessProfile.timeZone).rfc3339,scheduledEnd:time.formatInstant(record.authority.scheduledEnd,w.configuration.businessProfile.timeZone).rfc3339,appointmentStatus:record.authority.appointmentStatus,reason:'Review this simulated crew change.'};const preview=await send(id,'mutation-previews','assignment-preview-'+action+'-0001',w.integrity.revision,body);expect(preview.status).toBe(201);const v=preview.body.data;const approval={previewId:v.id,previewDigest:v.previewDigest,acknowledgedWarningDigests:v.warningDigests,acknowledgedReviewReasonDigests:v.reviewReasonDigests,reason:body.reason};const saved=await send(id,'mutation-approvals','assignment-approve-'+action+'-0001',v.demoWorkspaceRevision,approval);expect(saved.status).toBe(201);const replay=await send(id,'mutation-approvals','assignment-approve-'+action+'-0001',v.demoWorkspaceRevision,approval);expect(replay.status).toBe(200);expect(replay.body.data).toEqual(saved.body.data);const after=await read(),current=after.schedulingOverview.records.find(r=>r.appointmentId===id).authority;expect(current.workforceCrewId).toBe(target.id);expect(current.scheduledStart).toBe(record.authority.scheduledStart);expect(current.scheduledEnd).toBe(record.authority.scheduledEnd);expect(after.graphs.find(g=>g.ids.appointment===id).work.assignedTo).toBe(current.targetLabel);ordinal++;}
  expect((await f.ownerPool.query('SELECT state FROM demo_command_center_sessions')).rows[0].state.graphs).toEqual(raw);
 });
