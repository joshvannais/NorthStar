...
| External historical and continuous sources | `canonical_external_labor_import_consents`, `canonical_external_labor_import_runs`, `canonical_external_labor_import_records` and guarded external-labor routes | Older and ongoing CRM, fleet, accounting, payroll, fuel, routing, procurement and communications outcomes | Provider-neutral normalized labor/time envelope released: explicit source consent, bounded historical/continuous batches, cursors, schema/version pins, dedupe, immutable corrections, tombstones and revocation. Adapter/provider connection, reviewed entity reconciliation and other source classes remain unimplemented. |
