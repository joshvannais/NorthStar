...
window.CustomerDetail = (function() {
  ...
  function renderCapellaScenarios(review,host){
    ...
    var draft=_capellaScenarioDraft,changed=draft.digest!==basis.digest;draft.digest=basis.digest;if(changed){draft.result=null;Object.keys(draft.scenarios).forEach(function(k){draft.scenarios[k].overheadSeparate=false;});}
    ...
    var status=el('p',changed?'The saved basis changed. Check your entries and overhead coverage before recalculating.':'',form);status.id='cdScenarioStatus';status.setAttribute('role','status');status.tabIndex=-1;
    var actions=el('div',null,form);actions.className='drawer-review-actions';var calculate=el('button','Calculate Scenarios',actions);calculate.type='submit';calculate.id='cdScenarioCalculate';calculate.className='btn btn-primary';calculate.disabled=!basis.enabled;var clear=el('button','Clear Analysis',actions);clear.type='button';clear.className='btn btn-secondary';clear.onclick=function(){_capellaScenarioDraft=null;renderCapellaReview(review);var d=$('cdCapellaScenarios');d.open=true;d.querySelector('summary').focus();};
    var results=el('div',null,details);results.id='cdScenarioResults';
    function invalidate(){draft.edit=(draft.edit||0)+1;draft.result=null;results.replaceChildren();status.textContent='Assumptions changed. Calculate again to update the comparison.';}
