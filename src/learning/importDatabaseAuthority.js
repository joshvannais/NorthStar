...
async function grantAndVerify(client, runtimeRole) {
  ...
  await client.query(`REVOKE ALL PRIVILEGES ON TABLE public.canonical_external_labor_import_consents,
    ...
    public.canonical_external_labor_calibration_proposals,
    public.canonical_external_labor_adapter_revisions,
    public.canonical_external_labor_retention_revisions,
    public.canonical_external_labor_deletion_revisions,
    public.canonical_external_labor_cleanup_runs FROM ${identifier}`);
  const helpers = [
    ...
    'public.canonical_external_labor_cleanup_projection(public.canonical_external_labor_cleanup_runs)',
  ...
  const entries = [
    ...
    'public.canonical_external_labor_operations_read(uuid,uuid,text,uuid,text)',
    'public.canonical_external_labor_adapter_mutate(uuid,uuid,text,uuid,text,text,text,jsonb)',
    'public.canonical_external_labor_retention_mutate(uuid,uuid,text,uuid,text,text,text,jsonb)',
    'public.canonical_external_labor_deletion_mutate(uuid,uuid,text,uuid,text,text,text,jsonb)',
    'public.canonical_external_labor_cleanup_execute(uuid,uuid,text,uuid,text,text,text,jsonb)'
  ];
  ...
  const privileges = (await client.query(`SELECT
    ...
    NOT has_table_privilege($1,'public.canonical_external_labor_adapter_revisions','SELECT')
      AND NOT has_table_privilege($1,'public.canonical_external_labor_retention_revisions','SELECT')
      AND NOT has_table_privilege($1,'public.canonical_external_labor_deletion_revisions','SELECT')
      AND NOT has_table_privilege($1,'public.canonical_external_labor_cleanup_runs','SELECT') operations_withheld,
    NOT has_function_privilege($1,'public.canonical_external_labor_import_consent_projection(public.canonical_external_labor_import_consents)','EXECUTE')
      ...
      AND NOT has_function_privilege($1,'public.canonical_external_labor_cleanup_projection(public.canonical_external_labor_cleanup_runs)','EXECUTE')
    ...
    has_function_privilege($1,'public.canonical_external_labor_operations_read(uuid,uuid,text,uuid,text)','EXECUTE')
      AND has_function_privilege($1,'public.canonical_external_labor_adapter_mutate(uuid,uuid,text,uuid,text,text,text,jsonb)','EXECUTE')
      AND has_function_privilege($1,'public.canonical_external_labor_retention_mutate(uuid,uuid,text,uuid,text,text,text,jsonb)','EXECUTE')
      AND has_function_privilege($1,'public.canonical_external_labor_deletion_mutate(uuid,uuid,text,uuid,text,text,text,jsonb)','EXECUTE')
      AND has_function_privilege($1,'public.canonical_external_labor_cleanup_execute(uuid,uuid,text,uuid,text,text,text,jsonb)','EXECUTE') operations_entries_allowed`,
    [runtimeRole])).rows[0];
  if (!privileges.tables_withheld || !privileges.matches_withheld || !privileges.imported_outcomes_withheld ||
      ...
      !privileges.learning_center_entry_allowed || !privileges.operations_withheld ||
      !privileges.operations_entries_allowed) {
