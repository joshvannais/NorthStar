...
function fail(message) { const error = new Error(message); error.code = 'M25_IMPORTED_OUTCOME_INPUT_INVALID'; error.status = 400; throw error; }
...
function normalizeConsent(sourceKey, body) {
  ...
  if (!exact(body, CONSENT_KEYS) || !['grant', 'revoke'].includes(body.action) || !Number.isInteger(body.expectedRevision) ||
      ...
      body.confirmationVersion !== 'm25-imported-labor-duration-consent-v1') fail('Imported labor learning consent details are invalid.');
...
function normalizeObservation(sourceKey, estimateId, body) {
  ...
  if (!exact(body, OBSERVATION_KEYS) || !OPAQUE.test(String(body.externalJobReference || '')) ||
      ...
      body.confirmationVersion !== 'm25-imported-labor-duration-observation-v1') fail('Imported labor outcome details are invalid.');
