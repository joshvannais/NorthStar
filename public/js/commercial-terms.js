(function () {
  ...
  function render(review, parent, options) {
    ...
    if (!plans || plans.contract !== 'estimate-commercial-terms-v1' || plans.simulated !== review.simulated) { el('p', 'Commercial terms are unavailable. Refresh this estimate.'); return; }
    ...
    if (plans.current) { el('p', plans.current.action === 'withdraw' ? 'These terms were withdrawn. Saved history remains available.' : plans.binding ? 'The current scope, price and commercial terms were approved together.' : plans.current.current ? 'Saved terms are a proposal until you approve the full review.' : 'Earlier terms — review current pricing and sources before approving.'); resultView(plans.current.result, root); }
    else el('p', 'Review discounts, fees, tax treatment and payment stages from the saved pricing proposal.');
    if (plans.history.length) { var history = el('details'); el('summary', 'Commercial History', history); plans.history.forEach(function (p) { var d = el('details', null, history); el('summary', (p.action === 'withdraw' ? 'Withdrawn Terms' : 'Saved Terms') + ' — ' + (p.actorName || 'Company Reviewer'), d); el('p', p.reason, d); resultView(p.result, d); }); }
    ...
    function source() { return {kind:'unknown',note:'',reference:'',effectiveOn:null,endsOn:null,jurisdiction:'',location:'',serviceKey:plans.sources.serviceKey,collectionBasis:'',acknowledged:false,ruleId:null,ruleDigest:null}; }
    ...
    if(active.action==='save'){
      ...
      v.taxGroups.forEach(function(g,i){var id='cdCommercialTax'+i,box=section(taxes,g.label||'Tax Group',id),s=g.source;
        ...
        var eligibleRules=(plans.sources.validatedRules||[]).filter(function(r){return r.serviceKey===plans.sources.serviceKey;});
        field(evidence,'Tax Source',id+'Kind',s.kind==='validated'?'rule:'+s.ruleId:s.kind,function(x){var r=eligibleRules.find(function(r){return 'rule:'+r.id===x;});if(r){g.treatment=r.treatment;g.ratePercent=r.ratePercent;g.behavior=r.behavior;g.source={kind:'validated',note:r.sourceNote||'Current validated source artifact',reference:r.sourceReference||r.id,effectiveOn:r.effectiveOn,endsOn:r.endsOn,jurisdiction:r.jurisdiction,location:'',serviceKey:r.serviceKey,collectionBasis:r.collectionBasis,acknowledged:false,ruleId:r.id,ruleDigest:r.digest};}else{s.kind=x;s.ruleId=null;s.ruleDigest=null;s.acknowledged=false;if(x==='unknown'){g.treatment='unknown';g.ratePercent=null;}}rerender(id+'Kind');},[['unknown','Not Yet Reviewed'],['owner_recorded','Recorded By My Company']].concat(eligibleRules.map(function(r){return['rule:'+r.id,plans.simulated?'Simulated Practice Coverage':'Available Validated Coverage'];})));
        if(s.kind==='validated'){el('p',plans.simulated?'Authored practice coverage only. This is not a real tax rate or registration.':'Coverage matches the saved company setup. Review the exact job location and date before approval.',evidence);field(evidence,'Job Location',id+'location',s.location,function(x){s.location=x;});check(evidence,'I reviewed this source and its applicability to the job.',id+'Acknowledged',s.acknowledged,function(x){s.acknowledged=x;});}
        ...
        if(s.kind==='owner_recorded'){
          ...
          field(evidence,'Effective Date',id+'Effective',s.effectiveOn,function(x){s.effectiveOn=nullable(x);}).type='date';field(evidence,'Known End Date',id+'Ends',s.endsOn,function(x){s.endsOn=nullable(x);}).type='date';
    ...
    if(active.action==='approve'){
      resultView(plans.current.result,form);
