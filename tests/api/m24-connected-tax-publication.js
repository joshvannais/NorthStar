...
(async()=>{let f;try{
 ...
 let acquired;
 research.acquire=createTaxSourceAcquisition({allowedOrigins:new Set(['https://official.example']),transport:{acquire:async context=>{acquired=context;return [{url:'https://official.example/fixture',content:'Synthetic source requires independent review.',effectiveOn:'2020-01-01',endsOn:'2099-12-31',coverage:'Fixture only',exclusions:'No validated tax treatment'}];}}});
 ...
 const rule={...source.context,version:'tax-preparation-v1',validation:'validated',simulated:false,active:true,treatment:'zero_rate',behavior:'exclusive',ratePercent:'0',effectiveOn:'2020-01-01',endsOn:'2099-12-31',registration:'registered',collectionBasis:'Synthetic local coverage fixture only'};
 const input={candidates:[persisted],registry:[],manifest:{version:'tax-coverage-review-v1',mode:'synthetic_fixture',review:{authorId:'local-fixture-author',reviewerId:'local-fixture-reviewer',reviewedOn:new Date().toISOString().slice(0,10),verdict:'approved',evidenceDigest:hash('Synthetic local review; no real independent tax validation')},entries:[{ruleKey:'connected-fixture-only',expectedRevision:0,expectedDigest:'none',candidateDigest:persisted.digest,sourceDigest:source.contentDigest,reviewedRuleDigest:hash(stableStringify(rule)),rule}]}};
