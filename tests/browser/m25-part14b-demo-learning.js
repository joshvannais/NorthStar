...
(async () => {
  try {
    ...
    for (const layout of layouts) {
      ...
      await page.evaluate(() => scrollTo(0, document.body.scrollHeight)); await page.reload({ waitUntil: 'networkidle' }); await page.locator('#learningMain[aria-busy="false"]').waitFor(); await page.getByText('No completed-job suggestion has been adopted for this service.').waitFor(); await page.evaluate(() => new Promise(resolve => requestAnimationFrame(() => requestAnimationFrame(resolve)))); assert.equal(await page.evaluate(() => scrollY), 0);
