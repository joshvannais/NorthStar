...
A mounted PostgreSQL 17 journey now preserves one populated provider-neutral labor source through repeated production database-pool startup. Migration 134 makes PostgreSQL 17 and 18 migration-ledger catalog representations restart-safe and retires labor-source permission, import and connection retries after their exact permission period changes. The first restart leaves all 132 migration filenames, checksums and applied times unchanged, replays the exact current permission, import and connection requests, and resumes a paused connection through its current revision. A current correction replaces the earlier current source version without rewriting history. Explicit revocation hides the source and retires old requests. A later permission period may expose retained staged source evidence under the accepted import authority, while old requests, reviewed matches and downstream learning remain retired until separately renewed.

Retention tombstones both eligible staged records under its exact current policy. Deletion then processes 101 current records in a bounded 100-record page, persists its checkpoint through another database-pool restart, resumes from the exact cursor and finishes once. The completed cleanup request remains exactly replayable. Existing accepted all-source lifecycle suites retain broader source-class coverage. Bounds, performance, concurrency, failure recovery and operational observability remain Slice D. Mission-wide rendered accessibility and responsive review remain Slice E; no rendered path changes in Slice C.
