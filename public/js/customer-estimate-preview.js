(function (root, factory) {
  ...
  function deliveryRoute(review){var id=encodeURIComponent(review.pins.estimateId);return(review.simulated?'/api/demo/command-center/estimates/':'/api/v1/canonical/estimates/')+id+'/customer-estimate-links';}
  ...
  function mount(review, host) {
    ...
    function renderDelivery(data,current,newPath){
      ...
      var share=append(bar,'button','btn btn-primary',active?'Create New Link':'Share Customer Estimate');share.type='button';share.onclick=function(){createLink(current);};
      if(newPath){var link=location.origin+newPath,copy=append(deliveryHost,'div','customer-estimate-copy');var field=append(copy,'input');field.readOnly=true;field.value=link;field.setAttribute('aria-label','Customer estimate link');var copyButton=append(copy,'button','btn btn-secondary','Copy Link');copyButton.type='button';copyButton.onclick=function(){navigator.clipboard.writeText(link).then(function(){status.textContent='Customer estimate link copied.';}).catch(function(){field.select();document.execCommand('copy');status.textContent='Customer estimate link copied.';});};var openLink=append(copy,'a','btn btn-secondary','Open');openLink.href=link;openLink.target='_blank';openLink.rel='noopener noreferrer';}
      if(active){var summary=append(deliveryHost,'p','customer-estimate-delivery-summary',active.status==='accepted'?'Accepted by '+active.accepted.customerName+' · '+new Date(active.accepted.createdAt).toLocaleString():'Expires '+new Date(active.expiresAt).toLocaleDateString());var revoke=append(deliveryHost,'button','btn btn-secondary','Revoke Link');revoke.type='button';revoke.onclick=function(){revoke.disabled=true;request(deliveryRoute(review)+'/'+encodeURIComponent(active.id)+'/revoke',{method:'POST',headers:{'Content-Type':'application/json','Idempotency-Key':crypto.randomUUID()},body:'{}'}).then(function(){status.textContent='Customer access revoked.';return loadDelivery(current);}).catch(function(error){status.textContent=error.message||'The link could not be revoked.';}).finally(function(){revoke.disabled=false;});};}
      var questions=links.reduce(function(all,link){return all.concat(link.questions||[]);},[]);if(questions.length){var details=append(deliveryHost,'details','customer-estimate-questions');append(details,'summary','',questions.length+' customer '+(questions.length===1?'question':'questions'));questions.forEach(function(question){var item=append(details,'article','customer-estimate-question');append(item,'strong','',question.customerName+' · '+question.replyTo);append(item,'p','',question.message);append(item,'small','',new Date(question.createdAt).toLocaleString());});}
    }
    function request(url,options){options=options||{};if(review.simulated&&options.method==='POST'&&url.indexOf('/customer-estimate-links')!==-1){options.headers=Object.assign({},options.headers,{'X-NorthStar-Demo-Intent':'customer-estimate-delivery'});}return window.NorthStarAccountSession.fetch(url,options).then(responseData);}
    function loadDelivery(current,newPath){if(!current)return Promise.resolve();return request(deliveryRoute(review),{cache:'no-store'}).then(function(data){renderDelivery(data,current,newPath);}).catch(function(error){status.textContent=error.message||'Customer delivery status is unavailable.';});}
    function createLink(current){var key=crypto.randomUUID(),days=review.simulated?1:14;status.textContent='Creating a secure customer link…';request(deliveryRoute(review),{method:'POST',headers:{'Content-Type':'application/json','Idempotency-Key':key},body:JSON.stringify({versionId:current.id,expiresInDays:days,confirmed:true,confirmationVersion:'customer-estimate-delivery-v1'})}).then(function(data){status.textContent='Secure customer link ready.';return loadDelivery(current,data.urlPath);}).catch(function(error){status.textContent=error.message||'The customer link could not be created.';});}
    function renderHistory(data){historyHost.replaceChildren();deliveryHost.replaceChildren();var current=data&&data.current,match=!!(current&&current.approvalPin&&commercial.binding&&current.approvalPin.id===commercial.binding.id);if(issue)issue.hidden=false;if(current){heading.textContent=match?'Customer estimate issued':'New estimate revision ready';description.textContent=match?'View it from the estimate action above, or create a secure customer link below.':'An earlier version remains saved. Review and issue the newly approved revision.';if(match){externalState('Issued');external.onclick=function(){showEstimate(external,current.document,'Viewing issued version '+current.revision+'.');};if(issue)issue.hidden=!!external;loadDelivery(current);}else issue.textContent='Issue New Version';var details=append(historyHost,'details','customer-estimate-history');append(details,'summary','',data.total+' issued '+(data.total===1?'version':'versions'));(data.history||[]).forEach(function(version){var row=append(details,'div','customer-estimate-history-row');var view=append(row,'button','btn btn-secondary','Version '+version.revision);view.type='button';view.onclick=function(){showEstimate(view,version.document,'Viewing issued version '+version.revision+'.');};append(row,'span','',new Date(version.createdAt).toLocaleString());});}else status.textContent='Ready to preview or issue.';if(issue&&!match)issue.onclick=beginIssue;}
    function beginIssue(){historyHost.querySelector('.customer-estimate-issue-form')?.remove();var form=append(historyHost,'form','customer-estimate-issue-form');append(form,'h5','','Issue this estimate');append(form,'p','','This saves the exact customer version shown in the preview.');var confirmation=append(form,'label','drawer-decision-confirmation');var checkbox=append(confirmation,'input');checkbox.type='checkbox';checkbox.required=true;append(confirmation,'span','','I reviewed the customer-facing scope, price, tax, total and payment schedule.');var formActions=append(form,'div','customer-estimate-launch-actions'),confirm=append(formActions,'button','btn btn-primary','Confirm Issue');confirm.type='submit';var cancel=append(formActions,'button','btn btn-secondary','Cancel');cancel.type='button';cancel.onclick=function(){form.remove();issue.focus();};form.onsubmit=function(event){event.preventDefault();if(!form.reportValidity())return;var headers={'Content-Type':'application/json','Idempotency-Key':crypto.randomUUID()};if(review.simulated){headers['X-NorthStar-Demo-Revision']=String(review.demoWorkspaceRevision);headers['X-NorthStar-Demo-Intent']='customer-estimate-issue';}confirm.disabled=true;issue.disabled=true;preview.disabled=true;status.textContent='Issuing the customer estimate…';request(versionRoute(review),{method:'POST',headers:headers,body:JSON.stringify({reason:'Owner confirmed the customer-facing estimate',confirmed:true,confirmationVersion:'customer-estimate-issue-v1'})}).then(function(data){form.remove();if(data&&data.receipt){renderHistory({current:data.receipt,history:[data.receipt],total:1});showEstimate(issue,data.receipt.document,'Issued version '+data.receipt.revision+' is saved.');loadHistory();}else return loadHistory();}).catch(function(error){status.textContent=error&&error.message||'The issue result is unconfirmed. Refresh to check issued versions before retrying.';}).finally(function(){confirm.disabled=false;issue.disabled=false;preview.disabled=false;});};checkbox.focus();}
  ...
  return { money:money, presentationRows:presentationRows, pdfDefinition:pdfDefinition, estimateCard:estimateCard, pdfBlob:pdfBlob, canvasBlob:canvasBlob, save:save, mount:mount };
}));
