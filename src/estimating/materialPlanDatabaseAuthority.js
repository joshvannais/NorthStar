...
async function grantAndVerify(client,runtimeRole){
 ...
 const functions=(await client.query("SELECT oid,proname,oid::regprocedure::text identity FROM pg_proc WHERE pronamespace='public'::regnamespace AND (proname LIKE 'canonical_material_plan_%' OR proname LIKE 'canonical_material_source_%')")).rows;
