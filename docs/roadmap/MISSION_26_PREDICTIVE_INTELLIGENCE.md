...
The guarded availability snapshot also retains each active worker's latest reviewed Mission 23 work-profile certification record and expiry evidence under the same tenant snapshot. Those records do not establish job-specific qualification, current validity throughout the horizon, or an available worker; Part 5B's qualification and resource gates remain open.
