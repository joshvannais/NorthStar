...
function buildRevisionReview(item, selection, options = {}) {
  ...
  if (![...adoption.VERSIONS,composition.VERSION].includes(selected.calculationVersion) || sha256(selected.originalSourcePins) !== sha256(original.pins) ||
  ...
  const composed=selected.calculationVersion===composition.VERSION;
  if(composed&&sha256(selected.componentManifest)!==sha256(composition.manifest({material:selected.materialPlan,labor:selected.laborPlan})))throw Object.assign(new Error('Saved cost sources could not be verified. Refresh and try again.'),{status:503});
  const costs = composed?composition.calculate(item,{material:selected.materialPlan,labor:selected.laborPlan}):adoption.calculate(item, selected.materialPlan, selected.calculationVersion);
...
function demoAdopt(item, revisions, decisions, plan, raw, key, now, laborPlan=null) {
