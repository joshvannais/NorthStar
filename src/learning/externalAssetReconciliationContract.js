...
function fail() { const error = new Error('Vehicle and equipment reference match details are invalid.'); error.code = 'M25_ASSET_MATCH_INPUT_INVALID'; error.status = 400; throw error; }
...
function normalizeMatch(source, body) {
  ...
  if (!exact(body) || !['link', 'unlink'].includes(body.action) || !['job', 'vehicle', 'equipment'].includes(body.referenceKind) ||
      ...
      body.confirmationVersion !== 'm25-external-asset-reference-match-v1' || !text(body.reason, 2000)) fail();
