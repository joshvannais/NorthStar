-- Mission 25 Part 9E: travel-source adapter and cleanup operations.
...
CREATE TABLE public.canonical_external_travel_adapter_revisions (
 ...
 FOREIGN KEY(organization_id,source_key,previous_id) REFERENCES public.canonical_external_travel_adapter_revisions(organization_id,source_key,id) ON DELETE RESTRICT,
...
CREATE TABLE public.canonical_external_travel_retention_revisions (
 id UUID PRIMARY KEY DEFAULT gen_random_uuid(), organization_id UUID NOT NULL REFERENCES public.organizations(id) ON DELETE RESTRICT,
 source_key TEXT NOT NULL CHECK(source_key~'^[a-z0-9][a-z0-9._-]{1,63}$'), revision BIGINT NOT NULL CHECK(revision BETWEEN 1 AND 10000),
 ...
 canonical_digest CHAR(64) NOT NULL CHECK(canonical_digest~'^[0-9a-f]{64}$'), created_at TIMESTAMPTZ NOT NULL DEFAULT transaction_timestamp(),
 UNIQUE(organization_id,source_key,revision), UNIQUE(organization_id,actor_user_id,request_key_hash), UNIQUE(organization_id,source_key,id),
 FOREIGN KEY(organization_id,source_key,previous_id) REFERENCES public.canonical_external_travel_retention_revisions(organization_id,source_key,id) ON DELETE RESTRICT,
 FOREIGN KEY(organization_id,membership_id) REFERENCES public.organization_memberships(organization_id,id) ON DELETE RESTRICT,
 FOREIGN KEY(organization_id,actor_user_id,auth_session_id) REFERENCES public.auth_sessions(organization_id,user_id,id) ON DELETE RESTRICT,
...
CREATE TABLE public.canonical_external_travel_deletion_revisions (
 id UUID PRIMARY KEY DEFAULT gen_random_uuid(), organization_id UUID NOT NULL REFERENCES public.organizations(id) ON DELETE RESTRICT,
 source_key TEXT NOT NULL CHECK(source_key~'^[a-z0-9][a-z0-9._-]{1,63}$'), revision BIGINT NOT NULL CHECK(revision BETWEEN 1 AND 10000),
 ...
 canonical_digest CHAR(64) NOT NULL CHECK(canonical_digest~'^[0-9a-f]{64}$'), created_at TIMESTAMPTZ NOT NULL DEFAULT transaction_timestamp(),
 UNIQUE(organization_id,source_key,revision), UNIQUE(organization_id,actor_user_id,request_key_hash), UNIQUE(organization_id,source_key,id),
 FOREIGN KEY(organization_id,source_key,previous_id) REFERENCES public.canonical_external_travel_deletion_revisions(organization_id,source_key,id) ON DELETE RESTRICT,
 FOREIGN KEY(organization_id,membership_id) REFERENCES public.organization_memberships(organization_id,id) ON DELETE RESTRICT,
 FOREIGN KEY(organization_id,actor_user_id,auth_session_id) REFERENCES public.auth_sessions(organization_id,user_id,id) ON DELETE RESTRICT,
 CHECK((revision=1 AND previous_id IS NULL) OR (revision>1 AND previous_id IS NOT NULL))
);

CREATE TABLE public.canonical_external_travel_cleanup_runs (
 id UUID PRIMARY KEY DEFAULT gen_random_uuid(), organization_id UUID NOT NULL REFERENCES public.organizations(id) ON DELETE RESTRICT,
 ...
 FOREIGN KEY(organization_id,source_key,previous_run_id) REFERENCES public.canonical_external_travel_cleanup_runs(organization_id,source_key,id) ON DELETE RESTRICT,
 FOREIGN KEY(organization_id,membership_id) REFERENCES public.organization_memberships(organization_id,id) ON DELETE RESTRICT,
 FOREIGN KEY(organization_id,actor_user_id,auth_session_id) REFERENCES public.auth_sessions(organization_id,user_id,id) ON DELETE RESTRICT,
...
ALTER TABLE public.canonical_external_travel_import_records ADD COLUMN cleanup_run_id UUID;
ALTER TABLE public.canonical_external_travel_import_records ADD CONSTRAINT canonical_external_travel_import_record_cleanup_fk
 FOREIGN KEY(organization_id,source_key,cleanup_run_id) REFERENCES public.canonical_external_travel_cleanup_runs(organization_id,source_key,id) ON DELETE RESTRICT DEFERRABLE INITIALLY DEFERRED;

CREATE TRIGGER canonical_external_travel_adapter_revisions_immutable BEFORE UPDATE OR DELETE OR TRUNCATE ON public.canonical_external_travel_adapter_revisions FOR EACH STATEMENT EXECUTE FUNCTION public.canonical_learning_immutable();
CREATE TRIGGER canonical_external_travel_retention_revisions_immutable BEFORE UPDATE OR DELETE OR TRUNCATE ON public.canonical_external_travel_retention_revisions FOR EACH STATEMENT EXECUTE FUNCTION public.canonical_learning_immutable();
CREATE TRIGGER canonical_external_travel_deletion_revisions_immutable BEFORE UPDATE OR DELETE OR TRUNCATE ON public.canonical_external_travel_deletion_revisions FOR EACH STATEMENT EXECUTE FUNCTION public.canonical_learning_immutable();
CREATE TRIGGER canonical_external_travel_cleanup_runs_immutable BEFORE UPDATE OR DELETE OR TRUNCATE ON public.canonical_external_travel_cleanup_runs FOR EACH STATEMENT EXECUTE FUNCTION public.canonical_learning_immutable();

CREATE FUNCTION public.canonical_external_travel_operation_projection(kind TEXT,value JSONB)
...
DECLARE authority JSONB; current_value JSONB; replay_value JSONB; previous UUID; revision_value BIGINT; key_hash TEXT; request_hash TEXT; digest_value TEXT; inserted JSONB;
 consent_row public.canonical_external_travel_import_consents%ROWTYPE; revoke_digest TEXT;
BEGIN
 ...
 IF kind='adapter' THEN SELECT to_jsonb(item) INTO replay_value FROM (SELECT * FROM public.canonical_external_travel_adapter_revisions WHERE organization_id=org AND actor_user_id=actor AND request_key_hash=key_hash) item;
 ELSIF kind='retention' THEN SELECT to_jsonb(item) INTO replay_value FROM (SELECT * FROM public.canonical_external_travel_retention_revisions WHERE organization_id=org AND actor_user_id=actor AND request_key_hash=key_hash) item;
 ELSE SELECT to_jsonb(item) INTO replay_value FROM (SELECT * FROM public.canonical_external_travel_deletion_revisions WHERE organization_id=org AND actor_user_id=actor AND request_key_hash=key_hash) item; END IF;
 IF replay_value IS NOT NULL THEN IF rtrim(replay_value->>'request_digest')<>request_hash THEN RAISE EXCEPTION 'Source operation key conflict' USING ERRCODE='23505'; END IF; RETURN jsonb_build_object(kind,public.canonical_external_travel_operation_projection(kind,replay_value),'replayed',TRUE); END IF;
 IF kind='adapter' THEN SELECT to_jsonb(item) INTO current_value FROM (SELECT * FROM public.canonical_external_travel_adapter_revisions WHERE organization_id=org AND source_key=source_value ORDER BY revision DESC LIMIT 1 FOR UPDATE) item;
 ELSIF kind='retention' THEN SELECT to_jsonb(item) INTO current_value FROM (SELECT * FROM public.canonical_external_travel_retention_revisions WHERE organization_id=org AND source_key=source_value ORDER BY revision DESC LIMIT 1 FOR UPDATE) item;
 ELSE SELECT to_jsonb(item) INTO current_value FROM (SELECT * FROM public.canonical_external_travel_deletion_revisions WHERE organization_id=org AND source_key=source_value ORDER BY revision DESC LIMIT 1 FOR UPDATE) item; END IF;
 ...
 ELSE
  ...
  IF body->>'action'='request' THEN
   SELECT * INTO consent_row FROM public.canonical_external_travel_import_consents WHERE organization_id=org AND source_key=source_value ORDER BY revision DESC LIMIT 1 FOR UPDATE;
   IF consent_row.id IS NOT NULL AND consent_row.action='grant' THEN
    revoke_digest:=public.canonical_completion_digest(jsonb_build_object('organizationId',org,'sourceKey',source_value,'revision',consent_row.revision+1,'previousId',consent_row.id,'action','revoke','actorUserId',actor,'membershipId',(authority->>'membershipId')::uuid,'authSessionId',session_value,'sourceScope','["external_travel_actual_normalized_v1"]'::jsonb,'consentVersion','m25-external-travel-import-consent-v1','reason','Source deletion requested; new imports and derived reads are blocked.','requestDigest',request_hash));
    INSERT INTO public.canonical_external_travel_import_consents(organization_id,source_key,revision,previous_id,action,actor_user_id,membership_id,auth_session_id,source_scope,consent_version,reason,request_key_hash,request_digest,canonical_digest)
    VALUES(org,source_value,consent_row.revision+1,consent_row.id,'revoke',actor,(authority->>'membershipId')::uuid,session_value,'["external_travel_actual_normalized_v1"]'::jsonb,'m25-external-travel-import-consent-v1','Source deletion requested; new imports and derived reads are blocked.',encode(sha256(convert_to(key_value||':consent-revoke','UTF8')),'hex'),request_hash,revoke_digest);
   END IF;
  END IF;
 END IF;
 RETURN jsonb_build_object(kind,public.canonical_external_travel_operation_projection(kind,inserted),'replayed',FALSE);
END $$;

CREATE FUNCTION public.canonical_external_travel_adapter_mutate(org UUID,actor UUID,role_value TEXT,session_value UUID,csrf TEXT,key_value TEXT,source_value TEXT,body JSONB) RETURNS JSONB LANGUAGE sql SECURITY DEFINER SET search_path=pg_catalog,public,pg_temp AS $$ SELECT public.canonical_external_travel_operation_mutate(org,actor,role_value,session_value,csrf,key_value,source_value,'adapter',body) $$;
CREATE FUNCTION public.canonical_external_travel_retention_mutate(org UUID,actor UUID,role_value TEXT,session_value UUID,csrf TEXT,key_value TEXT,source_value TEXT,body JSONB) RETURNS JSONB LANGUAGE sql SECURITY DEFINER SET search_path=pg_catalog,public,pg_temp AS $$ SELECT public.canonical_external_travel_operation_mutate(org,actor,role_value,session_value,csrf,key_value,source_value,'retention',body) $$;
CREATE FUNCTION public.canonical_external_travel_deletion_mutate(org UUID,actor UUID,role_value TEXT,session_value UUID,csrf TEXT,key_value TEXT,source_value TEXT,body JSONB) RETURNS JSONB LANGUAGE sql SECURITY DEFINER SET search_path=pg_catalog,public,pg_temp AS $$ SELECT public.canonical_external_travel_operation_mutate(org,actor,role_value,session_value,csrf,key_value,source_value,'deletion',body) $$;

CREATE FUNCTION public.canonical_external_travel_operations_read(org UUID,actor UUID,role_value TEXT,session_value UUID,source_value TEXT)
...
BEGIN
 ...
 SELECT public.canonical_external_travel_operation_projection('adapter',to_jsonb(item)) INTO adapter FROM (SELECT * FROM public.canonical_external_travel_adapter_revisions WHERE organization_id=org AND source_key=source_value ORDER BY revision DESC LIMIT 1) item;
 SELECT public.canonical_external_travel_operation_projection('retention',to_jsonb(item)) INTO retention FROM (SELECT * FROM public.canonical_external_travel_retention_revisions WHERE organization_id=org AND source_key=source_value ORDER BY revision DESC LIMIT 1) item;
 SELECT public.canonical_external_travel_operation_projection('deletion',to_jsonb(item)) INTO deletion FROM (SELECT * FROM public.canonical_external_travel_deletion_revisions WHERE organization_id=org AND source_key=source_value ORDER BY revision DESC LIMIT 1) item;
 WITH imported AS (
  SELECT DISTINCT ON (mode) mode,sequence,cursor_after,complete,created_at FROM public.canonical_external_travel_import_runs WHERE organization_id=org AND source_key=source_value ORDER BY mode,sequence DESC
 ), cleanup AS (
  SELECT DISTINCT ON (operation) operation||'_cleanup' mode,sequence,cursor_after,complete,created_at
  FROM public.canonical_external_travel_cleanup_runs
 ...
 WITH current_records AS (SELECT DISTINCT ON (external_record_id) * FROM public.canonical_external_travel_import_records WHERE organization_id=org AND source_key=source_value ORDER BY external_record_id,revision DESC) SELECT count(*) INTO active_total FROM current_records WHERE state='active';
...
CREATE FUNCTION public.canonical_external_travel_cleanup_execute(org UUID,actor UUID,role_value TEXT,session_value UUID,csrf TEXT,key_value TEXT,source_value TEXT,body JSONB)
RETURNS JSONB LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public,pg_temp AS $$
DECLARE authority JSONB; authority_value JSONB; previous_run public.canonical_external_travel_cleanup_runs%ROWTYPE; replay_run public.canonical_external_travel_cleanup_runs%ROWTYPE;
 key_hash TEXT; request_hash TEXT; run_id UUID:=gen_random_uuid(); digest_value TEXT; sequence_value BIGINT; selected_count INTEGER; has_more BOOLEAN; next_cursor TEXT; record_row public.canonical_external_travel_import_records%ROWTYPE; record_digest TEXT; operation_value TEXT;
BEGIN
 IF current_setting('transaction_isolation')<>'serializable' THEN RAISE EXCEPTION 'Serializable required' USING ERRCODE='25001'; END IF;
 ...
 FOR record_row IN SELECT * FROM selected_cleanup_records ORDER BY external_record_id LOOP
  record_digest:=public.canonical_completion_digest(jsonb_build_object('organizationId',org,'sourceKey',source_value,
   ...
   'jobReference',NULL,'vehicleReference',NULL,'routeStartedAt',NULL,'routeEndedAt',NULL,'timeZone',NULL,
   'distance',NULL,'fuel',NULL,'evidenceClass',NULL,'sourceUpdatedAt',transaction_timestamp(),'cleanupRunId',run_id));
  INSERT INTO public.canonical_external_travel_import_records(organization_id,source_key,external_record_id,revision,
   previous_id,external_version,state,job_reference,vehicle_reference,route_started_at,route_ended_at,time_zone,
   distance_value,distance_unit,distance_basis,fuel_quantity,fuel_unit,fuel_cost_amount,fuel_currency,fuel_basis,
   evidence_class,source_updated_at,source_digest,import_run_id,cleanup_run_id)
...
BEGIN SELECT action INTO action_value FROM public.canonical_external_travel_deletion_revisions WHERE organization_id=NEW.organization_id AND source_key=NEW.source_key ORDER BY revision DESC LIMIT 1; IF action_value='request' THEN RAISE EXCEPTION 'Source deletion blocks imports' USING ERRCODE='40001'; END IF; RETURN NEW; END $$;
CREATE TRIGGER canonical_external_travel_import_deletion_guard BEFORE INSERT ON public.canonical_external_travel_import_runs FOR EACH ROW EXECUTE FUNCTION public.canonical_external_travel_import_deletion_guard();
...
REVOKE ALL ON FUNCTION public.canonical_external_travel_deletion_mutate(UUID,UUID,TEXT,UUID,TEXT,TEXT,TEXT,JSONB) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.canonical_external_travel_operations_read(UUID,UUID,TEXT,UUID,TEXT) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.canonical_external_travel_cleanup_execute(UUID,UUID,TEXT,UUID,TEXT,TEXT,TEXT,JSONB) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.canonical_external_travel_import_deletion_guard() FROM PUBLIC;
