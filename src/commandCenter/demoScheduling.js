...
function validateState(state){
 ...
 for(const item of value.history.slice().reverse()){
  ...
  if(!authority||authority.appointmentId!==item.appointmentId||authority.revision!==expected||!['unassigned','assigned'].includes(authority.targetState)||!['unscheduled','scheduled'].includes(authority.scheduleState)||authority.dispatchState!=='not_dispatched'||(authority.scheduleState==='scheduled'?(!Number.isFinite(Date.parse(authority.scheduledStart))||!Number.isFinite(Date.parse(authority.scheduledEnd))||Date.parse(authority.scheduledEnd)<=Date.parse(authority.scheduledStart)):(authority.scheduledStart!==null||authority.scheduledEnd!==null))||(authority.targetState==='unassigned'?(authority.workforceProfileId!==null||authority.workforceCrewId!==null):Boolean(authority.workforceProfileId)===Boolean(authority.workforceCrewId)))fail(503,'DEMO_SCHEDULE_STATE_INVALID','Saved demo schedules could not be read.');
...
function projection(workspace,state){
 ...
 const records=workspace.graphs.map(graph=>{
  ...
  return {appointmentId:authority.appointmentId,graphId:graph.ids.graph,customer:{id:graph.ids.customer,name:graph.customer.name},work:{opportunityId:authority.opportunityId,title:graph.work.title||graph.polaris.snapshot.service.label,serviceType:graph.polaris.snapshot.service.key,appointmentStatus:authority.appointmentStatus},authority,conflict:{...conflict,digest:sha256(conflict),persisted:false,grantsMutation:false},flags:classifyRecord({appointment:{scheduleAuthority:authority}},conflict,new Date()),allowedActions:policy.mutationsEnabled&&!closed?[authority.scheduleState==='scheduled'?'reschedule':'schedule',...(workforce.read(state)?(authority.targetState==='unassigned'?['assign']:['reassign','unassign']):[])]:[],demoWorkspaceRevision:workspace.integrity.revision};
 });
...
function preview(workspace,state,input,now){
 ...
 if(!['schedule','reschedule','assign','reassign','unassign'].includes(normalized.action)||normalized.appointmentStatus!==authority.appointmentStatus||['completed','cancelled'].includes(authority.appointmentStatus))fail(400,'DEMO_SCHEDULE_ACTION_UNSUPPORTED','Choose a supported change for this appointment.');
