...
(async()=>{let f;try{f=await createEstimateReviewFixture();const actors={organizationId:f.org,owner:f.actors.owner.actorUserId,admin:f.actors.admin.actorUserId};const {Pool}=require('pg');const single=new Pool({connectionString:process.env.DATABASE_URL,max:1});f.bindingPool=single;const sync=new SyncRepository(single);const configured=await sync.configureTarget(targetInput(actors,{maximumBytes:8192}));const completed=await completeKnowledge(knowledge,f.runtimePool,actors,'connected');
...
const worker=new SyncWorker({repository:sync,transports,batchSize:3});const first=await worker.drainOnce();result.first=first;assert.equal(first.succeeded,1);assert.equal(updates,1);assert.ok(remote.general_prompt.includes('Company connected'));assert.ok(!remote.general_prompt.includes('private-tax-connected'));assert.ok(!remote.general_prompt.includes('internalCost'));assert.ok(!remote.general_prompt.includes('canonicalPricing'));assert.ok(!remote.general_prompt.includes('private-connected@example.test'));
...
result.cases.push('actual immutable tombstone publication replaces only managed block and advances observed state');result.transport={updates,gets};result.states=(await f.ownerPool.query('SELECT status,desired_sequence FROM canonical_knowledge_sync_states WHERE target_id=$1',[configured.target.id])).rows;const beforeGets=gets;rawBindings=JSON.stringify([{...binding,basePromptDigest:'f'.repeat(64)}]);await assert.rejects(()=>transports.get(binding.providerKey).applyProjection({organizationId:f.org,targetId:binding.targetId,targetRevision:binding.targetRevision}),/binding/);assert.equal(gets,beforeGets);result.cases.push('Size-one pool production binding factory rechecks actual canonical target and integration; changed registry rejects before remote read');result.pass=true;
