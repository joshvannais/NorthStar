-- Mission 25 Part 10F: tenant-private multi-job imported vehicle and equipment calibration proposals.
-- Robust summaries remain advisory and never mutate estimates, plans, rates, schedules or policy.

CREATE TABLE public.canonical_external_asset_calibration_consents (
 id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
 organization_id UUID NOT NULL REFERENCES public.organizations(id) ON DELETE RESTRICT,
 source_key TEXT NOT NULL CHECK(source_key~'^[a-z0-9][a-z0-9._-]{1,63}$'),
 purpose TEXT NOT NULL CHECK(purpose='imported_asset_calibration_v1'),
 ...
 source_scope JSONB NOT NULL CHECK(source_scope='["canonical_external_asset_outcome_observations"]'::jsonb),
 consent_version TEXT NOT NULL CHECK(consent_version='m25-imported-asset-calibration-consent-v1'),
 ...
 FOREIGN KEY(organization_id,source_key,previous_id)
  REFERENCES public.canonical_external_asset_calibration_consents(organization_id,source_key,id) ON DELETE RESTRICT,
 FOREIGN KEY(organization_id,source_key,outcome_consent_id)
  REFERENCES public.canonical_external_asset_outcome_consents(organization_id,source_key,id) ON DELETE RESTRICT,
...
CREATE FUNCTION public.canonical_imported_asset_calibration_metrics_valid(value JSONB)
RETURNS BOOLEAN LANGUAGE sql IMMUTABLE SECURITY DEFINER SET search_path=pg_catalog,public,pg_temp AS $$
 SELECT public.canonical_field_evidence_object_keys_exact(value,ARRAY['utilization','operatingCost'])
  AND public.canonical_imported_asset_calibration_metric_valid(value->'utilization')
  AND public.canonical_imported_asset_calibration_metric_valid(value->'operatingCost')
$$;
...
CREATE TABLE public.canonical_external_asset_calibration_proposals (
 ...
 confirmation_version TEXT NOT NULL CHECK(confirmation_version='m25-imported-asset-calibration-proposal-v1'),
 calculation_version TEXT NOT NULL CHECK(calculation_version='m25-imported-asset-median-calibration-v1'),
 request_key_hash CHAR(64) NOT NULL CHECK(request_key_hash~'^[0-9a-f]{64}$'),
 request_digest CHAR(64) NOT NULL CHECK(request_digest~'^[0-9a-f]{64}$'),
 canonical_digest CHAR(64) NOT NULL CHECK(canonical_digest~'^[0-9a-f]{64}$'),
 created_at TIMESTAMPTZ NOT NULL DEFAULT transaction_timestamp(),
 ...
 UNIQUE(organization_id,source_key,id),
 FOREIGN KEY(organization_id,source_key,previous_id)
  REFERENCES public.canonical_external_asset_calibration_proposals(organization_id,source_key,id) ON DELETE RESTRICT,
 ...
 CHECK(public.canonical_imported_asset_calibration_metrics_valid(metrics)),
 CHECK(eligible_metric_count=public.canonical_imported_asset_calibration_eligible_count(metrics))
);

CREATE TRIGGER canonical_external_asset_calibration_consents_immutable BEFORE UPDATE OR DELETE OR TRUNCATE
 ON public.canonical_external_asset_calibration_consents FOR EACH STATEMENT EXECUTE FUNCTION public.canonical_learning_immutable();
CREATE TRIGGER canonical_external_asset_calibration_proposals_immutable BEFORE UPDATE OR DELETE OR TRUNCATE
 ON public.canonical_external_asset_calibration_proposals FOR EACH STATEMENT EXECUTE FUNCTION public.canonical_learning_immutable();

CREATE FUNCTION public.canonical_imported_asset_calibration_consent_projection(
 value public.canonical_external_asset_calibration_consents)
...
CREATE FUNCTION public.canonical_imported_asset_calibration_consent_read(org UUID,actor UUID,role_value TEXT,
 session_value UUID,source_value TEXT)
RETURNS JSONB LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path=pg_catalog,public,pg_temp AS $$
DECLARE source_consent public.canonical_external_asset_import_consents%ROWTYPE;
 outcome_consent public.canonical_external_asset_outcome_consents%ROWTYPE;
 current_row public.canonical_external_asset_calibration_consents%ROWTYPE; history JSONB; total BIGINT; active_value BOOLEAN;
BEGIN
 ...
 SELECT * INTO outcome_consent FROM public.canonical_external_asset_outcome_consents WHERE organization_id=org
  AND source_key=source_value AND purpose='imported_asset_utilization_cost_variance_v1' ORDER BY revision DESC LIMIT 1;
 SELECT * INTO source_consent FROM public.canonical_external_asset_import_consents WHERE organization_id=org
  AND source_key=source_value ORDER BY revision DESC LIMIT 1;
 SELECT * INTO current_row FROM public.canonical_external_asset_calibration_consents WHERE organization_id=org
  AND source_key=source_value AND purpose='imported_asset_calibration_v1' ORDER BY revision DESC LIMIT 1;
 ...
 SELECT count(*) INTO total FROM public.canonical_external_asset_calibration_consents WHERE organization_id=org
  AND source_key=source_value AND purpose='imported_asset_calibration_v1';
 SELECT COALESCE(jsonb_agg(public.canonical_imported_asset_calibration_consent_projection(item) ORDER BY revision DESC),'[]'::jsonb)
  INTO history FROM (SELECT * FROM public.canonical_external_asset_calibration_consents WHERE organization_id=org
   AND source_key=source_value AND purpose='imported_asset_calibration_v1' ORDER BY revision DESC LIMIT 20) item;
 RETURN jsonb_build_object('sourceKey',source_value,'active',active_value,'current',CASE WHEN current_row.id IS NULL THEN NULL
  ELSE public.canonical_imported_asset_calibration_consent_projection(current_row) END,'history',history,'total',total,
  'truncated',total>20,'blockedReason',CASE WHEN active_value THEN NULL WHEN outcome_consent.id IS NULL OR outcome_consent.action<>'grant'
   THEN 'Vehicle and equipment outcome consent is not active.' WHEN current_row.id IS NOT NULL AND current_row.action='grant'
   THEN 'Calibration consent must be renewed for the current outcome consent.' ELSE 'Vehicle and equipment calibration consent is not active.' END);
END $$;

CREATE FUNCTION public.canonical_imported_asset_calibration_consent_mutate(org UUID,actor UUID,role_value TEXT,
 session_value UUID,csrf TEXT,key_value TEXT,source_value TEXT,body JSONB)
RETURNS JSONB LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public,pg_temp AS $$
DECLARE authority JSONB; source_consent public.canonical_external_asset_import_consents%ROWTYPE;
 outcome_consent public.canonical_external_asset_outcome_consents%ROWTYPE;
 current_row public.canonical_external_asset_calibration_consents%ROWTYPE;
 replay_row public.canonical_external_asset_calibration_consents%ROWTYPE;
 inserted public.canonical_external_asset_calibration_consents%ROWTYPE;
 key_hash TEXT; request_hash TEXT; digest_value TEXT; next_revision BIGINT; currently_active BOOLEAN;
 scope_value JSONB:='["canonical_external_asset_outcome_observations"]'::jsonb;
BEGIN
 ...
 IF source_value IS NULL OR source_value!~'^[a-z0-9][a-z0-9._-]{1,63}$' OR key_value IS NULL
  ...
  OR body->>'confirmationVersion' IS DISTINCT FROM 'm25-imported-asset-calibration-consent-v1'
 ...
 PERFORM pg_advisory_xact_lock(hashtextextended(org::text||':imported-asset-calibration-consent:'||source_value,0));
 SELECT * INTO replay_row FROM public.canonical_external_asset_calibration_consents WHERE organization_id=org
 ...
 IF FOUND THEN
  ...
  RETURN jsonb_build_object('consent',public.canonical_imported_asset_calibration_consent_projection(replay_row),'replayed',TRUE);
 END IF;
 SELECT * INTO outcome_consent FROM public.canonical_external_asset_outcome_consents WHERE organization_id=org
  AND source_key=source_value AND purpose='imported_asset_utilization_cost_variance_v1' ORDER BY revision DESC LIMIT 1 FOR SHARE;
 SELECT * INTO source_consent FROM public.canonical_external_asset_import_consents WHERE organization_id=org
  AND source_key=source_value ORDER BY revision DESC LIMIT 1 FOR SHARE;
 SELECT * INTO current_row FROM public.canonical_external_asset_calibration_consents WHERE organization_id=org
  AND source_key=source_value AND purpose='imported_asset_calibration_v1' ORDER BY revision DESC LIMIT 1 FOR UPDATE;
 IF (body->>'expectedRevision')::bigint<>COALESCE(current_row.revision,0)
  ...
  RAISE EXCEPTION 'Vehicle and equipment calibration consent changed' USING ERRCODE='40001',CONSTRAINT='imported_asset_calibration_consent_stale'; END IF;
 ...
 IF body->>'action'='grant' AND (source_consent.id IS NULL OR source_consent.action<>'grant'
  ...
  RAISE EXCEPTION 'Vehicle and equipment outcome consent inactive' USING ERRCODE='40001',CONSTRAINT='imported_asset_calibration_outcome_consent_stale'; END IF;
 ...
 digest_value:=public.canonical_completion_digest(jsonb_build_object('organizationId',org,'sourceKey',source_value,
  'purpose','imported_asset_calibration_v1','revision',next_revision,'previousId',current_row.id,
  ...
  'consentVersion','m25-imported-asset-calibration-consent-v1','reason',body->>'reason','requestDigest',request_hash));
 INSERT INTO public.canonical_external_asset_calibration_consents(organization_id,source_key,purpose,revision,previous_id,
 ...
 VALUES(org,source_value,'imported_asset_calibration_v1',next_revision,current_row.id,body->>'action',
  outcome_consent.id,outcome_consent.revision,rtrim(outcome_consent.canonical_digest),actor,(authority->>'membershipId')::uuid,
  session_value,scope_value,'m25-imported-asset-calibration-consent-v1',body->>'reason',key_hash,request_hash,digest_value)
 RETURNING * INTO inserted;
 RETURN jsonb_build_object('consent',public.canonical_imported_asset_calibration_consent_projection(inserted),'replayed',FALSE);
END $$;

CREATE FUNCTION public.canonical_imported_asset_calibration_metric(observations JSONB,metric_key TEXT)
...
BEGIN
 IF metric_key NOT IN ('utilization','operatingCost') OR jsonb_typeof(observations)<>'array' THEN
  RAISE EXCEPTION 'Vehicle and equipment calibration metric input invalid' USING ERRCODE='22023'; END IF;
 label_value:=CASE metric_key WHEN 'utilization' THEN 'Equipment utilization' ELSE 'Equipment operating cost' END;
...
CREATE FUNCTION public.canonical_imported_asset_calibration_basis(org UUID,source_value TEXT,service_value TEXT)
RETURNS JSONB LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path=pg_catalog,public,pg_temp AS $$
DECLARE source_consent public.canonical_external_asset_import_consents%ROWTYPE;
 outcome_consent public.canonical_external_asset_outcome_consents%ROWTYPE;
...
BEGIN
 IF source_value IS NULL OR source_value!~'^[a-z0-9][a-z0-9._-]{1,63}$' OR service_value IS NULL
  OR service_value!~'^[a-z0-9][a-z0-9._-]{1,63}$' THEN RAISE EXCEPTION 'Vehicle and equipment calibration identity invalid' USING ERRCODE='22023'; END IF;
 SELECT * INTO source_consent FROM public.canonical_external_asset_import_consents WHERE organization_id=org
  AND source_key=source_value ORDER BY revision DESC LIMIT 1;
 SELECT * INTO outcome_consent FROM public.canonical_external_asset_outcome_consents WHERE organization_id=org
  AND source_key=source_value AND purpose='imported_asset_utilization_cost_variance_v1' ORDER BY revision DESC LIMIT 1;
 IF source_consent.id IS NULL OR source_consent.action<>'grant' OR outcome_consent.id IS NULL OR outcome_consent.action<>'grant'
  ...
  OR rtrim(outcome_consent.source_consent_digest)<>rtrim(source_consent.canonical_digest) THEN
  RAISE EXCEPTION 'Current vehicle and equipment outcome consent is required'
   USING ERRCODE='P0002',CONSTRAINT='imported_asset_calibration_outcome_consent_unavailable'; END IF;
 ...
 IF candidate_total>10000 THEN RAISE EXCEPTION 'Vehicle and equipment calibration candidate set is too broad for bounded review'
  USING ERRCODE='P0002',CONSTRAINT='imported_asset_calibration_sample_scope_too_broad'; END IF;
 ...
 LOOP
  BEGIN live_basis:=public.canonical_imported_asset_learning_basis(org,source_value,item.estimate_id,item.external_job_reference);
 ...
 END LOOP;
 IF fresh_total<5 THEN RAISE EXCEPTION 'At least five current vehicle and equipment outcomes are required'
  USING ERRCODE='P0002',CONSTRAINT='imported_asset_calibration_sample_insufficient'; END IF;
...
CREATE FUNCTION public.canonical_imported_asset_calibration_hidden_projection(value public.canonical_external_asset_calibration_proposals)
RETURNS JSONB LANGUAGE sql STABLE SECURITY DEFINER SET search_path=pg_catalog,public,pg_temp AS $$
...
CREATE FUNCTION public.canonical_imported_asset_calibration_propose(org UUID,actor UUID,role_value TEXT,session_value UUID,
...
RETURNS JSONB LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public,pg_temp AS $$
DECLARE authority JSONB; consent_row public.canonical_external_asset_calibration_consents%ROWTYPE;
 source_consent public.canonical_external_asset_import_consents%ROWTYPE;
 outcome_consent public.canonical_external_asset_outcome_consents%ROWTYPE;
 current_row public.canonical_external_asset_calibration_proposals%ROWTYPE;
 replay_row public.canonical_external_asset_calibration_proposals%ROWTYPE;
 inserted public.canonical_external_asset_calibration_proposals%ROWTYPE;
...
BEGIN
 ...
 IF source_value IS NULL OR source_value!~'^[a-z0-9][a-z0-9._-]{1,63}$' OR service_value IS NULL
  ...
  OR confirmation_version_value IS DISTINCT FROM 'm25-imported-asset-calibration-proposal-v1' THEN
 ...
 PERFORM pg_advisory_xact_lock(hashtextextended(org::text||':imported-asset-calibration:'||source_value||':'||service_value,0));
 SELECT * INTO consent_row FROM public.canonical_external_asset_calibration_consents WHERE organization_id=org
  AND source_key=source_value AND purpose='imported_asset_calibration_v1' ORDER BY revision DESC LIMIT 1 FOR SHARE;
 SELECT * INTO outcome_consent FROM public.canonical_external_asset_outcome_consents WHERE organization_id=org
  AND source_key=source_value AND purpose='imported_asset_utilization_cost_variance_v1' ORDER BY revision DESC LIMIT 1 FOR SHARE;
 SELECT * INTO source_consent FROM public.canonical_external_asset_import_consents WHERE organization_id=org
  AND source_key=source_value ORDER BY revision DESC LIMIT 1 FOR SHARE;
 SELECT * INTO replay_row FROM public.canonical_external_asset_calibration_proposals WHERE organization_id=org
   AND actor_user_id=actor AND request_key_hash=key_hash;
 IF FOUND THEN
   ...
   active_value:=source_consent.id IS NOT NULL AND source_consent.action='grant' AND outcome_consent.id IS NOT NULL
    AND outcome_consent.action='grant' AND outcome_consent.source_consent_id=source_consent.id
    AND rtrim(outcome_consent.source_consent_digest)=rtrim(source_consent.canonical_digest)
    AND consent_row.id IS NOT NULL AND consent_row.action='grant' AND consent_row.outcome_consent_id=outcome_consent.id
    AND rtrim(consent_row.outcome_consent_digest)=rtrim(outcome_consent.canonical_digest)
    AND replay_row.consent_id=consent_row.id;
   IF NOT active_value THEN
    RETURN jsonb_build_object('proposal',public.canonical_imported_asset_calibration_hidden_projection(replay_row),'replayed',TRUE);
   END IF;
   BEGIN basis:=public.canonical_imported_asset_calibration_basis(org,source_value,service_value);
 ...
 IF source_consent.id IS NULL OR source_consent.action<>'grant' OR consent_row.id IS NULL OR consent_row.action<>'grant'
  ...
  RAISE EXCEPTION 'Active vehicle and equipment calibration consent changed' USING ERRCODE='40001',CONSTRAINT='imported_asset_calibration_consent_stale'; END IF;
 basis:=public.canonical_imported_asset_calibration_basis(org,source_value,service_value);
 SELECT * INTO replay_row FROM public.canonical_external_asset_calibration_proposals WHERE organization_id=org
 ...
 IF FOUND THEN RAISE EXCEPTION 'The current vehicle and equipment calibration was already proposed'
  USING ERRCODE='22023',CONSTRAINT='imported_asset_calibration_already_current'; END IF;
 SELECT * INTO current_row FROM public.canonical_external_asset_calibration_proposals WHERE organization_id=org
 ...
 INSERT INTO public.canonical_external_asset_calibration_proposals(organization_id,source_key,service_key,revision,
 ...
 VALUES(org,source_value,service_value,next_revision,current_row.id,consent_row.id,consent_row.revision,
  ...
  reason_value,confirmed_value,confirmation_version_value,'m25-imported-asset-median-calibration-v1',
  key_hash,request_hash,digest_value) RETURNING * INTO inserted;
 RETURN jsonb_build_object('proposal',public.canonical_imported_asset_calibration_projection(inserted),'replayed',FALSE);
END $$;

CREATE FUNCTION public.canonical_imported_asset_calibration_read(org UUID,actor UUID,role_value TEXT,session_value UUID,
 source_value TEXT,service_value TEXT)
RETURNS JSONB LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path=pg_catalog,public,pg_temp AS $$
DECLARE consent_row public.canonical_external_asset_calibration_consents%ROWTYPE;
 source_consent public.canonical_external_asset_import_consents%ROWTYPE;
 outcome_consent public.canonical_external_asset_outcome_consents%ROWTYPE;
 current_row public.canonical_external_asset_calibration_proposals%ROWTYPE; basis JSONB; fresh BOOLEAN;
...
BEGIN
 ...
 IF source_consent.id IS NULL OR source_consent.action<>'grant' OR consent_row.id IS NULL OR consent_row.action<>'grant'
  ...
  'blockedReason','Current vehicle and equipment calibration consent is required.'); END IF;
 SELECT * INTO current_row FROM public.canonical_external_asset_calibration_proposals WHERE organization_id=org
 ...
 IF NOT FOUND THEN RETURN jsonb_build_object('sourceKey',source_value,'serviceKey',service_value,'activeConsent',TRUE,
  'consent',public.canonical_imported_asset_calibration_consent_projection(consent_row),'current',NULL,'history','[]'::jsonb,
 ...
 SELECT count(*) INTO total FROM public.canonical_external_asset_calibration_proposals WHERE organization_id=org
 ...
 FROM (SELECT * FROM public.canonical_external_asset_calibration_proposals WHERE organization_id=org
...
REVOKE ALL ON TABLE public.canonical_external_asset_calibration_consents,
 public.canonical_external_asset_calibration_proposals FROM PUBLIC;
REVOKE ALL ON FUNCTION public.canonical_imported_asset_calibration_metric_valid(JSONB) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.canonical_imported_asset_calibration_metrics_valid(JSONB) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.canonical_imported_asset_calibration_eligible_count(JSONB) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.canonical_imported_asset_calibration_consent_projection(public.canonical_external_asset_calibration_consents) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.canonical_imported_asset_calibration_metric(JSONB,TEXT) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.canonical_imported_asset_calibration_basis(UUID,TEXT,TEXT) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.canonical_imported_asset_calibration_masked_metrics(JSONB) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.canonical_imported_asset_calibration_projection(public.canonical_external_asset_calibration_proposals) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.canonical_imported_asset_calibration_hidden_projection(public.canonical_external_asset_calibration_proposals) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.canonical_imported_asset_calibration_consent_read(UUID,UUID,TEXT,UUID,TEXT) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.canonical_imported_asset_calibration_consent_mutate(UUID,UUID,TEXT,UUID,TEXT,TEXT,TEXT,JSONB) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.canonical_imported_asset_calibration_propose(UUID,UUID,TEXT,UUID,TEXT,TEXT,TEXT,TEXT,BIGINT,TEXT,TEXT,BOOLEAN,TEXT) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.canonical_imported_asset_calibration_read(UUID,UUID,TEXT,UUID,TEXT,TEXT) FROM PUBLIC;
